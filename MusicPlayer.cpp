...
void MusicPlayer::init() {
  ...
  smooth_transition =
      StringConverter::parseBool(cf.getSetting("smooth", "", "false"));
...
void MusicPlayer::toggleMusic(String filename, bool loop) {
  ...
  if (device) {
    ...
    stream =
        audiere::OutputStreamPtr(OpenSound(device, filename.c_str(), true));
    if (!stream) {
      LogManager::getSingleton().logMessage(
          "Audiere: Couldn't alloc audio source " + filename);
      // failure
      return;
    } else {
...
void MusicPlayer::upd(const FrameEvent &evt) {
  if (device && stream) {
    ...
    if (stream->isPlaying()) {
      if (fadesound) {
        if ((stream->getVolume() - fade_att * evt.timeSinceLastFrame) <
            fade_level) {
          playMusicOverride(cfn, cloop);
          fadesound = false;
          return;
        ...
        stream->setVolume(stream->getVolume() -
                          fade_att * evt.timeSinceLastFrame);
      }
      if (preg) {
        stream->setVolume(stream->getVolume() - (stream->getVolume() - modVol) *
