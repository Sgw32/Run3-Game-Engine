...
public:
  ...
  void createProjector(SceneNode *aNode) {
    ...
    // TextureManager::getSingleton().load("decal.png", "General", TEX_TYPE_2D,
    // 1); TextureManager::getSingleton().load("decal_filter.png", "General",
    // TEX_TYPE_1D, 1);
  }
  void makeMaterialReceiveDecal(const String &matName) {
    // get the material
    mProjectorNode->setPosition(Ogre::Vector3(mWeaponNode->getParentSceneNode()
                                                  ->getParentSceneNode()
                                                  ->getPosition()
                                                  .x,
                                              mWeaponNode->getParentSceneNode()
                                                  ->getParentSceneNode()
                                                  ->getPosition()
                                                  .y,
                                              mWeaponNode->getParentSceneNode()
    ...
    filterNode->setPosition(mProjectorNode->getPosition() +
                            Ogre::Vector3::NEGATIVE_UNIT_Z * 100);
    ...
    TextureUnitState *texState =
        pass->createTextureUnitState("bullet_concrete_hit.png");
