...
public:
  ...
  */
  void inspectRenderable(Ogre::Renderable *rend) { g_InspectRenderable = rend; }

  unsigned int getPixelCount() { return g_PixelCount; }

  int getPixelCount(Ogre::Renderable *rend) {
    if (occ.find(rend) != occ.end()) {
      int temp = occ[rend];
      occ[rend] = -1; // The most important part. Otherwise, after disappear,
                      // objects are not visible.
      //				Thus, without the affect to the OC code.
      //Thus, they won't appear.
      return temp;
    } else {
      return -1;
    }
  ...
  virtual void renderSingleObject(Ogre::Renderable *rend,
