...
public:
  ...
  void power();
  void setNearScript(String lua) { mNearLua = lua; }
  void setShutdownScript(String lua) { msLua = lua; }
  void logoff();
  void explode();
  void reset();
  void del();
  void setAllowVirtualDisplay(bool allow) { allowVirtualDisplay = allow; }
  SceneNode *getComp() { return mComp; }
  void activateMagic(bool act) { mActMagic = act; }
