...
CWeapon::~CWeapon() {}
//////////////////////////////////////////
void CWeapon::init(Ogre::SceneManager *SceneMgr, OgreNewt::World *world,
                   Ogre::Camera *camera, SoundManager *soundMgr,
                   Ogre::SceneNode *mAttachNode, Ogre::Root *_root,
                   SoundManager *sound) {
  if (!SceneMgr) {
...
void CWeapon::parse(String name, String fname, String type, String script) {
  ...
  if (type == "shockrifle") {
    ...
    wNode->translate(
        -1, -0.2, 3.1,
        SceneNode::TS_LOCAL); // wNode->translate(-1,0,3.2,SceneNode::TS_LOCAL);
    wNode->translate(Vector3(0, 0, 0), SceneNode::TS_LOCAL);
    // wNode->setOrientation(Quaternion(Radian(135),Vector3::NEGATIVE_UNIT_Y))
    wNode->setVisible(false);
    shrifle = new Shockrifle;
...
void CWeapon::addWeapon(String name) {
  LogManager::getSingleton().logMessage(
      "CWeapon was requested to add a weapon " + name);
  for (i = 0; i != weapons_n.size(); i++) {
    if (initialized && weapons_n[i] == name) {
      for (unsigned int j = 0; j != w_miniguns.size(); j++) {
        if (w_miniguns[j]->get_name() == weapons_n[i] &&
            !w_miniguns[j]->hasMinigun()) {
          LogManager::getSingleton().logMessage(
              "A new weapon class Minigun found!");
          cur_weapon = name;
          fstslot_c = -1;
          sndslot_c = -1;
          hide_all();
...
void CWeapon::Press(const OIS::KeyEvent &arg) {
  ...
  if (!global::getSingleton().computer_mode) {
    if (arg.key == OIS::KC_1) {
      ...
      LogManager::getSingleton().logMessage(
          StringConverter::toString(fstslot_c));
      sndslot_c = -1;
      for (int j = 0; j != fstslot.size(); j++) {
        ...
        if (canSelect(fstslot[j]) && (j > fstslot_c)) {
          LogManager::getSingleton().logMessage(
              StringConverter::toString(fstslot_c));
          LogManager::getSingleton().logMessage(
              "can select " + fstslot[j] + "j=" + StringConverter::toString(j));
          select(fstslot[j]);
          /*fstslot_c++;
