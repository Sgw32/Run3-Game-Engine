...
void CutScene::assign(Vector3 pos, Real length, String name, bool freezebefore,
                      bool unfreezeafter, bool splineanims, bool hidehud) {
  mName = name;
  this->length = length;
  mStartPosition = pos;
  fb = freezebefore;
...
String CutScene::getname() { return mName; }

void CutScene::setWait(Real wait) { wait_before_start = wait; }
...
bool CutScene::frameStarted(const Ogre::FrameEvent &ev) {
  ...
  if (!musicSync) {
    ...
    deltaTime =
        MusicPlayer::getSingleton().getPosition() * musicLength - timeBefore;
      ...
      if (mAnimState->getTimePosition() < mAnimState->getLength()) {
        ...
        for (i = 0; i != scripts.size(); i++) {
          ...
          if (!musicSync) {
            ...
            if (MusicPlayer::getSingleton().getPosition() * musicLength >
                scripts_s[i]) {
              LogManager::getSingleton().logMessage(
                  "Cutscene: Running lua script!");
              RunLuaScript(global::getSingleton().getLuaState(),
        ...
        for (j=scripts_s.begin();j!=scripts_s.end();j++)
        {
                ...
                if ((*j)==-1)
                {
                        LogManager::getSingleton().logMessage("Erasing a lua
        script..."); scripts_s.erase(j); k=scripts.begin(); advance(k,i);
                        scripts.erase(k);
                }
        }*/
        ...
        if (deltaTime < 3.0f) {
          // delta error
          if (skipping) {
            // LogManager::getSingleton().logMessage("Skipping cutscene
            // at:"+StringConverter::toString(deltaTime*speedcoef));
...
void CutScene::dispose2() {
  ...
  LogManager::getSingleton().logMessage(
      "Disposing CutScene 2. It has been used ingame");
  started = false;
  // LogManager::getSingleton().logMessage("Disposing CutScene 2. It has been
  // used ingame");
  camera_scenenode->detachAllObjects();
  // LogManager::getSingleton().logMessage("Disposing CutScene 2. It has been
  // used ingame"); mSceneMgr->destroyAnimation("CameraTrack"+getname());
  // LogManager::getSingleton().logMessage("Disposing CutScene 2. It has been
  // used ingame");
  mSceneMgr->destroyAnimationState("CameraTrack" + getname());
  // LogManager::getSingleton().logMessage("Disposing CutScene 2. It has been
  // used ingame"); if (time<=length)
...
void CutScene::dispose() {
  ...
  if (!disposed) {
    ...
    if (first) {
      LogManager::getSingleton().logMessage(
          "Disposing CutScene. It has been used ingame");
      started = false;
      camera_scenenode->detachAllObjects();
      ...
      LogManager::getSingleton().logMessage(
          "Disposing CutScene. It wasn't used ingame");
      camera_scenenode->detachAllObjects();
      mSceneMgr->destroyAnimation("CameraTrack" + getname());
      mSceneMgr->destroyAnimationState("CameraTrack" + getname());
      mSceneMgr->destroySceneNode(camera_scenenode);
