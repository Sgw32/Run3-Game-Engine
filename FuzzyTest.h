...
enum FUZZY_OBJECT_EVENT { ENABLE1, ENABLE2, ENABLE3, ENABLE4, ENABLE5 };

enum REG_TYPE_OBJECT2 { PID_CONTROL, FUZZY_CONTROL, FUZZY_CONTROL2 };
...
public:
  ...
  langVar() {}
  ~langVar() {}
  void setValue(float data) { mV = data; }
  void fuzzyfy() {
    float x = mV;
    if (x >= 0) {
      ltm = 0.0f;
...
if (HIGHM(htm))
{
...
#ifdef ALL_DEBUG
    cout << o1_htm << " " << o1_mtm << " " << o1_ltm << " " << o1_lt << " "
         << o1_mt << " " << o1_ht << endl;
    cout << o2_htm << " " << o2_mtm << " " << o2_ltm << " " << o2_lt << " "
         << o2_mt << " " << o2_ht << endl;
#endif
  }
  //  0  1
  ...
  //  0  1
  float calcCenterOfMassTrapezoid(float level, float x1, float x2, float x3,
                                  float x4) {
    // AREA
  ...
  void deffuzuficate() {
    ...
    if (ch2)
      cnt++;
#ifdef ALL_DEBUG
    cout << chm2 << " " << cmm2 << " " << clm2 << " " << cl2 << " " << cm2
         << " " << ch2 << endl;
#endif
    outputValue2 = out / cnt;
  }
  float getHighPlus() { return ht; }
  float getMediumPlus() { return mt; }
  float getLowPlus() { return lt; }
  float getHighMinus() { return htm; }
  float getMediumMinus() { return mtm; }
  float getLowMinus() { return ltm; }
  string getCurrentTerm() { return term; }

  float getOutputValue1() { return outputValue1; }
  float getOutputValue2() { return outputValue2; }
  string getOutputTerm1() { return outputterm1; }
  string getOutputTerm2() { return outputterm2; }
...
#endif
    motorin1 = fuzzyPitch.getOutputValue1() * 50; //  2   
                                                  // 
    motorin2 = fuzzyPitch.getOutputValue2() * 50;
  }
  void fuzzyfy(float pitch) {}
  void setPitch(float pitch) { p = pitch; }
  float getPitch() { return p; }
  float getPitchOmega() { return wp; }
...
class FuzzyObject {
public:
  FuzzyObject(String manName) {
    LogManager::getSingleton().logMessage(manName +
                                          " fuzzy object initialized!");
  }
  FuzzyObject() {};
  virtual ~FuzzyObject() {}
  ...
  virtual Quaternion get_orientation() { return Quaternion::IDENTITY; }
...
public:
  FuzzyTest();
  ~FuzzyTest();
  void init(Ogre::SceneManager *mSceneMgr) { scene = mSceneMgr; }
  ...
  void setUseInteract(bool interact) { mInteract = interact; }
  void enablePIDs() {}
...
public:
  ...
  float getPitch() { return get_orientation().getPitch().valueDegrees(); }
  ...
  OgreNewt::Body *addMotorGroup(Ogre::SceneNode *node, Entity *ent, Vector3 pos,
                                Vector3 scale, Real pscale, Quaternion rot);

  void parseMotor(Entity *ent, Vector3 pos, Vector3 refpos, Vector3 scale,
