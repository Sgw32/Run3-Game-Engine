 ...
 *    This function will play the sound and then stop. It will NOT repeat
 *playing. Use stopAudioSource() to stop a sound from playing if its still
 *playing
 *
 * 8. Call pauseAudio() or pauseAllAudio() to pause one or all sound(s).
 *    Call resumeAudio() or resumeAllAudio() to resume one or all paused
 *sound(s).
 *
...
public:
  ...
  std::map<unsigned int, String> sounds;
  SoundManager(void);
  ...
  // careful : testSound should not be used it doesn't use the resource
  // limitation
  void testSound(const char *wavFile);

  // Aquire an Audio Source
  // filename = pass in the sound file to play for this source (ex.
  // "myfile.wav") audioId   = returns the AudioSource identifier you will need
  // for the PlayAudioSource();
  bool loadAudio(std::string filename, unsigned int *audioId, bool loop);
  bool releaseAudio(unsigned int audioID);
  bool releaseAllAudio();

  void releaseBuffers();

  Real getAudioLength(unsigned int audioID) {

    ALint size, bits, channels, freq;
    alGetBufferi(mAudioSources[audioID], AL_SIZE, &size);
    alGetBufferi(mAudioSources[audioID], AL_BITS, &bits);
    alGetBufferi(mAudioSources[audioID], AL_CHANNELS, &channels);
    alGetBufferi(mAudioSources[audioID], AL_FREQUENCY, &freq);
    if (alGetError() != AL_NO_ERROR)
      return -1.0f;
    return (ALfloat)((ALuint)(size * 8) / channels / bits) / (ALfloat)freq;
  }

  Real getPhase(unsigned int audioID) {

    ALfloat phse;
    alGetSourcef(mAudioSources[audioID], AL_SEC_OFFSET, &phse);
    if (alGetError() != AL_NO_ERROR) {
      LogManager::getSingleton().logMessage("Error getting phase!");
      return 0.0f;
    }
    return (Real)phse;
  }

  bool setPhase(unsigned int audioId, Real phase);
  // deprecated
  ...
  bool setSound(unsigned int audioID, Vector3 position, Vector3 velocity,
                Vector3 direction, float maxDistance, bool playNow,
                bool forceRestart, float minGain, float rollOff,
                float distance);

  // bool setGain(unsigned int audioID, float gain);
