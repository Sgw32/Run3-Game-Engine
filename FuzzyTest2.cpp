...
void FuzzyTest2::setup(Ogre::SceneNode *node) {
  ...
  Ogre::Vector3 size =
      Vector3(fabs(max.x - min.x), fabs(max.y - min.y), fabs(max.z - min.z));
  // Rigid body
  OgreNewt::Collision *col =
      new OgreNewt::CollisionPrimitives::Box(mWorld, size);
  door_bod = new OgreNewt::Body(mWorld, col);
  door_bod->attachToNode(node);
  // initial position
  door_bod->setPositionOrientation(node->getPosition() - (center * scale),
                                   node->getOrientation());
  delete col;
  // Make it movable!
  Real mass = 10.0f;
  ...
  door_bod->setCustomForceAndTorqueCallback<FuzzyTest2>(
      &FuzzyTest2::force_callback, this);
  door_bod->setAutoFreeze(0);

  OgreNewt::Joint *joint;
  joint = new OgreNewt::BasicJoints::BallAndSocket(mWorld, door_bod, NULL,
                                                   node->_getDerivedPosition());
  // OgreNewt::BasicJoints::UpVector* hor = new
  // OgreNewt::BasicJoints::UpVector(mWorld,door_bod,Vector3::UNIT_X);
...
void FuzzyTest2::parseMotor(Entity *ent, Vector3 pos, Vector3 refpos,
                            Vector3 scale, Real pscale, Quaternion rot) {
  if (motor_cnt == 4)
    return;
  OgreNewt::World *mWorld = global::getSingleton().getWorld();

  SceneNode *nod = scene->getRootSceneNode()->createChildSceneNode(
      ent->getName(), refpos + pos * pscale, rot);
  nod->attachObject(ent);
  // nod->setPosition(pos*pscale);
  nod->setScale(scale * pscale);

  OgreNewt::Body *bod =
      addMotorGroup(nod, ent, refpos + pos * pscale, scale, pscale, rot);

  motor_cnt++;
  if (motor_cnt == 1) {
    bod->setCustomForceAndTorqueCallback<FuzzyTest2>(
        &FuzzyTest2::motor1_callback, this);
  }
  if (motor_cnt == 2) {
    bod->setCustomForceAndTorqueCallback<FuzzyTest2>(
        &FuzzyTest2::motor2_callback, this);
  }
  if (motor_cnt == 3) {
    bod->setCustomForceAndTorqueCallback<FuzzyTest2>(
        &FuzzyTest2::motor3_callback, this);
  }
  if (motor_cnt == 4) {
    bod->setCustomForceAndTorqueCallback<FuzzyTest2>(
        &FuzzyTest2::motor4_callback, this);
  }
  bod->setAutoFreeze(0);
  OgreNewt::Joint *joint = new OgreNewt::BasicJoints::BallAndSocket(
      mWorld, bod, door_bod, refpos + pos * pscale);
  OgreNewt::BasicJoints::UpVector *hor =
      new OgreNewt::BasicJoints::UpVector(mWorld, bod, Vector3::UNIT_Y);
  hors.push_back(hor);
...
OgreNewt::Body *FuzzyTest2::addMotorGroup(Ogre::SceneNode *node, Entity *ent,
                                          Vector3 pos, Vector3 scale,
                                          Real pscale, Quaternion rot) {
  OgreNewt::World *mWorld = global::getSingleton().getWorld();
...
void FuzzyTest2::force_callback(OgreNewt::Body *me) {
  ...
  // Quaternion mPitchRot =
  // Quaternion(Degree(mCpt->getPitch()),Vector3::UNIT_X);
  ...
  if (mNamedPipe) {
    myLog << currentDateTime().c_str() << " ANGLES "
          << StringConverter::toString(pitch).c_str() << " "
          << StringConverter::toString(roll).c_str() << "	OUTF	"
          ...
          << "	PID	" << StringConverter::toString(pidpitch).c_str()
...
void FuzzyTest2::motor1_callback(OgreNewt::Body *me) {
  ...
  if (reg_type == FUZZY_CONTROL2) {
    if (mNamedPipe)
      force =
          Ogre::Vector3(0, 100000.0f + mNamedPipe->getMotor1() * 900000.0f, 0);
  }
  force *= mass;
  LogManager::getSingleton().logMessage("Motor1 up:" +
                                        StringConverter::toString(force));
  Quaternion quat = get_orientation();
...
void FuzzyTest2::motor2_callback(OgreNewt::Body *me) {
  ...
  if (reg_type == FUZZY_CONTROL2) {
    if (mNamedPipe)
      force =
          Ogre::Vector3(0, 100000.0f + mNamedPipe->getMotor2() * 900000.0f, 0);
  }
  force *= mass;
  LogManager::getSingleton().logMessage("Motor2 up:" +
                                        StringConverter::toString(force));
  Quaternion quat = get_orientation();
...
void FuzzyTest2::motor3_callback(OgreNewt::Body *me) {
  ...
  if (reg_type == FUZZY_CONTROL2) {
    if (mNamedPipe)
      force =
          Ogre::Vector3(0, 100000.0f + mNamedPipe->getMotor3() * 900000.0f, 0);
  }
  force *= mass;
  LogManager::getSingleton().logMessage("Motor3 up:" +
                                        StringConverter::toString(force));
  Quaternion quat = get_orientation();
...
void FuzzyTest2::motor4_callback(OgreNewt::Body *me) {
  ...
  if (reg_type == PID_CONTROL) {
    force =
        Ogre::Vector3(0, 100000.0f + pidpitch * 900000 - pidroll * 900000, 0);
  }
  if (reg_type == FUZZY_CONTROL) {
  ...
  if (reg_type == FUZZY_CONTROL2) {
    if (mNamedPipe)
      force =
          Ogre::Vector3(0, 100000.0f + mNamedPipe->getMotor4() * 900000.0f, 0);
  }
  force *= mass;
  LogManager::getSingleton().logMessage("Motor4 up:" +
                                        StringConverter::toString(force));
  Quaternion quat = get_orientation();
...
void FuzzyTest2::setname(String name) { mName = name; }

String FuzzyTest2::getname() { return mName; }
...
void FuzzyTest2::fuzzySetup() {
  ...
  mediumminus =
      new FuzzySet(1.0f - 0.7f, 1.0f - 0.5f, 1.0f - 0.5f, 1.0f - 0.05f);
  ...
  if (ret_val < 0)
          {
                  LogManager::getSingleton().logMessage("Error Opening
tiltrotor.fcl");
...
void FuzzyTest2::buildRules() {
  ...
  FuzzyRuleAntecedent *pitchMediumPlusRollMediumPlus =
      new FuzzyRuleAntecedent();
  FuzzyRuleAntecedent *pitchZeroRollMediumPlus = new FuzzyRuleAntecedent();
  FuzzyRuleAntecedent *pitchMediumMinusRollMediumPlus =
      new FuzzyRuleAntecedent();
  ...
  FuzzyRuleAntecedent *pitchMediumPlusRollMediumMinus =
      new FuzzyRuleAntecedent();
  FuzzyRuleAntecedent *pitchZeroRollMediumMinus = new FuzzyRuleAntecedent();
  FuzzyRuleAntecedent *pitchMediumMinusRollMediumMinus =
      new FuzzyRuleAntecedent();
  FuzzyRuleAntecedent *pitchHighMinusRollMediumMinus =
      new FuzzyRuleAntecedent();
  ...
  FuzzyRuleAntecedent *pitchMediumMinusRollHighMinus =
      new FuzzyRuleAntecedent();
  ...
  // Motor1
  FuzzyRule *rpitchHighPlusRollHighPlus =
      new FuzzyRule(1, pitchHighPlusRollHighPlus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumPlusRollHighPlus =
      new FuzzyRule(2, pitchMediumPlusRollHighPlus, thenMotor1HighMinus);
  FuzzyRule *rpitchZeroRollHighPlus =
      new FuzzyRule(3, pitchZeroRollHighPlus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumMinusRollHighPlus =
      new FuzzyRule(4, pitchMediumPlusRollHighPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollHighPlus =
      new FuzzyRule(5, pitchHighMinusRollHighPlus, thenMotor1MediumPlus);

  FuzzyRule *rpitchHighPlusRollMediumPlus =
      new FuzzyRule(6, pitchHighPlusRollMediumPlus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumPlusRollMediumPlus =
      new FuzzyRule(7, pitchMediumPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollMediumPlus =
      new FuzzyRule(8, pitchZeroRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchMediumMinusRollMediumPlus =
      new FuzzyRule(9, pitchMediumPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollMediumPlus =
      new FuzzyRule(10, pitchHighMinusRollMediumPlus, thenMotor1MediumPlus);

  FuzzyRule *rpitchHighPlusRollZero =
      new FuzzyRule(11, pitchHighPlusRollZero, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumPlusRollZero =
      new FuzzyRule(12, pitchMediumPlusRollZero, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollZero =
      new FuzzyRule(13, pitchZeroRollZero, thenMotor1Zero);
  FuzzyRule *rpitchMediumMinusRollZero =
      new FuzzyRule(14, pitchMediumPlusRollZero, thenMotor1MediumPlus);
  FuzzyRule *rpitchHighMinusRollZero =
      new FuzzyRule(15, pitchHighMinusRollZero, thenMotor1MediumPlus);

  FuzzyRule *rpitchHighPlusRollMediumMinus =
      new FuzzyRule(16, pitchHighPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchMediumPlusRollMediumMinus =
      new FuzzyRule(17, pitchMediumPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollMediumMinus =
      new FuzzyRule(18, pitchZeroRollMediumMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollMediumMinus =
      new FuzzyRule(19, pitchMediumPlusRollMediumMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchHighMinusRollMediumMinus =
      new FuzzyRule(20, pitchHighMinusRollMediumMinus, thenMotor1HighPlus);

  FuzzyRule *rpitchHighPlusRollHighMinus =
      new FuzzyRule(21, pitchHighPlusRollHighMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollHighMinus =
      new FuzzyRule(22, pitchMediumPlusRollHighMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchZeroRollHighMinus =
      new FuzzyRule(23, pitchZeroRollHighMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollHighMinus =
      new FuzzyRule(24, pitchMediumPlusRollHighMinus, thenMotor1HighPlus);
  FuzzyRule *rpitchHighMinusRollHighMinus =
      new FuzzyRule(25, pitchHighMinusRollHighMinus, thenMotor1HighPlus);
  // Motor2
  FuzzyRule *rpitchHighPlusRollHighPlus2 =
      new FuzzyRule(1, pitchHighPlusRollHighPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollHighPlus2 =
      new FuzzyRule(2, pitchMediumPlusRollHighPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchZeroRollHighPlus2 =
      new FuzzyRule(3, pitchZeroRollHighPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollHighPlus2 =
      new FuzzyRule(4, pitchMediumPlusRollHighPlus, thenMotor1HighPlus);
  FuzzyRule *rpitchHighMinusRollHighPlus2 =
      new FuzzyRule(5, pitchHighMinusRollHighPlus, thenMotor1HighPlus);

  FuzzyRule *rpitchHighPlusRollMediumPlus2 =
      new FuzzyRule(6, pitchHighPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchMediumPlusRollMediumPlus2 =
      new FuzzyRule(7, pitchMediumPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollMediumPlus2 =
      new FuzzyRule(8, pitchZeroRollMediumPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollMediumPlus2 =
      new FuzzyRule(9, pitchMediumPlusRollMediumPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchHighMinusRollMediumPlus2 =
      new FuzzyRule(10, pitchHighMinusRollMediumPlus, thenMotor1HighPlus);

  FuzzyRule *rpitchHighPlusRollZero2 =
      new FuzzyRule(11, pitchHighPlusRollZero, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumPlusRollZero2 =
      new FuzzyRule(12, pitchMediumPlusRollZero, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollZero2 =
      new FuzzyRule(13, pitchZeroRollZero, thenMotor1Zero);
  FuzzyRule *rpitchMediumMinusRollZero2 =
      new FuzzyRule(14, pitchMediumPlusRollZero, thenMotor1MediumPlus);
  FuzzyRule *rpitchHighMinusRollZero2 =
      new FuzzyRule(15, pitchHighMinusRollZero, thenMotor1MediumPlus);

  FuzzyRule *rpitchHighPlusRollMediumMinus2 =
      new FuzzyRule(16, pitchHighPlusRollMediumMinus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumPlusRollMediumMinus2 =
      new FuzzyRule(17, pitchMediumPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollMediumMinus2 =
      new FuzzyRule(18, pitchZeroRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchMediumMinusRollMediumMinus2 =
      new FuzzyRule(19, pitchMediumPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollMediumMinus2 =
      new FuzzyRule(20, pitchHighMinusRollMediumMinus, thenMotor1MediumPlus);

  FuzzyRule *rpitchHighPlusRollHighMinus2 =
      new FuzzyRule(21, pitchHighPlusRollHighMinus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumPlusRollHighMinus2 =
      new FuzzyRule(22, pitchMediumPlusRollHighMinus, thenMotor1HighMinus);
  FuzzyRule *rpitchZeroRollHighMinus2 =
      new FuzzyRule(23, pitchZeroRollHighMinus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumMinusRollHighMinus2 =
      new FuzzyRule(24, pitchMediumPlusRollHighMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollHighMinus2 =
      new FuzzyRule(25, pitchHighMinusRollHighMinus, thenMotor1MediumPlus);
  // Motor3
  FuzzyRule *rpitchHighPlusRollHighPlus3 =
      new FuzzyRule(1, pitchHighPlusRollHighPlus, thenMotor1HighPlus);
  FuzzyRule *rpitchMediumPlusRollHighPlus3 =
      new FuzzyRule(2, pitchMediumPlusRollHighPlus, thenMotor1HighPlus);
  FuzzyRule *rpitchZeroRollHighPlus3 =
      new FuzzyRule(3, pitchZeroRollHighPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollHighPlus3 =
      new FuzzyRule(4, pitchMediumPlusRollHighPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchHighMinusRollHighPlus3 =
      new FuzzyRule(5, pitchHighMinusRollHighPlus, thenMotor1MediumPlus);

  FuzzyRule *rpitchHighPlusRollMediumPlus3 =
      new FuzzyRule(6, pitchHighPlusRollMediumPlus, thenMotor1HighPlus);
  FuzzyRule *rpitchMediumPlusRollMediumPlus3 =
      new FuzzyRule(7, pitchMediumPlusRollMediumPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchZeroRollMediumPlus3 =
      new FuzzyRule(8, pitchZeroRollMediumPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollMediumPlus3 =
      new FuzzyRule(9, pitchMediumPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollMediumPlus3 =
      new FuzzyRule(10, pitchHighMinusRollMediumPlus, thenMotor1HighPlus);

  FuzzyRule *rpitchHighPlusRollZero3 =
      new FuzzyRule(11, pitchHighPlusRollZero, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollZero3 =
      new FuzzyRule(12, pitchMediumPlusRollZero, thenMotor1MediumPlus);
  FuzzyRule *rpitchZeroRollZero3 =
      new FuzzyRule(13, pitchZeroRollZero, thenMotor1Zero);
  FuzzyRule *rpitchMediumMinusRollZero3 =
      new FuzzyRule(14, pitchMediumPlusRollZero, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollZero3 =
      new FuzzyRule(15, pitchHighMinusRollZero, thenMotor1HighMinus);

  FuzzyRule *rpitchHighPlusRollMediumMinus3 =
      new FuzzyRule(16, pitchHighPlusRollMediumMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollMediumMinus3 =
      new FuzzyRule(17, pitchMediumPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollMediumMinus3 =
      new FuzzyRule(18, pitchZeroRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchMediumMinusRollMediumMinus3 =
      new FuzzyRule(19, pitchMediumPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollMediumMinus3 =
      new FuzzyRule(20, pitchHighMinusRollMediumMinus, thenMotor1HighMinus);

  FuzzyRule *rpitchHighPlusRollHighMinus3 =
      new FuzzyRule(21, pitchHighPlusRollHighMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollHighMinus3 =
      new FuzzyRule(22, pitchMediumPlusRollHighMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollHighMinus3 =
      new FuzzyRule(23, pitchZeroRollHighMinus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumMinusRollHighMinus3 =
      new FuzzyRule(24, pitchMediumPlusRollHighMinus, thenMotor1HighMinus);
  FuzzyRule *rpitchHighMinusRollHighMinus3 =
      new FuzzyRule(25, pitchHighMinusRollHighMinus, thenMotor1HighMinus);

  // Motor4
  FuzzyRule *rpitchHighPlusRollHighPlus4 =
      new FuzzyRule(1, pitchHighPlusRollHighPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollHighPlus4 =
      new FuzzyRule(2, pitchMediumPlusRollHighPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollHighPlus4 =
      new FuzzyRule(3, pitchZeroRollHighPlus, thenMotor1HighMinus);
  FuzzyRule *rpitchMediumMinusRollHighPlus4 =
      new FuzzyRule(4, pitchMediumPlusRollHighPlus, thenMotor1HighMinus);
  FuzzyRule *rpitchHighMinusRollHighPlus4 =
      new FuzzyRule(5, pitchHighMinusRollHighPlus, thenMotor1HighMinus);

  FuzzyRule *rpitchHighPlusRollMediumPlus4 =
      new FuzzyRule(6, pitchHighPlusRollMediumPlus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollMediumPlus4 =
      new FuzzyRule(7, pitchMediumPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchZeroRollMediumPlus4 =
      new FuzzyRule(8, pitchZeroRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchMediumMinusRollMediumPlus4 =
      new FuzzyRule(9, pitchMediumPlusRollMediumPlus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollMediumPlus4 =
      new FuzzyRule(10, pitchHighMinusRollMediumPlus, thenMotor1HighMinus);

  FuzzyRule *rpitchHighPlusRollZero4 =
      new FuzzyRule(11, pitchHighPlusRollZero, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumPlusRollZero4 =
      new FuzzyRule(12, pitchMediumPlusRollZero, thenMotor1MediumPlus);
  FuzzyRule *rpitchZeroRollZero4 =
      new FuzzyRule(13, pitchZeroRollZero, thenMotor1Zero);
  FuzzyRule *rpitchMediumMinusRollZero4 =
      new FuzzyRule(14, pitchMediumPlusRollZero, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollZero4 =
      new FuzzyRule(15, pitchHighMinusRollZero, thenMotor1HighMinus);

  FuzzyRule *rpitchHighPlusRollMediumMinus4 =
      new FuzzyRule(16, pitchHighPlusRollMediumMinus, thenMotor1HighPlus);
  FuzzyRule *rpitchMediumPlusRollMediumMinus4 =
      new FuzzyRule(17, pitchMediumPlusRollMediumMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchZeroRollMediumMinus4 =
      new FuzzyRule(18, pitchZeroRollMediumMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollMediumMinus4 =
      new FuzzyRule(19, pitchMediumPlusRollMediumMinus, thenMotor1MediumMinus);
  FuzzyRule *rpitchHighMinusRollMediumMinus4 =
      new FuzzyRule(20, pitchHighMinusRollMediumMinus, thenMotor1MediumMinus);

  FuzzyRule *rpitchHighPlusRollHighMinus4 =
      new FuzzyRule(21, pitchHighPlusRollHighMinus, thenMotor1HighPlus);
  FuzzyRule *rpitchMediumPlusRollHighMinus4 =
      new FuzzyRule(22, pitchMediumPlusRollHighMinus, thenMotor1HighPlus);
  FuzzyRule *rpitchZeroRollHighMinus4 =
      new FuzzyRule(23, pitchZeroRollHighMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchMediumMinusRollHighMinus4 =
      new FuzzyRule(24, pitchMediumPlusRollHighMinus, thenMotor1MediumPlus);
  FuzzyRule *rpitchHighMinusRollHighMinus4 =
      new FuzzyRule(25, pitchHighMinusRollHighMinus, thenMotor1MediumPlus);
