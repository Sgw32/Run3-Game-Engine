...
public:
  func_door();
  ~func_door();
  void init(Ogre::SceneManager *mSceneMgr) { this->mSceneMgr = mSceneMgr; }
  void setDistanceDirection(Ogre::Real distance,
                            Ogre::Vector3 dir = Ogre::Vector3::UNIT_Z,
                            Ogre::Real speed = 100);

  void transformToFuncRot() { funcrot = true; }
  ...
  void setRotSpeed(Real rspeed) { rotSpeed = rspeed; }
  ...
  void setLockedLua(String lua) { lLocked = lua; }
  ...
  void allowPRegulator(bool allowP) { allowPReg = allowP; }
  void setAcceleration(Real acc) { mAcceleration = acc; }
  Real getAcceleration() { return mAcceleration; }
