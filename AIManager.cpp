...
AIManager::AIManager() { mNodeGen = 0; }
...
void AIManager::init(int AIType, Vector3 nodeGen, bool autoGenNodes) {
  ...
  if (autoGenNodes) {
    this->mNodeGen = new nodeGenerator(
        nodeGen, global::getSingleton().getSceneManager(), 300, true);
  ...
  for (unsigned int i = 0; i != grps.size(); i++) {
    if (grps[i]->getName() == name) {
      ...
      for (unsigned int j = 0; j != grp->nodes.size(); j++) {
        ...
        LogManager::getSingleton().logMessage("processing group element..." +
                                              aiel->getName());
        LogManager::getSingleton().logMessage(aiel->getCommand1());
        LogManager::getSingleton().logMessage(aiel->getCommand2());
        NPCManager::getSingleton().npc_event(
            aiel->getName(), aiel->getCommand1(), aiel->getCommand2());
      }
    }
  }
}
