...
public:
  ...
  /*
               ...
               cairo_select_ft_font(cr,
     ML_GetWindowsPath("fonts/tahomabd.ttf")); cairo_set_font_size(cr, 90.0);
          ...
          virtual void renderQueueStarted(Ogre::uint8 queueGroupId, const
     Ogre::String &invocation, bool &skipThisInvocation)
           {
             // very important
             // first check the invocation  and queueGroupId parameters
             if(  queueGroupId == RENDER_QUEUE_OVERLAY && invocation !=
     "SHADOWS")
              {
                      ML_UpdateTimer();
                      ML_Use2DView();
                              ...
                              for(float i=0;i<512;i++)
                               {
                                      float temp = i+xstart;
                                      if(temp>0 || temp<512)
                                            ML_DrawSprite(rastaImage, temp,
     ycenter + sin((i+angle)*freq*3.141592654/180.0)*scale);
                               }
                      ...
                      for(float i=0;i<512;i++)
                               {
                                     ...
                                     if(temp>0 || temp<512)
                                            ML_DrawSprite(rastaImage, temp,
     ycenter + sin((i+angle1)*freq1*3.141592654/180.0)*scale);
                               }
                      ...
                      for(float i=0;i<512;i++)
                               {
                                     ...
                                     if(temp>0 || temp<512)
                                           ML_DrawSprite(rastaImage, temp,
     ycenter + sin((i+angle2)*freq2*3.141592654/180.0)*scale);
                               }
          ...
          /// Called by Ogre, for being a render queue listener
          virtual void renderQueueEnded(Ogre::uint8 queueGroupId, const
     Ogre::String &invocation, bool &repeatThisInvocation)
           {
                if(  queueGroupId == RENDER_QUEUE_OVERLAY && invocation !=
     "SHADOWS")
                 {
