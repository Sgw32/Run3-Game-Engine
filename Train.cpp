...
void Train::setAcceleration(Real acc) { mAcceleration = acc; }
...
void Train::addDetail(Entity *ent, Vector3 pos, Vector3 scale, Quaternion rot) {
  ...
  OgreNewt::Collision *col2 =
      new OgreNewt::CollisionPrimitives::ConvexHull(w, d, rot, pos);
  // OgreNewt::co
...
void Train::addNoCollide(Entity *ent, Vector3 pos, Vector3 scale,
                         Quaternion rot) {
  LogManager::getSingleton().logMessage("3");
  SceneNode *d = nod->createChildSceneNode(pos, rot);
  LogManager::getSingleton().logMessage("4");
  ...
  d->setScale(scale * nod->getScale());
  LogManager::getSingleton().logMessage("2");
  // OgreNewt::Collision* col2 = new
  // OgreNewt::CollisionPrimitives::ConvexHull(w,d,rot,pos); OgreNewt::co
  // OgreNewt::CollisionPrimitives::ConvexHull(
  // vc.clear();
...
void Train::finish_build() {
  ...
  OgreNewt::Collision *col3 =
      new OgreNewt::CollisionPrimitives::CompoundCollision(w, vc);
  bod = new OgreNewt::Body(w, col3);
  bod->attachToNode(nod);
  bod->setName(nod->getName());

  bod->setPositionOrientation(nod->getPosition(),
                              nod->getOrientation()); //,Quaternion::IDENTITY);//
                                                      //nod->getOrientation() );
  Vector3 inertia =
      OgreNewt::MomentOfInertia::CalcBoxSolid(2000, aab.getSize());
  bod->setMassMatrix(2000, inertia);
  OgreNewt::BasicJoints::UpVector *uv2 =
      new OgreNewt::BasicJoints::UpVector(w, bod, Vector3::UNIT_Y);
...
void Train::init(String name, SceneNode *sn, Real speed) {
  ...
  // OgreNewt::CollisionPrimitives::Box(w,aab.getSize()*nod->getScale());
  OgreNewt::Collision *col =
      new OgreNewt::CollisionPrimitives::ConvexHull(w, nod);
  ...
  bod->setPositionOrientation(
      nod->getPosition(), nod->getOrientation()); // nod->getOrientation() );
  Vector3 inertia = OgreNewt::MomentOfInertia::CalcBoxSolid(
      2000, aab.getSize() * nod->getScale());
  bod->setMassMatrix(2000, inertia);
  OgreNewt::BasicJoints::UpVector *uv2 =
      new OgreNewt::BasicJoints::UpVector(w, bod, Vector3::UNIT_Y);
  sMgr = global::getSingleton().getSoundManager();
  /*OgreNewt::BasicJoints::UpVector* uv1 = new
OgreNewt::BasicJoints::UpVector(w,bod,Vector3::UNIT_X);
  OgreNewt::BasicJoints::UpVector* uv3 = new
OgreNewt::BasicJoints::UpVector(w,bod,Vector3::UNIT_Z);
/*TrainMat* mPhysCallback = new TrainMat;
const OgreNewt::MaterialID* mMatDefault =
global::getSingleton().getPlayer()->getPlayerMat(); const OgreNewt::MaterialID*
trainMat= new OgreNewt::MaterialID( w ); OgreNewt::MaterialPair* mMatPair = new
OgreNewt::MaterialPair( w, mMatDefault, trainMat );
...
void Train::setFixOrient(bool x) {
  ...
  if (setor) {
    ...
    Vector3 myDir = Quaternion(Radian(Degree(180)), Vector3::UNIT_Y) * myRot *
  ...
  if (moving) {
    ...
    if (setor) {
      ...
      if (next != Vector3::ZERO) {
        ...
        /*LogManager::getSingleton().logMessage("Passed node is
        "+StringConverter::toString(dest));
        LogManager::getSingleton().logMessage("Next node is
        "+StringConverter::toString(next));
        LogManager::getSingleton().logMessage("My orientation is
        "+StringConverter::toString(myRot));*/

        Vector3 myDir = Quaternion(Radian(Degree(mYawAngle)), Vector3::UNIT_Y) *
                        myRot * Vector3::NEGATIVE_UNIT_Z;
        ...
        /*LogManager::getSingleton().logMessage("My direction is
        "+StringConverter::toString(myDir));
        LogManager::getSingleton().logMessage("Path direction is
        "+StringConverter::toString(pathDir));
        LogManager::getSingleton().logMessage("All delta direction
        "+StringConverter::toString(deltaDir));
        LogManager::getSingleton().logMessage("Time-byp delta
        "+StringConverter::toString(deltaOr));
        LogManager::getSingleton().logMessage("Delta quat
        "+StringConverter::toString(deltaRot));
        LogManager::getSingleton().logMessage("Final orientation is
        "+StringConverter::toString(myRot));*/
      }
    ...
    sMgr->setSound(sound, getpos(body), Vector3(1, 1, 1), Vector3(1, 1, 1),
                   1000, false, false, 200, 50, 800);
  } else {
    if (body->getType() == 6) {
...
void Train::start() {
  ...
  if (startSound != "none")
    Run3SoundRuntime::getSingleton().emitSound(startSound, soundDuration, false,
                                               getpos(bod), 200, 50, 800);
  moving = true;
  if (particleNode)
  ...
  for (int i = 0; i != nod->numChildren(); i++) {
    if ((SceneNode *)nod->getChild(i)) {
      if (((SceneNode *)nod->getChild(i))->getAttachedObject(0)) {
        if (global::getSingleton().getSceneManager()->hasEntity(
                (((SceneNode *)nod->getChild(i))
                     ->getAttachedObject(0)
                     ->getName()))) {
          LogManager::getSingleton().logMessage(
              "Train: destroying entity child");
          global::getSingleton().getSceneManager()->destroyEntity(
              (((SceneNode *)nod->getChild(i))
                   ->getAttachedObject(0)
                   ->getName()));
        } else {
          LogManager::getSingleton().logMessage(
              "Train: destroying particle system");

          if (global::getSingleton().getSceneManager()->hasParticleSystem(
                  (((SceneNode *)nod->getChild(i))
                       ->getAttachedObject(0)
                       ->getName())))
            global::getSingleton().getSceneManager()->destroyParticleSystem(
                (((SceneNode *)nod->getChild(i))
                     ->getAttachedObject(0)
                     ->getName()));
        }
...
void Train::stop() {
  ...
  if (stopSound != "none")
    Run3SoundRuntime::getSingleton().emitSound(stopSound, 4, false, getpos(bod),
                                               200, 50, 800);
  curKPoint = 1;
  if (movingSound != "none") {
    sMgr->stopAudio(sound);
...
void Train::pause() {
  if (moving) {
    ...
    if (stopSound != "none")
      Run3SoundRuntime::getSingleton().emitSound(stopSound, 4, false,
                                                 getpos(bod), 200, 50, 800);
    if (movingSound != "none") {
      sMgr->stopAudio(sound);
      sMgr->releaseAudio(sound);
