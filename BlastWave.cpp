...
void BlastWave::init() { mWorld = global::getSingleton().getWorld(); }
...
void BlastWave::blast(Vector3 pos, Vector3 size, Real power) {
  ...
  for (j = 0; j != d; j++) {
    for (i = 1; i != 360; i++) {
      //			OgreNewt::Body* bod2;
      String bod3;
      getBodyOnDegree(stdPos + size / 2 * j,
                      i); // LogManager::getSingleton().logMessage("BlastWave:
                          // scanning "+StringConverter::toString(i));
    }
  }
  // LogManager::getSingleton().logMessage("BlastWave");
}
...
void BlastWave::getBodyOnDegree(Ogre::Vector3 pos, int deg) {
  ...
  // LogManager::getSingleton().logMessage("BlastWave: direction
  // "+StringConverter::toString(direction) + " pos
  // "+StringConverter::toString(pos ) + " end "
  // +StringConverter::toString(end));
  ...
  if (bod2) {
    // LogManager::getSingleton().logMessage("BlastWave: harms
    // body:"+bod2->getName()+" "+StringConverter::toString(harm));
