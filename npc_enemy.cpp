...
void npc_enemy::init(NPCSpawnProps props) {
  ...
  // mSceneMgr->getRootSceneNode()->createChildSceneNode("npc_enemy_path_"+props.mName);
  // mDebugLines = new Ogre::ManualObject("npc_enemy_path_"+props.mName);
  // mRoot->addFrameListener(this);
  health = props.health;
}

void npc_enemy::setPlayerPosition(Vector3 pos) { pPosit = pos; }
...
void npc_enemy::spawnNPC() {
  ...
  OgreNewt::Collision *col = new OgreNewt::CollisionPrimitives::Box(
      mWorld, size * physSize,
      Quaternion(Radian(Degree(mProps.yShift)), Vector3::UNIT_Y), physPos);
  ...
  npcBody->setCustomForceAndTorqueCallback<npc_enemy>(
      &npc_enemy::npc_force_callback, this);
  ...
  if (mProps.strange_look) {
    ...
    Quaternion finalOrient =
        Quaternion::IDENTITY * Quaternion(Degree(180), Vector3::UNIT_X);
...
void npc_enemy::step(const Ogre::FrameEvent &evt) {
  ...
  if (going) {
    ...
    if (farFindB) {
      ...
      if (farFindB && path_find && !stra) {
        ...
        if (!dyeNow) {
          ...
          Vector3 dir =
              Vector3(dest.x, 0, dest.z) - Vector3(myPos.x, 0, myPos.z);
          dir.normalise();
          ...
          if (in_rad_precision) {
            ...
            if ((i) == mPath.size()) {
              ...
              dir.normalise();
              Vector3 dir1 =
                  Quaternion(Radian(Degree(mProps.yShift)), Vector3::UNIT_Y) *
                  getNPCDirection();
              Degree angle;
          ...
          if (rotation_needed) {
            ...
            if (rot_dir) {
              ...
              niNode->rotate(Vector3::UNIT_Y,
                             Degree(mProps.rotateSpeed *
                                    evt.timeSinceLastFrame * TIME_SHIFT));
            } else {
              // LogManager::getSingleton().logMessage(StringConverter::toString(angle)+"-");
              niNode->rotate(Vector3::UNIT_Y,
                             Degree(-mProps.rotateSpeed *
      ...
      if (!path_find || (!goal_achieved && stra)) {
        if (!dyeNow) {
          ...
          if ((sounds) && (mProps.sounds)) {
            if ((rand() % 50) == 2)
              Run3SoundRuntime::getSingleton().emitSound(
                  mProps.soundRandom1, 3, false, myPos, 200, 50, 800);
            if ((rand() % 50) == 3) {
              int sosel = rand() % 3 + 1;
              if (sosel == 1)
                Run3SoundRuntime::getSingleton().emitSound(
                    mProps.soundSubRandom21, 3, false, myPos, 200, 50, 800);
              if (sosel == 2)
                Run3SoundRuntime::getSingleton().emitSound(
                    mProps.soundSubRandom22, 3, false, myPos, 200, 50, 800);
              if (sosel == 3)
                Run3SoundRuntime::getSingleton().emitSound(
                    mProps.soundSubRandom23, 3, false, myPos, 200, 50, 800);
            }
          }
    ...
    if (mProps.strange_look) {
      Ogre::Node *m = skel->getBone(mProps.headBone);
      Quaternion finalOrient = Quaternion::IDENTITY *
                               Quaternion(Degree(angleSwitch), Vector3::UNIT_X);
      m->setOrientation(finalOrient);
      // Ogre::Node::
