...
public:
  ...
  bool quit() const { return m_bQuitProgram; }

  bool learn() const { return m_bNewData; }
...
private:
  ...
  bool null_input() const { return m_sInput.length() == 0; }

  bool bot_is_repeating() const { return (bot_repeat() || similar_response()); }

  bool user_repeat() const {
    return (m_sInput.length() > 0 && m_sInput == m_sPrevInput);
  }

  bool bot_understand() const { return response_list.size() > 0; }
  ...
  bool isGoodKey(const std::string key, const std::string bkey, int pos,
                 int bestp) const;
