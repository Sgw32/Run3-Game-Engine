...
#define BENCHMARK1(i)                                                          \
  if (!(Run3Benchmark::getSingleton().benchMarkIteration(i))) {                \
    delete XMLDoc;                                                             \
    return;                                                                    \
  }
#define BENCHMARK2(i)                                                          \
...
public:
  ...
  nodeProperty(const String &node, const String &propertyName,
      ...
      : nodeName(node), propertyNm(propertyName), valueName(value),
        typeName(type) {}
};
...
public:
  ...
  void parseDotScene(const String &SceneName, const String &groupName,
                     ...
                     OgreNewt::World *World = NULL, Camera *mCamera = NULL,
                     SoundManager *soundMgr = NULL, Player *ply = NULL);
  void mergeDotScene(const String &SceneName, const String &groupName,
                     SceneManager *yourSceneMgr, SceneNode *pAttachNode = NULL,
                     const String &sPrependNode = "",
                     OgreNewt::World *World = NULL, Camera *mCamera = NULL,
                     SoundManager *soundMgr = NULL, Player *ply = NULL);
  ...
  void util_processRagDoll(String name, String script, String meshFile,
                           SceneNode *pParent);
...
protected:
  ...
  void processEntity(TiXmlElement *XMLNode,
                     SceneNode *pParent); // modified by Sgw32
  void processPhysObj(TiXmlElement *XMLNode,
                      SceneNode *pParent); // added by Sgw32
  void processNoCollide(TiXmlElement *XMLNode,
                        SceneNode *pParent); // added by Sgw32
  void processBreakable(TiXmlElement *XMLNode,
                        SceneNode *pParent); // added by Sgw32
  // void processWood(TiXmlElement *XMLNode, SceneNode *pParent);	//added
  // by Sgw32
  void processPhysBlock(TiXmlElement *XMLNode,
                        SceneNode *pParent); // added by Sgw32
  void processPhysBox(TiXmlElement *XMLNode,
                      SceneNode *pParent); // added by Sgw32
  void processPhysCyl(TiXmlElement *XMLNode,
                      SceneNode *pParent); // added by Sgw32

  void processRagDoll(TiXmlElement *XMLNode,
                      SceneNode *pParent);                     // added by Sgw32
  ...
  void processMirror(TiXmlElement *XMLNode,
                     SceneNode *pParent); // added by Sgw32
  ...
  void processLightDynamics(TiXmlElement *XMLNode,
                            Light *pLight); // added by Sgw32
