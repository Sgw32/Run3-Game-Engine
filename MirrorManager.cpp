...
void MirrorManager::createNewMirror(String metalTex, Vector3 pos, Vector3 size,
                                    String name, Quaternion rot) {
  ...
  SceneNode *node =
      mSceneMgr->getRootSceneNode()->createChildSceneNode(pos, rot);
...
void MirrorManager::createNewMirror(String metalTex, SceneNode *node,
                                    Vector3 size) {
  World *mWorld = global::getSingleton().getWorld();
  SceneManager *mSceneMgr = global::getSingleton().getSceneManager();
  ...
  MeshManager::getSingleton().createPlane(
      node->getName() + "ReflectionPlane",
      ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, *mPlane, 1, 1, 1, 1,
      true, 1, 1, 1, Vector3::UNIT_Z);

  Entity *ent = mSceneMgr->createEntity(node->getName(),
                                        node->getName() + "ReflectionPlane");
  ...
  TexturePtr texture = TextureManager::getSingleton().createManual(
      node->getName() + "RttTex",
      ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, TEX_TYPE_2D, 512, 512,
...
(Real)mCamera->getViewport()->getActualHeight());
  ...
  t = mat->getTechnique(0)->getPass(0)->createTextureUnitState(node->getName() +
                                                               "RttTex");
  // Blend with base texture
  t->setColourOperationEx(LBX_BLEND_MANUAL, LBS_TEXTURE, LBS_CURRENT,
                          ColourValue::White, ColourValue::White, 0.9);
...
bool MirrorManager::frameRenderingQueued(const FrameEvent &evt) {
  ...
  for (i = reflectCams.begin(); i != reflectCams.end(); i++) {
    ...
    Quaternion rot =
        global::getSingleton().getCamera()->getDerivedOrientation();
