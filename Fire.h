...
class Fire {
public:
  Fire(SceneManager *scene, Vector3 pos, Vector3 size, String psys,
       Real renderDist) {
    mSceneMgr = scene;
  ...
  ~Fire() {};
  void fire() { fire_s->getEmitter(0)->setEnabled(true); }
  void extinguish() { fire_s->getEmitter(0)->setEnabled(false); }
  void toggle() {
