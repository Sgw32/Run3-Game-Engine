...
Player::~Player() {}
//////////////////////////////////////////
void Player::init(Ogre::SceneManager *scene, OgreNewt::World *world,
                  Ogre::Camera *cam, SoundManager *soundMgr,
                  Ogre::Root *_root) {
  if (!scene) {
  ...
  PlayerProps props;
  props.health =
      Ogre::StringConverter::parseReal(cf.getSetting("startHealth", "", "100"));
  props.elastic =
      Ogre::StringConverter::parseReal(cf.getSetting("elastic", "", "-1"));
  props.softeness =
      Ogre::StringConverter::parseReal(cf.getSetting("softness", "", "-1"));
  props.inertia_active = Ogre::StringConverter::parseBool(
      cf.getSetting("inertiaActive", "", "false"));
  props.kinematic_movement = Ogre::StringConverter::parseBool(
      cf.getSetting("kinematicMovement", "", "false"));
  props.upvectors =
      Ogre::StringConverter::parseBool(cf.getSetting("upVectors", "", "true"));
  props.splitvectors = Ogre::StringConverter::parseBool(
      cf.getSetting("splitVectors", "", "false"));
  props.dfoz = Ogre::StringConverter::parseBool(
      cf.getSetting("deactivateForcesOnZero", "", "false"));
  props.rotate_hor = Ogre::StringConverter::parseBool(
      cf.getSetting("rotateHotizon", "", "true"));
  props.resetVelocityOnRelease = Ogre::StringConverter::parseBool(
      cf.getSetting("resetVelocityOnRelease", "", "false"));
  props.runVel =
      Ogre::StringConverter::parseReal(cf.getSetting("runVel", "", "6"));
  props.stdVel =
      Ogre::StringConverter::parseReal(cf.getSetting("stdVel", "", "3"));
  props.flashConeI =
      Ogre::StringConverter::parseReal(cf.getSetting("flashConeI", "", "45"));
  props.flashConeU =
      Ogre::StringConverter::parseReal(cf.getSetting("flashConeO", "", "60"));
  props.range =
      Ogre::StringConverter::parseReal(cf.getSetting("Range", "", "2000"));
  props.stairEffect =
      Ogre::StringConverter::parseReal(cf.getSetting("stairEffect", "", "-1"));
  props.trainForce =
      Ogre::StringConverter::parseReal(cf.getSetting("trainForce", "", "400"));
  props.nearClip =
      Ogre::StringConverter::parseReal(cf.getSetting("nearClip", "", "10"));
  props.devMode =
      Ogre::StringConverter::parseBool(cf.getSetting("devMode", "", "false"));
  regen_Time =
      Ogre::StringConverter::parseReal(cf.getSetting("stdVel", "", "200"));
  allowsubtitles = props.allowsubtitles = Ogre::StringConverter::parseBool(
      cf.getSetting("allowSubtitles", "", "true"));
  props.allowflashlight = Ogre::StringConverter::parseBool(
      cf.getSetting("allowFlashLight", "", "true"));
  props.fov = FOV;
  stop_count = 0;
  stop_count_std = Ogre::StringConverter::parseReal(
      cf.getSetting("stop_count_std", "", "1"));
  mProps = props;
  // props.mPlayerName=
  sound = soundMgr;
  fstindex = 0;
  ...
  LogManager::getSingleton().logMessage(
      "FarClip is:" + StringConverter::toString(mProps.farClip));
  ...
  no_movement = true;
  mRotate = Ogre::StringConverter::parseReal(
      cf.getSetting("mouseSensivity", "", "0.26"));
  mMove = 20;
  gravity = -GRAVITY;
  y_rotation_cont = 0;
  y_limit_a = 90;
  ...
  bod->setPositionOrientation(mCamNode->getPosition(),
                              Ogre::Quaternion::IDENTITY);
  inertia = Ogre::StringConverter::parseVector3(
      cf.getSetting("inertia", "", "-1 -1 -1"));
  if (inertia == Vector3(-1, -1, -1))
    inertia = OgreNewt::MomentOfInertia::CalcEllipsoidSolid(
        Ogre::StringConverter::parseReal(cf.getSetting("mass", "", "40")),
        Ogre::StringConverter::parseVector3(
            cf.getSetting("sizeexp", "", "50 100 50")));
  bod->setMassMatrix(40, inertia);
  bod->setCustomForceAndTorqueCallback<Player>(
      &Player::camera_force_callback,
      this); // add the previous defined callback function as the body custom
  ...
  for (i=1;i!=4;i++)
  {
          ...
          String fname="";
          fname = fname +
  "run3/sounds/footsteps/"+"concrete"+StringConverter::toString(i)+".wav";
  ...
  if (Ogre::StringConverter::parseBool(
          cf.getSetting("frictionSet", "", "false"))) {
    LogManager::getSingleton().logMessage("Friction set!");
    mMatPair->setDefaultFriction(
        Ogre::StringConverter::parseReal(cf.getSetting("stat", "", "0")),
        Ogre::StringConverter::parseReal(cf.getSetting("kinetic", "", "0")));
  }
  ...
  CWeapon::getSingleton().init(mSceneMgr, mWorld, mCamera, sound, mViewNode,
                               root, sound);
  freezed = false;
  // flashlight section
  flashLight = 0;
...
flashLight->setAttenuation(1000, 1, 1, 1); // meter range.  the others our
shader ignores flashLight->setDirection(Ogre::Vector3(0, 0, -1));
  mViewNode->attachObject(flashLight);
  flashLight->setVisible(true);*/
  LogManager::getSingleton().logMessage(
      StringConverter::toString(mSceneMgr->hasLight("flashLight")) +
      "PLAYER!!!");
...
void Player::setAllowSubtitles(bool enable) {
  ...
  LogManager::getSingleton().logMessage(
      "Now allowsubtitles is:" + StringConverter::toString(allowsubtitles));
...
bool Player::getAllowSubtitles() { return allowsubtitles; }
...
void Player::toggleFlashLight() {
  if (mProps.allowflashlight) {
    ...
    /*}*/
    Run3SoundRuntime::getSingleton().emitSound("run3/sounds/flash01.wav", 2.0f,
                                               false);
    if (flashLight && (mSceneMgr->hasLight("flashLighter"))) {

      mViewNode->detachObject(flashLight);
      LogManager::getSingleton().logMessage(
          StringConverter::toString(mSceneMgr->hasLight("flashLighter")) +
          "PLAYERB!!!");
      if (flashLight)
      ...
      flashLight->setAttenuation(
          mProps.range, 1, 1, 1); // meter range.  the others our shader ignores
...
void Player::multiply(Real multyplier) {
  ...
  bod->attachToNode(mCamNode);
  bod->setPositionOrientation( mCamNode->getPosition(),
  Ogre::Quaternion::IDENTITY ); inertia =
  OgreNewt::MomentOfInertia::CalcEllipsoidSolid(40,size); bod->setMassMatrix(
  40, inertia ); bod->setCustomForceAndTorqueCallback<Player>(
  &Player::camera_force_callback ,this); // add the previous defined callback
  function as the body custom force and torque callback
  //bod->setStandardForceCallback();
  bod->setAutoFreeze(0);
  OgreNewt::BasicJoints::UpVector* uv2 = new
  OgreNewt::BasicJoints::UpVector(mWorld,bod,Vector3::UNIT_Y);
  bod->setOmega(Vector3(0,0,0));   //no rotation,for now  */
...
//////////////////////////////////////////
Ogre::Camera *Player::get_camera() { return mCamera; }
...
OgreNewt::Body *Player::get_ray_shoot() {
  Ogre::Vector3 myPos =
      get_location() + Vector3(0, cur_size.y * mViewNode->getPosition().y, 0);
...
void Player::set_additional_force(Vector3 force) { mForce = force; }

void Player::remove_additional_force() { mForce = Vector3::ZERO; }
...
void Player::camera_force_callback(OgreNewt::Body *me) {
  ...
  if (parentRelation) {
    if (mFullP) {
      me->setPositionOrientation(
          mPar->getPosition() +
              Quaternion(Radian(Degree(180)), Vector3::UNIT_Y) *
                  mPar->getOrientation() * pRelPosition,
          Quaternion(Radian(Degree(x_rotation_cont)), Vector3::UNIT_Y));
    } else {
      me->setPositionOrientation(
          mPar->getPosition() + pRelPosition,
          Quaternion(Radian(Degree(x_rotation_cont)), Vector3::UNIT_Y));
    }
  } else {
    ...
    Vector3 stdAddForce =
        me->getStdAddForce() * 60000 * (y_rotation_cont + 200) / 290;
    ...
    if (jumping) // in the jump
    {
      ...
      if (jumpTime <= MAX_JUMPTIME) {
        ...
        // now time is up, and if we are not on earth anymore, then the start
        // sequence is finished
        if (!isOnEarth())
          jumpStartFinished = true;
      }

      if (isOnEarth() && jumpStartFinished &&
          jumping) // means we landed, but time is not up yet
      {
        if (!jumpLanded) {
          Run3SoundRuntime::getSingleton().emitSound("run3/sounds/land.wav",
                                                     0.7f, false);
          jumpLanded = true;
        }
        ...
        if (jumpLandedTime >=
            MIN_JUMPPAUSE) // you need a little time to rest after the jump.
                           // otherwise you can use jumping to accelerate, which
    ...
    if (mProps.kinematic_movement) {
      if (alive && isOnEarth()) {
        ...
        me->addForce( force2 );*/
        if (!jumping)
          me->setVelocity(Vector3((force + force2).x, 0, (force + force2).z) /
                          100);
      } else {
      ...
      if (alive) {
        ...
        if (mProps.splitvectors) {
          if ((!(m_FrontVelocity == 0) && (m_StrafeVelocity == 0)) ||
              ((m_FrontVelocity == 0) && !(m_StrafeVelocity == 0))) {
            ...
            // LogManager::getSingleton().logMessage("BOTH WORKING!!!
            // SPLITTED..");
    ...
    /*if
    ((m_FrontVelocity==Ogre::Vector3::ZERO)&&(m_StrafeVelocity==Ogre::Vector3::ZERO))
    {
            me->setVelocity(Vector3::ZERO);
    }*/
    // me->setVelocity(force+force2);
    // me->addForce(Vector3(0,-gravity,0)*mass);
    // me->setPositionOrientation(get_body_position(bod),
    // Quaternion(get_body_orientation(bod).w,mViewNode->getOrientation().x,get_body_orientation(bod).y,mViewNode->getOrientation().z));
    // me->setOmega(Vector3(0,camera_rotation_x*2,0));   //to rotate camera in x
    // LogManager::getSingleton().logMessage(StringConverter::toString(me->getOmega())+"
    // "+StringConverter::toString(Vector3(0,camera_rotation_x*2,0)));*/
    ...
    if (b2)
      bod->addForce(Vector3(b2->getVelocity().x * mProps.trainForce, 0,
                            b2->getVelocity().z * mProps.trainForce));
    // Real y =
    // get_orientation().getYaw().valueAngleUnits()+camera_rotation_x*2;*/
...
bool Player::isTop() {
  ...
  OgreNewt::BasicRaycast floorRay(mWorld, myPos,
                                  myPos + Ogre::Vector3::UNIT_Y * jump_power);
  int hits;
  ...
  if (hits > 0) {
    ...
    for (int i = 0; i < floorRay.getHitCount(); i++) {
      ...
      if (found.mBody != bod && found.mDistance < hit.mDistance) {
        // if the body I found is not my own, and is not water and is closer
        // than last result you will probably have a different water system than
        // me...
        hit = found;
        // break;
      }
    ...
    if (distToFloor >
        0.05) // this much over the floor is considered on the floor
    {
      return false;
    } else {
      return true;
...
bool Player::isOnEarth() {
  ...
  OgreNewt::BasicRaycast floorRay(
      mWorld, myPos, myPos + Ogre::Vector3::NEGATIVE_UNIT_Y * jump_power);
  int hits;
  hits = floorRay.getHitCount();
  if (hits > 0) {
    OgreNewt::BasicRaycast::BasicRaycastInfo hit;
...
Vector3 Player::checkNotAvailable(void) {
  ...
  OgreNewt::BasicRaycast floorRay(
      mWorld, myPos, myPos + Ogre::Vector3::NEGATIVE_UNIT_X * jump_power);
  int hits;
  hits = floorRay.getHitCount();
  if (hits > 0) {
    OgreNewt::BasicRaycast::BasicRaycastInfo hit;
  ...
  floorRay = OgreNewt::BasicRaycast(mWorld, myPos,
                                    myPos + Ogre::Vector3::UNIT_X * jump_power);
  hits = floorRay.getHitCount();
  if (hits > 0) {
    OgreNewt::BasicRaycast::BasicRaycastInfo hit;
    hit.mBody = NULL;
  ...
  floorRay = OgreNewt::BasicRaycast(
      mWorld, myPos, myPos + Ogre::Vector3::NEGATIVE_UNIT_Z * jump_power);
  ...
  floorRay = OgreNewt::BasicRaycast(mWorld, myPos,
                                    myPos + Ogre::Vector3::UNIT_Z * jump_power);
  hits = floorRay.getHitCount();
  if (hits > 0) {
    OgreNewt::BasicRaycast::BasicRaycastInfo hit;
    hit.mBody = NULL;
...
//////////////////////////////////////////
Ogre::Vector3 Player::get_location() { return get_body_position(bod); }
//////////////////////////////////////////
Ogre::Quaternion Player::get_orientation() { return get_body_orientation(bod); }
//////////////////////////////////////////
...
void Player::MousePress(const OIS::MouseEvent &arg, OIS::MouseButtonID id) {
  ...
  if (!alive) {
    ...
    LoadMap::getSingleton().LoadPrev("Ogre Testmap", "/scripts", "run3/maps",
                                     "General", mSceneMgr);
    this->noclip = false;
  }
  if (Inventory::getSingleton().isVisible())
...
void Player::Duck() {
  ...
  bod->setPositionOrientation(
      Vector3(get_body_position(bod).x,
              get_body_position(bod).y - (size - size_dc).y,
              get_body_position(bod).z),
      get_body_orientation(bod));
}
//////////////////////////////////////////
void Player::UnDuck() {
  cur_size = size;
...
//////////////////////////////////////////
Ogre::Vector3 Player::get_cur_size() { return cur_size; }
//////////////////////////////////////////
void Player::playerDie() {
  HUD::getSingleton().SetHealth(0);
  HUD::getSingleton().SSBlood();
  alive = false;
  Run3SoundRuntime::getSingleton().emitSound("run3/sounds/bodysplat.wav", 2.0f,
                                             false);
  bod->setVelocity(Vector3(0, 0, 0));
  m_FrontVelocity = 0;
  m_StrafeVelocity = 0;
  ...
  bod->setPositionOrientation(get_location(),
                              Quaternion(Degree(90), Vector3::UNIT_Z));
  Duck(); //<-start here
  CWeapon::getSingleton().stripall();
  LogManager::getSingleton().logMessage("PlayerController: Player died!");
  /*if (uv2)
  {
          LogManager::getSingleton().logMessage("PlayerController: Player
  died!"); delete uv2; uv=0;
  }*/
  /*if (uv1&&uv3)
  {
          LogManager::getSingleton().logMessage("PlayerController: Player
  died!"); delete uv1; LogManager::getSingleton().logMessage("PlayerController:
  Player died!"); delete uv3;
          LogManager::getSingleton().logMessage("PlayerController: Player
  died!"); uv1=0;uv2=0;uv3=0;
  }*/
  LogManager::getSingleton().logMessage(
      "PlayerController: all player resources freed!");
}
//////////////////////////////////////////
...
void Player::FCUpdate(const FrameEvent &evt) {
  sound->setListenerPosition(get_body_position(bod), Vector3::ZERO,
                             mViewNode->_getDerivedOrientation());
  if (st != -1.0f) {
  ...
  if (!global::getSingleton().computer_mode) {
    if (razbros != 0.0f) {
      ...
      if (y_rotation_cont > y_limit_a ||
          y_rotation_cont < y_limit_b && !noclip) {
  ...
  if (!god) {
    ...
    if (health2 != health) {
      ...
      BloodEmitter::getSingleton().emitBlood(get_location(),
                                             Vector3(0.3, 0.3, 0.3));
      HUD::getSingleton().SSBlood();
      Run3SoundRuntime::getSingleton().emitSound("run3/sounds/pl_fallpain1.wav",
                                                 1.0f, false);
    }
  }
  if (health < 0 && alive) {
  ...
  if ((m_FrontVelocity != 0) || (m_StrafeVelocity != 0)) {
    if (isOnEarth() || mLadder) {
      ...
      if (fstTimer > ((1 / fps_speed) * 0.5f)) {
        fstindex++;
        String fname = "";
        fname = fname + "run3/sounds/footsteps/" + mFootstep +
                StringConverter::toString(fstindex) + ".wav";
  ...
  if (debug == 2) {
    ...
    String debugstr6 = "";
    debugstr3 = debugstr3 +
                "Body Position(xyz)=" + _tostr(get_body_position(bod).x) +
                " \n" + _tostr(get_body_position(bod).y) + " \n" +
                _tostr(get_body_position(bod).z);
    debugstr4 = debugstr4 + "Body Orientation(wxyz)=" +
                _tostr(get_body_orientation(bod).w) + " \n" +
    ...
    debugstr5 = debugstr5 + "Front Velocity(xyz)=" + _tostr(m_FrontVelocity) +
                ...
                _tostr(m_FrontVelocity);
    debugstr6 =
        debugstr6 + "Strafe Velocity(xyz)=" + _tostr(m_StrafeVelocity) + " \n" +
  ...
  if (debug == 1) {
    const RenderTarget::FrameStats &stats =
        global::getSingleton().getWindow()->getStatistics();
    textbox->setCaption(
  ...
  if (debug == 3) {
    ...
    for (unsigned int i = 0;
         i != Run3SoundRuntime::getSingleton().fileNames.size(); i++) {
