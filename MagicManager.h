...
enum WRAP_TYPE { CENTER, TOP_LEFT, TOP_LEFT_COMP };
...
public:
  ...
  static int buttonGUI_getX(lua_State *pL) {
    ...
    MagicManager::getSingleton().getBMgr()->getButton(name)->getPosition(
        (short &)x, (short &)y);
  ...
  static int buttonGUI_setPos(lua_State *pL) {
    ...
    MagicManager::getSingleton().getBMgr()->getButton(name)->setPosition(
        buttonGUI::buttonPosition(x1, y1));
    return 1;
  }
  ...
  static int buttonGUI_createButton(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
      ...
      Ogre::Vector2 position =
          StringConverter::parseVector2(lua_tostring(pL, 3));
      Ogre::Vector2 size = StringConverter::parseVector2(lua_tostring(pL, 4));
      if (MagicManager::getSingleton().center640) {
        Real x =
            MagicManager::getSingleton().mCam->getViewport()->getActualWidth();
        Real y =
            MagicManager::getSingleton().mCam->getViewport()->getActualHeight();
        ...
        case TOP_LEFT_COMP:
          LogManager::getSingleton().logMessage(
              "Transform: " + StringConverter::toString(position));
          LogManager::getSingleton().logMessage(
              "Transform: " + StringConverter::toString(size));
      ...
      if (MagicManager::getSingleton().getBMgr()->getButton(name) == 0) {
        MagicManager::getSingleton().getBMgr()->createButton(
            name, matname, buttonGUI::buttonPosition(position.x, position.y),
            size.x, size.y, 0, true, true, lue);
      } else {
  ...
  static int buttonGUI_create3DButton(lua_State *pL) {
    ...
    if (n != 6) {
      ...
      buttonGUI::button *btn =
          MagicManager::getSingleton().getBMgr()->createButton(
              "star1", "BLANK",
              buttonGUI::buttonPosition(buttonGUI::CENTER, -128, -128), 150,
              150, 2, true, true, "");
      btn->setMovable(true);
      btn->addButtonMesh("star1mesh", "ninjaStar.mesh", 0, 0, 150, 150)
          ->setZoom(50);
      return 1;
    }
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
        lua_isstring(pL, 4) && lua_isstring(pL, 5) && lua_isstring(pL, 6)) {
      LogManager::getSingleton().logMessage("Creating 3d button!");
      ...
      if (MagicManager::getSingleton().getBMgr()->getButton(name) == 0) {
        buttonGUI::button *btn =
            MagicManager::getSingleton().getBMgr()->createButton(
                name, "BLANK",
                buttonGUI::buttonPosition(position.x, position.y), size.x,
                size.y, 0, true, true, lue);
        btn->setMovable(true);
        btn->addButtonMesh(name + "_mesh", meshname, 0, 0, size.x, size.y)
            ->setZoom(StringConverter::parseReal(lua_tostring(pL, 5)));
      } else {
        MagicManager::getSingleton().getBMgr()->getButton(name)->show(true);
      }
    }
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
      ...
      if (MagicManager::getSingleton().getBMgr()->getButton(name) == 0) {
        ...
        btn->addButtonMesh(name + "_mesh", meshname, 0, 0, size.x, size.y)
            ->setZoom(StringConverter::parseReal(lua_tostring(pL, 5)))
            ->setRotation(
                StringConverter::parseQuaternion(lua_tostring(pL, 6)));
      } else {
        MagicManager::getSingleton().getBMgr()->getButton(name)->show(true);
      }
    }
  ...
  static int buttonGUI_deleteAllButtons(lua_State *pL) {
    ...
    std::vector<buttonGUI::button *> b =
        MagicManager::getSingleton().getBMgr()->getAllButtons();
