  ...
  try {
    ...
    scene->SetAttribute(
        ...
        StringConverter::toString(
            global::getSingleton().getPlayer()->getRegenerationMultiplier())
            .c_str());
    scene->SetAttribute(
        "maxh", StringConverter::toString(
                    global::getSingleton().getPlayer()->getRegenerationHealth())
                    .c_str());
    ...
    LogManager::getSingleton().logMessage(
        "[SaveFile] Save Error. TiXml reporated an error.");
  }
  //	delete doc;
}
...
String SaveGame::openLast() {
  try {
    ...
    if (SequenceDoc->Error()) {
      ...
      LogManager::getSingleton().logMessage(
          "[SaveFile] TiXml reporated an error.");
      return "";
    }
  } catch (...) {
    // rest=true;
    // We'll just log, and continue on gracefully
    LogManager::getSingleton().logMessage(
        "[SaveFile] SaveFile does not present,or TiXml reporated an error.");
    return "";
  }
  ...
  if (String(SequenceRoot->Value()) != "savefile") {
    LogManager::getSingleton().logMessage(
        "[SaveFile] Error: Invalid .xml File. Missing <savefile>");
    return "";
  }
  ...
  global::getSingleton().getPlayer()->setHealthRegeneration(
      StringConverter::parseReal(SequenceRoot->Attribute("mulh")),
