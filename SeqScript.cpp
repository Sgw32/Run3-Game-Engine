...
void SeqScript::assign(Vector3 pos, Real length, String name, bool freezebefore,
...
String SeqScript::getname() { return mName; }

void SeqScript::setWait(Real wait) { wait_before_start = wait; }
...
void SeqScript::dispose() {
  if (!disposed) {
    if (first) {
      LogManager::getSingleton().logMessage(
          "Disposing SeqScript. It has been used ingame");
      started = false;
      disposed = true;
    } else {
      LogManager::getSingleton().logMessage(
          "Disposing SeqScript. It wasn't used ingame");
      disposed = true;
    }
  }
}
