...
///////////////Copyright(c) 2026 PHOL-LABS Kft		   ////////
///////////////////////////////////////////////////////////////////
/// started:22.03.2010
/// /*/ finished:?
/// /*/
...
void LaserMinigun::buyWeapon() {
  LogManager::getSingleton().logMessage("Weapon " + this->get_name() +
                                        " is now buying");
  // mWeaponNode->setVisible(true);
  mLightenNode->setVisible(false);
  // LogManager::getSingleton().logMessage("Weapon "+this->get_name()+" is now
  // buying");
  In();
  // LogManager::getSingleton().logMessage("Weapon "+this->get_name()+" is now
  // buying");
...
void LaserMinigun::init(Ogre::Root *_root, SceneManager *sceneMgr,
                        SceneNode *weaponNode, Entity *_ent,
                        SoundManager *sound, OgreNewt::World *world) {
  root = _root;
  mWorld = world;
  mSceneMgr = sceneMgr;
  ...
  hasMuzzleFlash = StringConverter::parseBool(
      cf.getSetting("hasMuzzleFlash", StringUtil::BLANK, "true"));
  ...
  singleShot = StringConverter::parseBool(
      cf.getSetting("singleShot", StringUtil::BLANK, "false"));
  haveanim = StringConverter::parseBool(
      cf.getSetting("haveAnims", StringUtil::BLANK, "false"));
  ringMat = cf.getSetting("ringMat", StringUtil::BLANK, "Run3/CrosshairRing");
  crosshairMat =
      cf.getSetting("crosshairMat", StringUtil::BLANK, "Run3/Crosshair");
  stammo = StringConverter::parseUnsignedInt(
      cf.getSetting("Ammo", StringUtil::BLANK, "360"));
  Vector3 tr = StringConverter::parseVector3(
      cf.getSetting("Translate", StringUtil::BLANK, "0 0 0"));
  Vector3 mtr = StringConverter::parseVector3(
      cf.getSetting("muzzleTran", StringUtil::BLANK, "0 0 0"));
  Vector3 msc = StringConverter::parseVector3(
      cf.getSetting("muzzleScale", StringUtil::BLANK, "1 1 1"));
  dmgPSHT = StringConverter::parseReal(
      cf.getSetting("damagePerShot", StringUtil::BLANK, "10"));
  defOtt = StringConverter::parseReal(
      cf.getSetting("ottdacha", StringUtil::BLANK, "1"));
  ottSpd = 1.0f + StringConverter::parseReal(
                      cf.getSetting("ottSpd", StringUtil::BLANK, "0"));
  if (tr != Vector3::ZERO)
    mWeaponNode->translate(tr);
  ammo = stammo;
  hasRing = StringConverter::parseBool(
      cf.getSetting("hasRing", StringUtil::BLANK, "false"));
  secondSlot_allow = StringConverter::parseBool(
      cf.getSetting("AllowSecondSlot", StringUtil::BLANK, "false"));
  hasRingShift = StringConverter::parseBool(
      cf.getSetting("hasRingShift", StringUtil::BLANK, "false"));
  second_Slot = StringConverter::parseUnsignedInt(
      cf.getSetting("SecondSlot", StringUtil::BLANK, "0"));
  m_pause = StringConverter::parseReal(
      cf.getSetting("master_Pause", StringUtil::BLANK, "10"));
  fuzzy = StringConverter::parseReal(
      cf.getSetting("FuzzyShot", StringUtil::BLANK, "0"));
  col = StringConverter::parseColourValue(
      cf.getSetting("lightColor", StringUtil::BLANK, "1 0.7 0.1"));
  resetTimePosition = StringConverter::parseBool(
      cf.getSetting("resetTimePosition", StringUtil::BLANK, "false"));
  ...
  lShock_t = mSceneMgr->createParticleSystem("muzzleFlashS" + _ent->getName(),
                                             "muzzleFlashAK");
  mLightenNode = weaponNode->createChildSceneNode(mtr);
  String ha = StringConverter::toString(haveanim);
  pauseBetweenShots = StringConverter::parseReal(
      cf.getSetting("pauseBetweenShots", StringUtil::BLANK, "0"));
  ...
  MeshManager::getSingleton().createPlane(
      "Shockrifle_Light_n" + _ent->getName(),
      ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, plane, 27, 60, 1, 1,
      true, 1, 1, 1, Vector3::UNIT_Z);
  MeshManager::getSingleton().createPlane(
      "Shockrifle_Light2_n" + _ent->getName(),
      ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, plane2, 27, 60, 1, 1,
      true, 1, 1, 1, Vector3::UNIT_Z);

  /*ent = mSceneMgr->createEntity( "Lightning"+_ent->getName(),
  "Shockrifle_Light_n"+_ent->getName() );
  //mSceneMgr->getRootSceneNode()->createChildSceneNode()->attachObject(ent);
  ent->setMaterialName("Run3/MuzzleFlash");
  ent2 = mSceneMgr->createEntity( "Lightning2"+_ent->getName(),
  "Shockrifle_Light2_n"+_ent->getName() );
  //mSceneMgr->getRootSceneNode()->createChildSceneNode()->attachObject(ent);
...
void LaserMinigun::Press(const OIS::KeyEvent &arg) {
  if (arg.key == OIS::KC_R && secondSlot_allow) {
    if ((ammo + primaryAmmo) >= second_Slot) {
      ...
      primaryAmmo = second_Slot;
      // HUD::getSingleton().setAmmo(StringConverter::toString(primaryAmmo)+"
...
void LaserMinigun::MousePress(const OIS::MouseEvent &arg,
  ...
  if ((ammo > 0 && !secondSlot_allow) ||
    ...
    if (id == OIS::MB_Left) {
      ...
      soundmgr->setSound(shoots, mPlayerNode->getPosition(), Vector3(0, 0, 0),
                         Vector3(0, 0, 0), 10000, false, false, 1000, 1000,
                         1000);
      //	curOtt=defOtt;
      if (singleShot) {
        curOtt = defOtt;
      ...
      flashLight->setAttenuation(
          2000, 1, 1, 1); // meter range.  the others our shader ignores
  ...
  if (id == OIS::MB_Right) {
    ...
    LogManager::getSingleton().logMessage(StringConverter::toString(modPos) +
                                          "AA2!");
    Modulator::getSingleton().modulate_scenenode_pos(mWeaponNode,
                                                     modPos + ironSight, 0.5f);
    Modulator::getSingleton().modulator_complete_fov();
    Modulator::getSingleton().modulate_FOV(
        global::getSingleton().getPlayer()->getFOV(),
        global::getSingleton().getPlayer()->getFOV() - 20, 0.7);
...
OgreNewt::Body *LaserMinigun::get_ray_shoot() {
  ...
  // Ogre::Vector3 myPos =
  // global::getSingleton().getPlayer()->get_location()+Vector3(0,50,0);
  // Ogre::Quaternion myOrient =
  // mWeaponNode->getParentSceneNode()->getOrientation();
  Ogre::Vector3 myPos =
      mWeaponNode->getParentSceneNode()->_getDerivedPosition();
  Ogre::Vector3 direction, end;
  // direction = myOrient*Vector3::NEGATIVE_UNIT_Z;
  direction = get_direction();
  Ogre::Vector3 rnd(Ogre::Math::RangeRandom(-fuzzy, fuzzy),
                    Ogre::Math::RangeRandom(-fuzzy, fuzzy),
                    Ogre::Math::RangeRandom(-fuzzy, fuzzy));
  // Ogre::Vector3 myPos =
  // mPlayerNode->_getDerivedPosition()+Vector3(0,50,0)+direction*20.5;
  end = myPos + direction * 100000.0f + rnd;
  // OgreNewt::BasicRaycast
  // shootRay(mWorld,myPos,myPos+Ogre::Vector3::NEGATIVE_UNIT_Z * 100);
  ...
  // LogManager::getSingleton().logMessage("LaserMinigun template: first collide
  // @"+StringConverter::toString(shootRay.getFirstHit().mDistance*(end-myPos).length()));
  /*Real hitLen = (end-myPos).length();
  LogManager::getSingleton().logMessage("Laser minigun hit
  length="+StringConverter::toString(hitLen));*/
  Vector3 hitpos =
      myPos + shootRay.getFirstHit().mDistance * (direction * 100000 + rnd);
  ...
  if (shootRay2.getFirstHit().mBody &&
      (shootRay2.getFirstHit().mBody == shootRay3.getFirstHit().mBody) &&
      (shootRay2.getFirstHit().mBody->getType() == 4)) {
    Vector3 hitpos2 =
        myPos + shootRay2.getFirstHit().mDistance * 0.99 *
                    (direction * 100000 + rnd - Vector3(500, 0, 500));
    Vector3 hitpos3 =
        myPos + shootRay3.getFirstHit().mDistance * 0.99 *
                    (direction * 100000 + rnd + Vector3(500, 0, 500));

    MeshDecalMgr::getSingleton().addHoleEffect(hitpos - direction * 0.01,
                                               shootRay2.getFirstHit().mNormal);
  }
...
Ogre::Vector3 LaserMinigun::get_direction() {
  /*Ogre::Quaternion myOrient = mPlayerNode->getOrientation(); //Where the body
  is Ogre::Quaternion myOrient2 =
  mWeaponNode->getParentSceneNode()->_getDerivedOrientation(); //An orientation
  of mViewNode Ogre::Vector3 direction2,down,down2,direction; direction2 =
  myOrient2*Vector3::NEGATIVE_UNIT_Z; down = Vector3(0,direction2.y,0);
  direction = myOrient*Vector3::NEGATIVE_UNIT_Z+down;
  LogManager::getSingleton().logMessage(StringConverter::toString(direction));
  return direction;*/

  Ogre::Quaternion myOrient2 =
      mWeaponNode->getParentSceneNode()->_getDerivedOrientation();
  // LogManager::getSingleton().logMessage(StringConverter::toString(
  // myOrient2*Vector3::NEGATIVE_UNIT_Z));
...
bool LaserMinigun::frameStarted(const Ogre::FrameEvent &evt) {
  ...
  // LogManager::getSingleton().logMessage(StringConverter::toString(fcnt)+"
  // "+StringConverter::toString(soundLoaded));
  if (startCount) {
    ...
    if (!(bod->getName() == "PLAYER1")) {
      for (i = 0; i != pobj.size(); i++) {
        if (pobj[i]->isThisPO(bod)) {
          ...
          /*bullet->init(mPlayerNode->getPosition()+mPlayerNode->getOrientation()*Vector3(0,0,-100),dir,100000,
          mWorld,mSceneMgr); bullet->start();*/
...
bool LaserMinigun::frameEnded(const Ogre::FrameEvent &evt) { return true; }