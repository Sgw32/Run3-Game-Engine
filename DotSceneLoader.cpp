...
void DotSceneLoader::parseDotScene(const String &SceneName,
                                   ...
                                   OgreNewt::World *World, Camera *camera,
  ...
  if (!mWorld) {
    LogManager::getSingleton().logMessage(
        "[DotSceneLoader] Physics world doesnt exists!");
  }
  ...
  try {
    ...
    if (ResourceGroupManager::getSingleton().resourceExists(groupName,
                                                            basename)) {
      pStream = ResourceGroupManager::getSingleton().openResource(basename,
                                                                  groupName);
    ...
    if (XMLDoc->Error()) {
      ...
      LogManager::getSingleton().logMessage(
          StringConverter::toString(XMLDoc->ErrorRow()));
      LogManager::getSingleton().logMessage(
          StringConverter::toString(XMLDoc->ErrorCol()));
      delete XMLDoc;
      return;
    }
  } catch (...) {
  ...
  if (XMLRoot->FirstChildElement("integratedSequence"))
    Sequence::getSingleton().SetSceneSeq(
        XMLRoot->FirstChildElement("integratedSequence"));
  if (StringConverter::parseInt(getAttrib(XMLRoot, "tLod", "-1")) != -1)
    levelofDetailT =
        StringConverter::parseInt(getAttrib(XMLRoot, "tLod", "-1"));
  if (StringConverter::parseInt(getAttrib(XMLRoot, "mLod", "-1")) != -1)
    levelofDetailM =
        StringConverter::parseInt(getAttrib(XMLRoot, "mLod", "-1"));
  if (StringConverter::parseBool(getAttrib(XMLRoot, "glass", "false")) ==
      false) {
    CompositorManager::getSingleton().setCompositorEnabled(
        mCamera->getViewport(), "Run3Glass", false);
  } else {
    CompositorManager::getSingleton().setCompositorEnabled(
        mCamera->getViewport(), "Run3Glass", true);
  }
  if (StringConverter::parseBool(getAttrib(XMLRoot, "glow", "false")) ==
      false) {
    CompositorManager::getSingleton().setCompositorEnabled(
        mCamera->getViewport(), "Glow", false);
    Inventory::getSingleton().setGlowWasActive(false);
  } else {
    CompositorManager::getSingleton().setCompositorEnabled(
        mCamera->getViewport(), "Glow", true);
...
void DotSceneLoader::mergeDotScene(const String &SceneName,
                                   const String &groupName,
                                   SceneManager *yourSceneMgr,
                                   SceneNode *pAttachNode,
                                   const String &sPrependNode,
...
void DotSceneLoader::processWater(TiXmlElement *XMLNode) {
  ...
  if ((sunPos == Vector3::ZERO) && (sunColor == Vector3::ZERO) &&
      fileName.empty()) {
...
void DotSceneLoader::processScene(TiXmlElement *XMLRoot) {
  ...
  if (XMLRoot->Attribute("minOgreVersion"))
    message +=
        ", min. Ogre version " + String(XMLRoot->Attribute("minOgreVersion"));
...
void DotSceneLoader::processNPCNode(TiXmlElement *XMLNode) {
  ...
  SceneNode *sn =
      mSceneMgr->getRootSceneNode()->createChildSceneNode(n->getPosition());
  Entity *e = mSceneMgr->createEntity(sn->getName(), "box.mesh");
  sn->setScale(50 * sceneMultiplier, 50 * sceneMultiplier,
               50 * sceneMultiplier);
  sn->attachObject(e);
  return;
#endif
...
void DotSceneLoader::processPortals(TiXmlElement *XMLNode) {
  ...
  if (farClip)
    ZonePortalManager::getSingleton().setZoneCameraParams(
        mCamera->getFarClipDistance(), farClip);
...
void DotSceneLoader::processNodes(TiXmlElement *XMLNode) {
  ...
  while (pElement) {
    ...
    if (Run3Benchmark::getSingleton().benchMarkEnabled()) {
      Run3Benchmark::getSingleton().setIterationCommentary(
          getAttrib(pElement, "name"));
    }
    processNode(pElement);
    pElement = pElement->NextSiblingElement("node");
  }
...
void DotSceneLoader::processEnvironment(TiXmlElement *XMLNode) {
  ...
  //! @todo Set the background colour of all viewports (RenderWindow has to be
  //! provided then)
...
void DotSceneLoader::processUserDataReference(TiXmlElement *XMLNode,
                                              SceneNode *pParent) {
  //! @todo Implement this
}
...
void DotSceneLoader::processZone(TiXmlElement *XMLNode) {
  ...
  Vector3 scale =
      Vector3(getAttribReal(XMLNode, "sX"), getAttribReal(XMLNode, "sY"),
              getAttribReal(XMLNode, "sZ"));
  ZonePortalManager::getSingleton().passZone(pos, scale);
...
void DotSceneLoader::processLight(TiXmlElement *XMLNode, SceneNode *pParent) {
  ...
  if (!Loader::getSingleton().deferred) {
    ...
    if (dist != -1.0f) {
      ...
      LightPerfomanceManager::getSingleton().addLight(
          AxisAlignedBox(pos - Vector3(dist, dist, dist),
                         pos + Vector3(dist, dist, dist)),
          name);
    }
    if (shdist != -1.0f) {
      // Vector3 pos = pParent->getPosition();
    ...
    LogManager::getSingleton().logMessage(
        "Derived direction of the light:" +
        StringConverter::toString(pLight->getDerivedDirection()));
    ...
    if (pElement) {
      ...
      m->setAttenuation(getAttribReal(pElement, "a"),
                        getAttribReal(pElement, "b"),
                        getAttribReal(pElement, "c"));
    }
  }
}

void DotSceneLoader::processLightDynamics(TiXmlElement *XMLNode,
                                          Light *pLight) {
  ColourValue c = parseColour(XMLNode);
  ControllerValueRealPtr mLightFlasher =
      ControllerValueRealPtr(new LightFlasher(pLight, c));
  ...
  Controller<Real> *mLightController = mControllerManager->createController(
      mControllerManager->getFrameTimeSource(), mLightFlasher,
      mLightControllerFunc);
...
void DotSceneLoader::processLookTarget(TiXmlElement *XMLNode,
                                       SceneNode *pParent) {
...
void DotSceneLoader::processTrackTarget(TiXmlElement *XMLNode,
                                        SceneNode *pParent) {
  // Process attributes
  String nodeName = getAttrib(XMLNode, "nodeName");
...
void DotSceneLoader::processNoCollide(TiXmlElement *XMLNode,
                                      SceneNode *pParent) {
  // Process attributes
  ...
  if (!mSceneMgr->hasEntity(name))
    ...
    // LogManager::getSingleton().logMessage("assigning a new name to
    // entity"+pParent->getName());
  ...
  try {
    ...
    if (!materialFile.empty() && !Loader::getSingleton().material_disable) {
      ...
      MaterialPtr mat =
          (MaterialPtr)MaterialManager::getSingleton().getByName(materialFile);
      if ((scalex == 1) && (scaley == 1) && (scrollx == 0) && (scrolly == 0)) {
        mat2 = mat;
      } else {
        mat->clone(_tostr((float)texture_copy_counter) + "_" + mat->getName());
        mat2 = (MaterialPtr)MaterialManager::getSingleton().getByName(
            _tostr((float)texture_copy_counter) + "_" + mat->getName());
        mat2->getTechnique(0)
            ->getPass(0)
        ...
        mat2->getTechnique(0)
            ->getPass(0)
            ->getTextureUnitState(0)
            ->setTextureScroll(scrollx, scrolly);
  ...
  if (isStatic) {
    Run3Batcher::getSingleton().addObject(ent, pParent->getPosition(),
                                          pParent->getScale(),
...
mat->setParameter("scale",str1+" "+str2);*/
      /*mat->clone(_tostr((float)texture_copy_counter)+"_"+mat->getName());
      MaterialPtr mat2 =
      (MaterialPtr)MaterialManager::getSingleton().getByName(_tostr((float)texture_copy_counter)+"_"+mat->getName());
      mat2->getTechnique(0)->
      getPass(0)->getTextureUnitState(0)->setTextureScale(scalex,scaley);
      mat2->getTechnique(0)->
      getPass(0)->getTextureUnitState(0)->setTextureScroll(scrollx,scrolly);
      mat2->getTechnique(0)->
      getPass(0)->getTextureUnitState(0)->setTextureMipmapBias(levelofDetailT);*/
      // mat2->getTechnique(0)->setLodIndex(100);
...
void DotSceneLoader::processMirror(TiXmlElement *XMLNode, SceneNode *pParent) {
  ...
  MirrorManager::getSingleton().createNewMirror(mat, pParent,
                                                pParent->getScale());
}
...
void DotSceneLoader::util_processRagDoll(String name, String scriptFile,
                                         String meshFile, SceneNode *pParent) {
  dynamicObjects.push_back(name);
  try {
    ...
    LogManager::getSingleton().logMessage(
        "[DotSceneLoader] Error loading ragdoll");
  }
}
...
void DotSceneLoader::processPhysObj(TiXmlElement *XMLNode, SceneNode *pParent) {
  ...
  try {
    ...
    LogManager::getSingleton().logMessage(
        "[DotSceneLoader] Error loading a physics object!");
  }
...
void DotSceneLoader::processPhysBlock(TiXmlElement *XMLNode,
                                      SceneNode *pParent) {
  // Process attributes
  String name = getAttrib(XMLNode, "name");
  String id = getAttrib(XMLNode, "id");
  ...
  try {
    ...
    LogManager::getSingleton().logMessage(
        "[DotSceneLoader] Error building a physbox!");
  }
...
void DotSceneLoader::processPhysCyl(TiXmlElement *XMLNode, SceneNode *pParent) {
  ...
  try {
    ...
    LogManager::getSingleton().logMessage(
        "[DotSceneLoader] Error building a physcyl!");
  }
...
void DotSceneLoader::processParticleSystem(TiXmlElement *XMLNode,
                                           SceneNode *pParent) {
  // Process attributes
  String name = getAttrib(XMLNode, "name");
  String id = getAttrib(XMLNode, "id");
...
void DotSceneLoader::processBillboardSet(TiXmlElement *XMLNode,
                                         SceneNode *pParent) {
  //! @todo Implement this
}
...
void DotSceneLoader::processSound(TiXmlElement *XMLNode) {
  ...
  sound->setSound(audioId, pos, Vector3::ZERO, Vector3::ZERO, maxDistance,
                  false, loop, minGain, rollOff, distance);
...
void DotSceneLoader::processAmbient(TiXmlElement *XMLNode) {
  ...
  Real maxDistance =
      getAttribReal(XMLNode, "maxDistance", 500) * sceneMultiplier;
  ...
  // set sound
  if (minGain > 0) {
    Run3SoundRuntime::getSingleton().addAmbientSound(objName, name, pos,
                                                     maxDistance, distance);
  } else {
    Run3SoundRuntime::getSingleton().addAmbientSound(
        objName, name, pos, maxDistance, distance, minGain);
  }
}
...
void DotSceneLoader::processTree(TiXmlElement *XMLNode, SceneNode *pParent) {
  ...
  // Entity* ent =
  // mSceneMgr->createEntity(pParent->getName(),pParent->getName()+"TREEM");
  // pParent->attachObject(ent);
  ...
  try {
    phys->init(mSceneMgr, mWorld);
    phys->CreateStaticObject(name + "TREEME", pParent->getName() + "TREEM",
                             pParent);
    // phys->ent->setCastShadows(castShadows);
    global::getSingleton().addPhysObject(phys);
  } catch (Ogre::Exception & /*e*/) {
    LogManager::getSingleton().logMessage(
        "[DotSceneLoader] Error loading an tree!");
  }
  /*OgreNewt::Collision* col = new
  OgreNewt::CollisionPrimitives::TreeCollision(mWorld, pParent,true);
  ...
  ); trunks4.push_back(bod); delete col;*/
}
...
void DotSceneLoader::processBreakable(TiXmlElement *XMLNode,
                                      SceneNode *pParent) {
  String name = getAttrib(XMLNode, "name");
  String id = getAttrib(XMLNode, "id");
  String meshFile = getAttrib(XMLNode, "meshFile");
  ...
  Vector3 gibScale =
      StringConverter::parseVector3(getAttrib(XMLNode, "gibScale", "1 1 1"));
  String materialFile = getAttrib(XMLNode, "materialFile");
  ...
  unsigned int c =
      StringConverter::parseUnsignedInt(getAttrib(XMLNode, "count", "10"));
  // TEMP: Maintain a list of static and dynamic objects
  dynamicObjects.push_back(name);
  ...
  try {
    ...
    phys->set_breakable(StringConverter::parseInt(health),
                        StringConverter::parseInt(strength));
...
void DotSceneLoader::processNewton(TiXmlElement *XMLNode) {
  // Process attributes
  Vector3 min, max;
  min = Vector3(getAttribReal(XMLNode, "x1", -100000),
                getAttribReal(XMLNode, "y1", -100000),
                getAttribReal(XMLNode, "z1", -100000));
  max = Vector3(getAttribReal(XMLNode, "x2", 100000),
                getAttribReal(XMLNode, "y2", 100000),
                getAttribReal(XMLNode, "y2", 100000));
...
void DotSceneLoader::processPlayer(TiXmlElement *XMLNode) {
  ...
  Vector3 look_at =
      Vector3(getAttribReal(XMLNode, "lx", 0), getAttribReal(XMLNode, "ly", 0),
              getAttribReal(XMLNode, "lz", 0));
...
void DotSceneLoader::processLightAttenuation(TiXmlElement *XMLNode,
                                             Light *pLight) {
  // Process attributes
  Real range = getAttribReal(XMLNode, "range");
  Real constant = getAttribReal(XMLNode, "constant");
...
String DotSceneLoader::getAttrib(TiXmlElement *XMLNode, const String &attrib,
...
Real DotSceneLoader::getAttribReal(TiXmlElement *XMLNode, const String &attrib,
                                   Real defaultValue) {
  if (XMLNode->Attribute(attrib.c_str()))
...
bool DotSceneLoader::getAttribBool(TiXmlElement *XMLNode, const String &attrib,
                                   bool defaultValue) {
  if (!XMLNode->Attribute(attrib.c_str()))
    return defaultValue;
...
Vector3 DotSceneLoader::parseVector3(TiXmlElement *XMLNode) {
  ...
  return orientation *
         Vector3(StringConverter::parseReal(XMLNode->Attribute("x")),
...
ColourValue DotSceneLoader::parseColour(TiXmlElement *XMLNode) {
  return ColourValue(StringConverter::parseReal(XMLNode->Attribute("r")),
                     ...
                     XMLNode->Attribute("a") != NULL
                         ? StringConverter::parseReal(XMLNode->Attribute("a"))
                         : 1);
}
