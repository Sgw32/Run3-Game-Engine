...
#ifndef __Run3FrameListener_H__
#define __Run3FrameListener_H__
...
class Run3FrameListener : public FrameListener, public WindowEventListener {
protected:
  virtual void updateStats(void) {
    ...
    try {
      ...
      OverlayElement *guiCurr =
          OverlayManager::getSingleton().getOverlayElement("Core/CurrFps");
      OverlayElement *guiBest =
          OverlayManager::getSingleton().getOverlayElement("Core/BestFps");
      OverlayElement *guiWorst =
          OverlayManager::getSingleton().getOverlayElement("Core/WorstFps");
      ...
      guiBest->setCaption(bestFps + StringConverter::toString(stats.bestFPS) +
                          ...
                          " ms");
      guiWorst->setCaption(
          worstFps + StringConverter::toString(stats.worstFPS) + " " +
...
public:
  ...
  // context
  Run3FrameListener(RenderWindow *win, Camera *cam, bool bufferedKeys = false,
                    bool bufferedMouse = false, bool bufferedJoy = false)
      : mCamera(cam), mTranslateVector(Vector3::ZERO), mCurrentSpeed(0),
        mWindow(win), mStatsOn(true), mNumScreenShots(0), mMoveScale(0.0f),
        mRotScale(0.0f), mTimeUntilNextToggle(0), mFiltering(TFO_BILINEAR),
    ...
    // Create all devices (We only catch joystick exceptions here, as, most
    // people have Key/Mouse)
  ...
  virtual bool processUnbufferedKeyInput(const FrameEvent &evt) {
    ...
    if (displayCameraDetails)
      mDebugText =
          "P: " + StringConverter::toString(mCamera->getDerivedPosition()) +
          " " +
  ...
  virtual void moveCamera() {
    ...
    // Note that YAW direction is around a fixed axis (freelook style) rather
    // than a natural YAW
    //(e.g. airplane)
    mCamera->yaw(mRotX);
    mCamera->pitch(mRotY);
  ...
  // Override frameRenderingQueued event to process that (don't care about
  // frameEnded)
