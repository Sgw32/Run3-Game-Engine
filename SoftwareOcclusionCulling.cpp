...
SoftwareOcclusionCulling
    *Ogre::Singleton<SoftwareOcclusionCulling>::ms_Singleton = 0;

SoftwareOcclusionCulling::SoftwareOcclusionCulling() { cnt = 1.0f; }

SoftwareOcclusionCulling::~SoftwareOcclusionCulling() {}

void SoftwareOcclusionCulling::init() {
  mgr = (CustomSceneManager *)global::getSingleton()
            .getSceneManager(); // TODO Replace with your mSceneMgr SceneManager
}

void SoftwareOcclusionCulling::upd(
    const FrameEvent &evt) // Feed it from some registered framelistener
{
  ...
  if (cnt < 0) {
    ...
    for (i = zps.begin(); i != zps.end(); i++) {
      Entity *cur = (*i);
#ifdef DEBUG_CULLING
      LogManager::getSingleton().logMessage("First entity name:" +
                                            cur->getName());
#endif
      ...
      for (unsigned int j = 0; j != cnt; j++) {
        ...
        if (ent2) {
          ...
          LogManager::getSingleton().logMessage(
              "Pixel count is:" + StringConverter::toString(pixelc));
#endif
          if ((pixelc < 100) && (pixelc != -1)) {
#ifdef DEBUG_CULLING
            LogManager::getSingleton().logMessage(
                "Setting off " + cur->getName() +
                " num:" + StringConverter::toString(j));
#endif
