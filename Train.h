  ...
  int userProcess() {
    ...
    if ((m_body0->getType() == 6) && (m_body1->getType() == 6)) {
      ...
      Run3SoundRuntime::getSingleton().emitSound("run3/sounds/explode_3.wav", 4,
                                                 false, p, 200, 50, 800);
      GibManager::getSingleton().spawnGibs("metal_chunk01.mesh", p,
                                           Vector3(3, 3, 3), 10.0f, 30, 500);
      /*m_body0->setCustomForceAndTorqueCallback<TrainMat>(
      &TrainMat::train_derail ,this);
      m_body1->setCustomForceAndTorqueCallback<TrainMat>(
      &TrainMat::train_derail ,this);*/
      m_body0->setType(4);
      m_body1->setType(4);
    ...
    if (y > 100000 * TIME_SHIFT * TIME_SHIFT) {
      Run3SoundRuntime::getSingleton().emitSound("run3/sounds/trbr.wav", 5.0f,
                                                 false, p, 200, 50, 800);
    }
...
public:
  ...
  void addUpVectors() {
    ...
    OgreNewt::BasicJoints::UpVector *uv1 =
        new OgreNewt::BasicJoints::UpVector(w, bod, Vector3::UNIT_X);
    OgreNewt::BasicJoints::UpVector *uv3 =
        new OgreNewt::BasicJoints::UpVector(w, bod, Vector3::UNIT_Z);
  }
  void setYawAngle(Real angle) { mYawAngle = angle; }
  ...
  Quaternion getDirectionQuat(const Ogre::Vector3 &vec,
  ...
  void disablePositionSaveMode() { positionSaveMode = false; }
