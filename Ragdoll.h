...
public:
  /*
          Main bone class, represents a single bone (rigid body) in the ragdoll.
     is linked to a bone in the model too.
  */
  class RagBone {
  public:
    enum BoneShape {
    ...
    RagBone(RagDoll *creator, OgreNewt::World *world, RagBone *parent,
            Ogre::Bone *ogreBone, Ogre::MeshPtr mesh, Ogre::Vector3 dir,
            RagBone::BoneShape shape, Ogre::Vector3 size, Ogre::Real mass);
  ...
  private:
    OgreNewt::ConvexCollision *_makeConvexHull(OgreNewt::World *world,
                                               Ogre::MeshPtr mesh,
                                               Ogre::Real minWeight);
  ...
  RagDoll(Ogre::String filename, OgreNewt::World *world, Ogre::SceneNode *node,
          Real lifetime);
  ~RagDoll();
  void init(Ogre::String filename, OgreNewt::World *world,
            Ogre::SceneNode *node);
  ...
  static void _placementCallback(OgreNewt::Body *me,
...
private:
  ...
  enum JointType { JT_BALLSOCKET, JT_HINGE };

  // add a bone!
  RagBone *_addBone(OgreNewt::World *world, RagBone *parent, Ogre::Vector3 dir,
                    RagBone::BoneShape shape, Ogre::Vector3 size,
                    Ogre::Real mass, Ogre::Bone *ogrebone);
  ...
  void _joinBones(RagDoll::JointType type, RagBone *parent, RagBone *child,
                  Ogre::Vector3 pos, Ogre::Vector3 pin, Ogre::Real limit1,
                  Ogre::Real limit2);
