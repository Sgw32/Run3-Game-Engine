...
void GibManager::spawnGib(String meshFile, Vector3 pos, Vector3 scale,
                          Real lifetime) {
  if (lifetime > 0) {
...
void GibManager::spawnGibs(String meshFile, Vector3 pos, Vector3 scale,
                           Real lifetime, unsigned int num, Real maxdist) {
  LogManager::getSingleton().logMessage(
      "Gibs:" + StringConverter::toString(pos) + " " +
      StringConverter::toString(num) + " " +
      StringConverter::toString(maxdist));
  for (unsigned int i = 0; i != num; i++) {
    Ogre::Vector3 p(pos.x + Ogre::Math::RangeRandom(-maxdist, maxdist),
