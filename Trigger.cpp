...
void Trigger::addMultipleLightOn(Light *lt) { mLightsOn[lt] = true; }

void Trigger::addMultipleLightOff(Light *lt) { mLightsOff[lt] = true; }
...
void Trigger::init(AxisAlignedBox aabb, Player *ply) {
  ...
  LogManager::getSingleton().logMessage(
      StringConverter::toString(aabb.getSize()));
  type = 2;
  if (ply)
    tPlayer = ply;
  objaabb = aabb;
  trig =
      scene->createEntity(StringConverter::toString(objaabb.getMinimum()) +
                              StringConverter::toString(objaabb.getMaximum()),
                          "box.mesh");
  trigsnap =
      scene->getRootSceneNode()->createChildSceneNode(objaabb.getCenter());
...
void Trigger::init(Vector3 pos, Vector3 scale, Player *ply) {
  ...
  LogManager::getSingleton().logMessage(StringConverter::toString(scale) + " " +
...
void Trigger::setCallbackEnt(String name, String file, String event,
                             Vector3 pos) {
...
void Trigger::setname(String name) { mName = name; }

String Trigger::getname() { return mName; }
...
bool Trigger::frameStarted(const Ogre::FrameEvent &evt) {
  ...
  if (type == 4) // Trigger multiple
  {
    if (enabled) {
      ...
      AxisAlignedBox plyaab =
          AxisAlignedBox(pos - size * hyst, pos + size * hyst);
