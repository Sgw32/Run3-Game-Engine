...
void Pendulum::setname(String name) { mName = name; }

String Pendulum::getname() { return mName; }
...
bool Pendulum::frameStarted(const Ogre::FrameEvent &evt) {
  ...
  if (nowfire) {
    if (funcrot) {
      if (rotDoor) {
        /*Real mpt1=
        -rotSpeed*evt.timeSinceLastFrame*5*Ogre::Math::Sign(mp1)*TIME_SHIFT;
        Real myt1=
        -rotSpeed*evt.timeSinceLastFrame*5*TIME_SHIFT*Ogre::Math::Sign(my1);
        Real mrt1=
        -rotSpeed*evt.timeSinceLastFrame*5*TIME_SHIFT*Ogre::Math::Sign(mr1);
        ...
        // LogManager::getSingleton().logMessage("step");
        set_orientation(rot);
        set_position(basePos + Vector3(positionPend.x * Math::Sin(mp1),
                                       positionPend.y * Math::Sin(mp1),
                                       positionPend.z * Math::Sin(mp1)));
      } else {
