...
void FadeListener::rebuild() {
  if (inited && !fadeb) {
    ...
    OverlayManager::getSingleton()
        .getByName("Overlays/FadeInOut")
        ->getChild("FadeInOutPanel")
        ->setMaterialName("Materials/OverlayMaterial");
    fader = new Fader("Overlays/FadeInOut", "Materials/OverlayMaterial");
  }
}
...
void FadeListener::setDuration(Real dur) { duration = dur; }

void FadeListener::setDurationF(Real durf) { durationf = durf; }
