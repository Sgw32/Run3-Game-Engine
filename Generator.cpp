...
void Generator::fireGenerator() {
  ...
  Vector3 dcorner =
      pos - Vector3(nx * spacing / 2, ny * spacing / 2, nz * spacing / 2);
  ...
  for (unsigned int i = 0; i != nx; i++) {
    for (unsigned int j = 0; j != ny; j++) {
      for (unsigned int k = 0; k != nz; k++) {
        ...
        LogManager::getSingleton().logMessage("Generator: gen" +
                                              StringConverter::toString(pom));
        SceneNode *pParent =
            mSceneMgr->getRootSceneNode()->createChildSceneNode(pom);
        ...
        try {
          ...
          phys->CreateObject(pParent->getName(), meshFile, pParent, 100.0,
                             false, "");
          global::getSingleton().addPhysObject(phys);
        } catch (Ogre::Exception & /*e*/) {
          LogManager::getSingleton().logMessage(
              "[Generator] Error loading a physics object!");
        }
      }
    }
  }
