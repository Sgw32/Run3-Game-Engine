...
void Run3Batcher::prepareBatch() {
  LogManager::getSingleton().logMessage(
      "Run3Batcher: Preparing Static Batch for scene...");
  if (empty) {
    LogManager::getSingleton().logMessage(
        "Run3Batcher: Created a new StaticGeom instance");
...
void Run3Batcher::addObject(Entity *ent, Vector3 pos, Vector3 scale,
                            Quaternion quat) {
  LogManager::getSingleton().logMessage(
      "Run3Batcher: Adding object to batch...");
  // LogManager::getSingleton().logMessage(StringConverter::toString(pos)+"
  // "+StringConverter::toString(scale)+StringConverter::toString(quat));
  empty = false;
...
void Run3Batcher::buildBatch() {
  if (!empty) {
    LogManager::getSingleton().logMessage(
        "Run3Batcher: Building a static batch.");
