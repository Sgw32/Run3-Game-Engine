...
DLight::DLight(MaterialGenerator *sys, Ogre::Light *parentLight)
    : mParentLight(parentLight), bIgnoreWorld(false), mGenerator(sys),
      mPermutation(0) {
  // Set up geometry
...
void DLight::setAttenuation(float c, float b, float a) {
  ...
  if (c != 1.0f || b != 0.0f || a != 0.0f) {
    ...
    if (mParentLight->getType() == Light::LT_POINT) {
      ...
      int threshold_level =
          10; // difference of 10-15 levels deemed unnoticeable
...
void DLight::createSphere(float radius, int nRings, int nSegments) {
  ...
  GeomUtils::createSphere(mRenderOp.vertexData, mRenderOp.indexData, radius,
                          nRings, nSegments, false // no normals
                          ,
                          false // no texture coordinates
  );

  // Set bounding box and sphere
  setBoundingBox(AxisAlignedBox(Vector3(-radius, -radius, -radius),
...
void DLight::createCone(float radius, float height, int nVerticesInBase) {
  ...
  GeomUtils::createCone(mRenderOp.vertexData, mRenderOp.indexData, radius,
                        height, nVerticesInBase);

  // Set bounding box and sphere
  setBoundingBox(AxisAlignedBox(Vector3(-radius, 0, -radius),
...
//-----------------------------------------------------------------------
Real DLight::getBoundingRadius(void) const { return mRadius; }
//-----------------------------------------------------------------------
...
void DLight::getWorldTransforms(Matrix4 *xform) const {
  if (mParentLight->getType() == Light::LT_SPOTLIGHT) {
    Quaternion quat =
        Vector3::UNIT_Y.getRotationTo(mParentLight->getDerivedDirection());
...
bool DLight::isCameraInsideLight(Ogre::Camera *camera) {
  ...
  case Ogre::Light::LT_POINT: {
    Ogre::Real distanceFromLight = camera->getDerivedPosition().distance(
  ...
  case Ogre::Light::LT_SPOTLIGHT: {
    ...
    // tip accordingly. Some trigonometry needed here.
    Ogre::Vector3 clipRangeFix = -lightDir * (camera->getNearClipDistance() /
                                              Ogre::Math::Tan(attAngle / 2));
...
void DLight::updateFromCamera(Ogre::Camera *camera) {
  ...
  for (unsigned short i = 0; i < tech->getNumPasses(); i++) {
    ...
    // If inside light geometry, render back faces with CMPF_GREATER, otherwise
    // normally
    ...
    sm->getShadowCameraSetup()->getShadowCamera(
        sm, camera, camera->getViewport(), mParentLight, &shadowCam, 0);
