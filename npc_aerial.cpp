...
void npc_aerial::init(NPCSpawnProps props) {
  // LogManager::getSingleton().logMessage("Npc manager: npc enemy
  // initializing");
  mProps = props;
  ...
  // LogManager::getSingleton().logMessage("Npc manager: npc enemy ended
  // initializing"); mDebugPathNode =
  // mSceneMgr->getRootSceneNode()->createChildSceneNode("npc_aerial_path_"+props.mName);
  // mDebugLines = new Ogre::ManualObject("npc_aerial_path_"+props.mName);
  // mRoot->addFrameListener(this);
  health = props.health;
}

void npc_aerial::setPlayerPosition(Vector3 pos) { pPosit = pos; }
...
void npc_aerial::suspend() { going = false; }
...
void npc_aerial::npc_force_callback(OgreNewt::Body *me) {
  ...
  if (health1 != health) {
    ...
    if (((me->damagePosition - myPosN).y > mProps.headshotDist) &&
        (mProps.headshot)) {
    ...
    if (mProps.exploding) {
      ExplosionManager::getSingleton().spawnExplosionNosound(me->damagePosition,
                                                             Vector3(2, 2, 2));
      if (mProps.sounds)
        Run3SoundRuntime::getSingleton().emitSound("run3/sounds/explode_3.wav",
                                                   4, false, me->damagePosition,
                                                   200, 50, 800);
      kill();
  ...
  Ogre::Vector3 force2(steps.x * mVel + stdAdd.x * 5 * mVel, steps.y * mVel,
  ...
  if (!going && !dyeNow) {
    ...
    if ((!takingOff) &&
        (!landing_aer)) // Aircraft landing and taking off exception
    {
      me->setVelocity(Vector3::ZERO);
    }
  }
...
void npc_aerial::processEvent(int flag, String param1, String param2) {
  if (flag == SPAWN_NPC) {
    ...
    npcBody->setCustomForceAndTorqueCallback<npc_aerial>(
        &npc_aerial::npc_force_callback, this);
  ...
  if (flag == KILL_NPC) {
    if (mProps.sounds)
      Run3SoundRuntime::getSingleton().emitSound(
          "run3/sounds/bodysplat_1.wav", 2, false, npcBody->damagePosition, 200,
          50, 800);
    ...
    if (mProps.exploding) {
      ...
      if (mProps.sounds)
        Run3SoundRuntime::getSingleton().emitSound(
            "run3/sounds/explode_3.wav", 4, false, npcBody->damagePosition, 200,
            50, 800);
  ...
  if (!path_find || (!goal_achieved && stra)) {
    if (!dyeNow) {
      ...
      if ((sounds) && (mProps.sounds)) {
        ...
        if ((rand() % 50) == 3)
          Run3SoundRuntime::getSingleton().emitSound(
              "run3/sounds/cough" + StringConverter::toString(rand() % 3 + 1) +
                  ".wav",
...
bool npc_aerial::find_path() {
  ...
  // enemy"+StringConverter::toString(pos->getPosition()));
  // LogManager::getSingleton().logMessage("to
