...
enum COLLIDE_SOUND_TYPE { CONCRETE, METAL, BODY };

class AmbientSound {
public:
  AmbientSound(SoundManager *mgr, String sound, Vector3 pos, Real dist,
               Real fallofdist) {
    sMgr = mgr;
    mSound = sound;
    mPos = pos;
  ...
  {
    if ((global::getSingleton().getPlayer()->get_location() - mPos).length() <
        mDist)
      return true;
    return false;
  }
  ...
  void spawn() {
    ...
    if (!exists) {
      ...
      if (mMinGain > 0) {
        sMgr->setSound(mSoundAL, mPos, Vector3(1, 1, 1), Vector3(1, 1, 1),
                       mDist, false, false, mMinGain, 1, mFallOfDist);
      } else {
        sMgr->setSound(mSoundAL, mPos, Vector3(1, 1, 1), Vector3(1, 1, 1),
                       mDist, false, false, mDist, 1, mFallOfDist);
      }
      restorePhase();
  ...
  void enableAmbientSound() { disabled = false; }
  ...
  void restorePhase() {
    ...
    LogManager::getSingleton().logMessage("Phase restored to " +
                                          StringConverter::toString(phase));
  }

  unsigned int getSoundAL() { return mSoundAL; }
...
public:
  ...
  void emitSound(String sound, Real duration, bool loop, Vector3 pos, Real dist,
                 Real fallofDist);
  void emitSound(String sound, Real duration, bool loop, Vector3 pos,
                 Real minGain, Real rolloff, Real fallofDist);
  void emitSoundOp(String sound, Real duration, bool loop, Vector3 pos,
                   Real dist, Real fallofDist);
  void addAmbientSound(String sound, Vector3 pos, Real dist, Real fallofDist);
  void addAmbientSound(String name, String sound, Vector3 pos, Real dist,
                       Real fallofDist);
  void addAmbientSound(String name, String sound, Vector3 pos, Real dist,
                       Real fallofDist, Real gain);
