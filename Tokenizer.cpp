...
void Tokenizer::resetPosition(void) {
  std::string::iterator cp = buffer.begin();
  currentPosition = cp;
}
...
bool Tokenizer::isDelim(char c) { return delim.find(c) != std::string::npos; }
...
std::string Tokenizer::currentToken(void) { return thisToken; }
