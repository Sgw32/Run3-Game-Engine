...
void Run3SoundRuntime::emitSound(String sound, Real duration, bool loop) {
  ...
  unsigned int s;
  sMgr->loadAudio(sound, &s, loop);
  sMgr->setSound(s, global::getSingleton().getPlayer()->get_location(),
                 Vector3(1, 1, 1), Vector3(1, 1, 1), 60000.0f, false, false,
                 60000.0f, 1, 30000.0f);
  sMgr->playAudio(s, false);
  String msg = sound;
  msg += StringConverter::toString(duration);
...
void Run3SoundRuntime::emitSound(String sound, Real duration, bool loop,
                                 Vector3 pos, Real dist, Real fallofDist) {
  if (durations.size() > MAX_AUDIO_SOURCES - 1)
    return;
  if (sound == "none")
    return;
  ...
  unsigned int s;
  sMgr->loadAudio(sound, &s, loop);
  sMgr->setSound(s, pos, Vector3::ZERO, Vector3::ZERO, dist, false, false, dist,
                 dist, dist);
  sMgr->playAudio(s, false);
  String msg = sound;
  msg += StringConverter::toString(duration);
...
void Run3SoundRuntime::emitSound(String sound, Real duration, bool loop,
                                 Vector3 pos, Real minGain, Real rolloff,
                                 Real fallofDist) {
  if (durations.size() > MAX_AUDIO_SOURCES - 1)
    return;
  if (sound == "none")
  ...
  unsigned int s;
  sMgr->loadAudio(sound, &s, loop);
  sMgr->setSound(s, pos, Vector3::ZERO, Vector3::ZERO, 1000, false, false,
                 minGain, rolloff, fallofDist);
  sMgr->playAudio(s, false);
  String msg = sound;
  msg += StringConverter::toString(duration);
  LogManager::getSingleton().logMessage("Playing sound: file " + msg);
...
void Run3SoundRuntime::emitSoundOp(String sound, Real duration, bool loop,
                                   Vector3 pos, Real dist, Real fallofDist) {
  if (durations.size() > MAX_AUDIO_SOURCES - 1)
    return;
  if (sound == "none")
...
void Run3SoundRuntime::addAmbientSound(String sound, Vector3 pos, Real dist,
                                       Real fallofDist) {
  LogManager::getSingleton().logMessage(
      "adding AmbientSound1 to environment...");
  AmbientSound s = AmbientSound(sMgr, sound, pos, dist, fallofDist);
  asounds.push_back(s);
}

void Run3SoundRuntime::addAmbientSound(String name, String sound, Vector3 pos,
                                       Real dist, Real fallofDist) {
  LogManager::getSingleton().logMessage(
      "adding AmbientSound2 to environment...");
  AmbientSound s = AmbientSound(sMgr, sound, pos, dist, fallofDist);
  s.setName(name);
  asounds.push_back(s);
}
...
void Run3SoundRuntime::addAmbientSound(String name, String sound, Vector3 pos,
                                       Real dist, Real fallofDist, Real gain) {
  LogManager::getSingleton().logMessage(
      "adding AmbientSound3 to environment...");
...
void Run3SoundRuntime::setCheckAmbientSounds() { check = 3.0f; }
    ...
    if (durations[i2] > 0) {
      if (optim[i2]) {
        ...
        if (l < dists[i2] && sounds[i2] == 0) {
          ...
          sMgr->setSound(s, poss[i2], Vector3::ZERO, Vector3::ZERO, dists[i2],
                         false, false, dists[i2], dists[i2], dists[i2]);
          sMgr->playAudio(s, false);
          sounds[i2] = s;
        }
      }
