...
if (checkALError("init::alGenBuffers:") )
{
    ...
    Ogre::LogManager::getSingleton().logMessage("SoundManager: Error reloading
buffers!"); #endif
}*/
  alDeleteSources(MAX_AUDIO_SOURCES, mAudioSources);
  alDeleteBuffers(MAX_AUDIO_BUFFERS, mAudioBuffers);
...
bool SoundManager::checkALError(std::string pMsg) {
  ...
  case AL_INVALID_NAME:
    ...
    /*  case AL_INVALID_ENUM:
        Ogre::LogManager::getSingleton().logMessage(LML_CRITICAL,"ERROR
       SoundManager:: Invalid Enum"+pMsg); break;*/
...
bool SoundManager::loadAudio(std::string filename, unsigned int *audioId,
  ...
  if (mAudioSourcesInUseCount ==
      MAX_AUDIO_SOURCES) // out of Audio Source slots!
  {
#ifdef DEBUG_SOUNDSYSTEM
    Ogre::LogManager::getSingleton().logMessage(
        "SoundManager: out of Audio Source slots!");
#endif
    return false;
  }
...
#ifdef DEBUG_SOUNDSYSTEM
      Ogre::LogManager::getSingleton().logMessage(
          "SoundManager: The sound file isn't loaded in a buffer, attempt "
          "failed");
#endif
      return false; // failed!
    }
  ...
  while (mAudioSourceInUse[sourceID] == true)
    sourceID++;
...
#ifdef DEBUG_SOUNDSYSTEM
    Ogre::LogManager::getSingleton().logMessage(
        "SoundManager: ERROR loadSource()::alSourcei ");
#endif
    return false;
  }
...
int SoundManager::loadAudioInToSystem(std::string filename) {
  ...
  strcpy(mAudioBufferFileName[bufferID],
         filename.c_str()); // save the file descriptor
...
#ifdef DEBUG_SOUNDSYSTEM
    Ogre::LogManager::getSingleton().logMessage(
        "SoundManager: ERROR loadWAV::alutLoadWAVFile ");
#endif
    return false;
  }
...
#ifdef DEBUG_SOUNDSYSTEM
    Ogre::LogManager::getSingleton().logMessage(
        "SoundManager: ERROR alBufferData(pDestAudioBuffer, format, data, "
        "size, freq); ");
#endif
    return false;
  }
...
#ifdef DEBUG_SOUNDSYSTEM
    Ogre::LogManager::getSingleton().logMessage(
        "SoundManager: ERROR alutUnloadWAV(format, data, size, freq); ");
#endif
    return false;
  }
  return true;
...
bool SoundManager::setSound(unsigned int audioID, Vector3 position,
                            ...
                            float maxDistance, bool playNow, bool forceRestart,
                            float minGain, float rollOff, float distance) {
  if (audioID >= MAX_AUDIO_SOURCES || !mAudioSourceInUse[audioID])
    return false;
  ...
  // Set the max gain
  alSourcef(mAudioSources[audioID], AL_MAX_GAIN,
            1.0f); // TODO as parameter ? global ?

  // Set the rollof factor
  alSourcef(mAudioSources[audioID], AL_ROLLOFF_FACTOR,
            rollOff); // TODO as parameter ?

  // Set the rollof factor
  alSourcef(mAudioSources[audioID], AL_REFERENCE_DISTANCE,
            distance); // TODO as parameter ?
