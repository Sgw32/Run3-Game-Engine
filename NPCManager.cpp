...
void NPCManager::init() {
  ...
  mMatPair = new OgreNewt::MaterialPair(mWorld, mWorld->getDefaultMaterialID(),
                                        enemyMat);
  mMatPair->setContactCallback(mPhysCallback);
...
void NPCManager::npc_spawn(NPCSpawnProps npc) {
  ...
  if (className == "npc_enemy") {
    LogManager::getSingleton().logMessage(
        "Npc Manager: npc enemy requested for a spawn:");
    npc_enemy *enemy = new npc_enemy();
    enemy->init(npc);
    LogManager::getSingleton().logMessage(
        "Setting node list..." +
        StringConverter::toString(mNodeList->getNode().size()));
  ...
  if (className == "npc_neutral") {
    LogManager::getSingleton().logMessage(
        "Npc Manager: npc neutral requested for a spawn:");
  ...
  if (className == "npc_friend") {
    LogManager::getSingleton().logMessage(
        "Npc Manager: npc friend requested for a spawn:");
  ...
  if (className == "npc_aerial") {
    LogManager::getSingleton().logMessage(
        "Npc Manager: npc aerial requested for a spawn:");
...
void NPCManager::npc_event(String name, String param1, String param2) {
  for (unsigned int i = 0; i != enemies.size(); i++) {
    if (enemies[i]->getName() == name) {
      enemies[i]->processEvent(StringConverter::parseInt(param1), param2,
                               param2);
    }
  }
  for (unsigned int i = 0; i != friends.size(); i++) {
    if (friends[i]->getName() == name) {
      friends[i]->processEvent(StringConverter::parseInt(param1), param2,
                               param2);
    }
  }
  for (unsigned int i = 0; i != neutrals.size(); i++) {
    if (neutrals[i]->getName() == name) {
      neutrals[i]->processEvent(StringConverter::parseInt(param1), param2,
                                param2);
    }
  }
  for (unsigned int i = 0; i != aerials.size(); i++) {
    if (aerials[i]->getName() == name) {
      aerials[i]->processEvent(StringConverter::parseInt(param1), param2,
                               param2);
    }
  }
}

void NPCManager::npc_event(String name, String type, String param1,
                           String param2) {
  for (unsigned int i = 0; i != enemies.size(); i++) {
    if (enemies[i]->getName() == name) {
  ...
  for (unsigned int i = 0; i != neutrals.size(); i++) {
    if (neutrals[i]->getName() == name) {
      neutrals[i]->processEvent(StringConverter::parseInt(type), param1,
                                param2);
    }
  }
  for (unsigned int i = 0; i != aerials.size(); i++) {
