...
public:
  ...
  void show_overlay();
  void hide_overlay();
  void resetNearClip() { mCamera->setNearClipDistance(mProps.nearClip); }
  void resetFarClip() { mCamera->setFarClipDistance(mProps.farClip); }
  void resetFov() { mCamera->setFOVy(Degree(mProps.fov)); }
  void teleport(Vector3 dest);
  void setParentRelation(SceneNode *pPar);
  void setParentRelation(String node);
  void setFullParentRelation(bool allow) { mFullP = allow; }
  ...
  OgreNewt::Body *getBodyOnBottom() {
    ...
    OgreNewt::BasicRaycast wallRay1(mWorld, pos,
                                    pos - Vector3(0, cur_size.y * 2 + 50, 0));
  ...
  bool checkStairEffect(Vector3 pos, Vector3 direction) {
    ...
    OgreNewt::BasicRaycast wallRay1(mWorld, pos - Vector3(0, cur_size.y, 0),
                                    pos - Vector3(0, cur_size.y, 0) +
                                        direction * 50);
    OgreNewt::BasicRaycast wallRay2(
        mWorld, pos - Vector3(0, cur_size.y - mProps.stairEffect, 0),
        pos - Vector3(0, cur_size.y - mProps.stairEffect, 0) + direction * 40);
...
uv3 = new OgreNewt::BasicJoints::UpVector(mWorld,bod,Vector3::UNIT_Z);*/
  ...
  void shakeAmplitude() {
    ...
    Real a = a0 * exp(-b * st) *
             cos(sqrt(w * w - b * b) *
                 st); // harmonic oscilator a0-the amplitude in the beginning,
                      // b=2*Velocity of rotating / mass, w-cycle frequency;
    da = a - pa;
    pa = a;
    ...
    mCamera->setPosition(Vector3(Ogre::Math::RangeRandom(-fabs(a), fabs(a)),
                                 Ogre::Math::RangeRandom(-fabs(a), fabs(a)),
                                 Ogre::Math::RangeRandom(-fabs(a), fabs(a))));
    ...
    if (exp(-b * st) < 0.02) {
      ...
      LogManager::getSingleton().logMessage("shakend");
      // Real stdB =
      // mViewNode->_getDerivedOrientation().getRoll().valueDegrees();
  ...
  bool getSkippingCutScene() { return skippingCutScene; }
  ...
  Real getMouseSensivity() { return mRotate / 2.0f; }
  void setMouseSensivity(Real mSens) {
    mRotate = mSens * 2.0f;
    LogManager::getSingleton().logMessage("Now sens is:" +
                                          StringConverter::toString(mRotate));
  ...
  inline Real getRegenerationMultiplier() { return regen_Mul; }
  inline Real getRegenerationHealth() { return mMaxHealth; }
