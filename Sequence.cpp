...
Sequence::~Sequence() { mRoot->removeFrameListener(this); }

void Sequence::init(Ogre::Root *root, SceneManager *scene,
                    SoundManager *sound) {
  ...
  lua_register(pLuaState, "SkyX_setRayleighMultiplier",
               SkyX_setRayleighMultiplier);
...
void Sequence::SetSceneSeq(String seq) {
  if (seq != "none") {
    ...
    try {
      ...
      if (SequenceDoc->Error()) {
        ...
        LogManager::getSingleton().logMessage(
            "[Sequence] This scene has no sequence,or TiXml reporated an "
            "error.");
        LogManager::getSingleton().logMessage(String(SequenceDoc->ErrorDesc()));
        LogManager::getSingleton().logMessage(
            StringConverter::toString(SequenceDoc->ErrorRow()));
        LogManager::getSingleton().logMessage(
            StringConverter::toString(SequenceDoc->ErrorCol()));
        return;
      }
    } catch (...) {
      rest = true;
      // We'll just log, and continue on gracefully
      LogManager::getSingleton().logMessage(
          "[Sequence] This scene has no sequence,or TiXml reporated an error.");
      return;
    }
    ...
    if (String(SequenceRoot->Value()) != "sequence") {
      LogManager::getSingleton().logMessage(
          "[Sequence] Error: Invalid .xml File. Missing <sequence>");
      return;
    }
...
void Sequence::processLuaScript(TiXmlElement *XMLNode) {
  LogManager::getSingleton().logMessage("Lua run:" +
                                        getAttrib(XMLNode, "script", ""));
  RunLuaScript(pLuaState, getAttrib(XMLNode, "script", "").c_str());
}

void Sequence::processLuaScriptOnExit(TiXmlElement *XMLNode) {
  LogManager::getSingleton().logMessage("Lua deferred run:" +
                                        getAttrib(XMLNode, "script", ""));
...
void Sequence::processTrigger(TiXmlElement *XMLNode) {
  ...
  if (sw) {
    ...
    trig->init(AxisAlignedBox(x1, y1, z1, x2, y2, z2),
               global::getSingleton().getPlayer());
  trig->setEnabled(en);
  ...
  if (multiple) {
    trig->setMultiple(getAttrib(XMLNode, "luaOnEnter", ""),
                      getAttrib(XMLNode, "luaOnLeave", ""));
...
void Sequence::processTrain(TiXmlElement *XMLNode) {
  ...
  Entity *ent = mSceneMgr->createEntity(
      name, getAttrib(pElement, "meshFile", "box.mesh"));
  if (dist != -1.0f)
    ent->setRenderingDistance(dist);
  String materialFile = getAttrib(XMLNode, "materialFile");
  if (!materialFile.empty())
    ent->setMaterial(
        MaterialManager::getSingleton().getByName(materialFile)); // SKINZ
  pElement = XMLNode->FirstChildElement("scale");
  ...
  SceneNode *sn = mSceneMgr->getRootSceneNode()->createChildSceneNode(
      name, parseVector3(pElement), rot);
  sn->setScale(scale);
  sn->attachObject(ent);
  ...
  train->stopSound =
      getAttrib(XMLNode, "stopSound", "run3/sounds/tram_squeak.wav");
  train->startSound =
      getAttrib(XMLNode, "startSound", "run3/sounds/train_horn2.wav");
  train->movingSound =
      getAttrib(XMLNode, "movingSound", "run3/sounds/train2.wav");
  ...
  while (pElement) {
    train->addKeyPoint(parseVector3(pElement),
                       getAttrib(pElement, "script", "none"),
                       getAttribBool(pElement, "startup", false), false);
  ...
  if (!sim) {
    ...
    if (pElement) {
      ...
      // Entity* ent =
      // mSceneMgr->createEntity(getAttrib(pElement1,"name",""),getAttrib(pElement1,"meshFile","box.mesh"));
    ...
    while (pElement) {
      ...
      Entity *ent =
          mSceneMgr->createEntity(getAttrib(pElement1, "name", ""),
                                  getAttrib(pElement1, "meshFile", "box.mesh"));
      if (dist != -1.0f)
        ent->setRenderingDistance(dist);
      LogManager::getSingleton().logMessage("1");
      pElement1 = pElement->FirstChildElement("scale");
      ...
      pElement1 = pElement->FirstChildElement("position");
      // ent =
      // mSceneMgr->createEntity(getAttrib(XMLNode,"name",""),getAttrib(pElement,"meshFile","box.mesh"));
      if (ent)
        train->addDetail(ent, parseVector3(pElement1), scale, rot);
      else
        LogManager::getSingleton().logMessage(
            "Train: error adding a detail object");
...
void Sequence::processButton(TiXmlElement *XMLNode) {
  ...
  if (buttonName.empty()) {
    ...
    bNod = mSceneMgr->getRootSceneNode()->createChildSceneNode(buttonName, pos,
                                                               rot);
  }
  bNod->setScale(scale);
  ...
  if (type == "test2") {
    ...
    while (pElement) {
      ...
      if (ent)
        ...
        LogManager::getSingleton().logMessage(
            "FuzzyObj: error adding a detail object");
    ...
    tst->setRegType(
        StringConverter::parseInt(getAttrib(XMLNode, "reg_type", "0")));
...
void Sequence::processLadder(TiXmlElement *XMLNode) {
  ...
  if (objName.empty())
    ...
    bNod =
        mSceneMgr->getRootSceneNode()->createChildSceneNode(objName, pos, rot);
  bNod->setScale(scale);
  Entity *ent = mSceneMgr->createEntity(bNod->getName(), fileName);
  String materialFile = getAttrib(XMLNode, "materialFile");
...
void Sequence::processNPC(TiXmlElement *XMLNode) {
  ...
  if (pElement) {
    soundAttack =
        getAttrib(pElement, "soundAttack", "run3/sounds/attack_growl3.wav");
    soundHit = getAttrib(pElement, "soundHit", "run3/sounds/iceaxe_swing1.wav");
    soundFind =
        getAttrib(pElement, "soundFind", "run3/sounds/attack_growl3.wav");
    soundRandom1 =
        getAttrib(pElement, "soundRandom1", "run3/sounds/breathing3.wav");
    soundSubRandom21 =
        getAttrib(pElement, "soundSubRandom21", "run3/sounds/cough1.wav");
    soundSubRandom22 =
        getAttrib(pElement, "soundSubRandom22", "run3/sounds/cough1.wav");
    soundSubRandom23 =
        getAttrib(pElement, "soundSubRandom23", "run3/sounds/cough1.wav");
  ...
  prop.headAxis =
      StringConverter::parseVector3(getAttrib(XMLNode, "headAxis", "0 1 0"));
  ...
  LogManager::getSingleton().logMessage(
      "Sequence: npc passed, all data was got;ready for spawn");
...
void Sequence::processNPCGroup(TiXmlElement *XMLNode) {
  ...
  LogManager::getSingleton().logMessage(
      "Sequence: npc group passed, all data was got;ready for spawn");
}
...
void Sequence::processLensFlare(TiXmlElement *XMLNode) {
  ...
  LensFlare *lensflare =
      new LensFlare(pos, global::getSingleton().getCamera(), mSceneMgr, scale);
...
void Sequence::processCutScene(TiXmlElement *XMLNode) {
  ...
  if (music) {
    cscene->setMusicSync(getAttrib(XMLNode, "musicFile", ""),
                         getAttribReal(XMLNode, "musicLength", 0.0f));
  }
  ...
  while (pElement) {
    ...
    if (!or) {
      ...
      quat = StringConverter::parseQuaternion(
          getAttrib(pElement, "orient", "1 0 0 0"));
...
void Sequence::processCutSceneEvent(TiXmlElement *XMLNode) {
  ...
  if (cscene) {
    ...
    while (pElement) {
      ...
      LogManager::getSingleton().logMessage(
          "(Cutscene)Lua run script processing.");
      if (!script.empty()) {
...
void Sequence::processSeqScriptEvent(TiXmlElement *XMLNode) {
  ...
  if (cscene) {
    ...
    while (pElement) {
      ...
      LogManager::getSingleton().logMessage(
          "(SeqScript)Lua run script processing.");
      if (!script.empty()) {
        cscene->addLuaRun(sec, script);
      }
      pElement = pElement->NextSiblingElement("run");
...
void Sequence::processPickup(TiXmlElement *XMLNode) {
  ...
  Vector3 pos = parseVector3(XMLNode);
  Vector3 scale =
      Vector3(getAttribReal(XMLNode, "sX", 1), getAttribReal(XMLNode, "sY", 1),
              getAttribReal(XMLNode, "sZ", 1));
...
void Sequence::processDoor(TiXmlElement *XMLNode) {
  ...
  Vector3 dir = Vector3(getAttribReal(XMLNode, "dirX", 0),
                        getAttribReal(XMLNode, "dirY", 0),
                        getAttribReal(XMLNode, "dirZ", 1));
  Vector3 axis =
      Vector3(getAttribReal(XMLNode, "aX", 1), getAttribReal(XMLNode, "aY", 0),
              getAttribReal(XMLNode, "aZ", 0));
  ...
  door->setLuaScripts(getAttrib(XMLNode, "lOnOpen"),
                      getAttrib(XMLNode, "lOnClosed"),
                      getAttrib(XMLNode, "lOnStarted"));
  ...
  if (ParentName.empty()) {
    if (!name.empty()) {
      doorn =
          mSceneMgr->getRootSceneNode()->createChildSceneNode(name, pos, quat);
    } else {
  ...
  if (rotating) {
    door->setRotateDoor(
        getAttribReal(XMLNode, "pitch"), getAttribReal(XMLNode, "yaw"),
        getAttribReal(XMLNode, "roll"), getAttribReal(XMLNode, "rotspeed"));
  }
...
void Sequence::processPendulum(TiXmlElement *XMLNode) {
  ...
  if (rotating) {
    if (!positionPend) {
      ...
      door->setRotateDoor(getAttribReal(XMLNode, "pitch"),
                          getAttribReal(XMLNode, "yaw"),
                          getAttribReal(XMLNode, "roll"),
                          getAttribReal(XMLNode, "rotspeed"), dir);
    }
  }
...
void Sequence::processComputer(TiXmlElement *XMLNode) {
  ...
  bool allowVirtualDisplay =
      getAttribBool(XMLNode, "allowVirtualDisplay", true);
  Vector3 pos;
  Vector3 scale = Vector3(1, 1, 1);
  Quaternion quat;
  TiXmlElement *pElement = XMLNode->FirstChildElement("position");
  ...
  String materialFile = getAttrib(XMLNode, "materialFile");
  if (!materialFile.empty())
    eComp->getSubEntity(0)->setMaterial(
        MaterialManager::getSingleton().getByName(materialFile)); // SKINZ
  ...
  LogManager::getSingleton().logMessage(
      "comp" + StringConverter::toString(cComp->getPosition()));
}
...
#define CLEAR_T4(t, v)                                                         \
  ;                                                                            \
  for (vector<t *>::iterator m = v.begin(); m != v.end(); m++) {               \
    (*m)->dispose();                                                           \
    delete (*m);                                                               \
  }                                                                            \
  v.clear();
  ...
  if (!rest) {
    ...
    if (global::getSingleton().port) {
      ...
      if (a == 'r') {
        ...
        // String str = String(*&d,23);
        LogManager::getSingleton().logMessage(
            "(Sequence) X1 Register operation from COM:");
        // global::getSingleton().port->ReadData(&a,1);
...
bool Sequence::frameEnded(const Ogre::FrameEvent &evt) { return true; }

String Sequence::getAttrib(TiXmlElement *XMLNode, const String &attrib,
...
Real Sequence::getAttribReal(TiXmlElement *XMLNode, const String &attrib,
                             Real defaultValue) {
  if (XMLNode->Attribute(attrib.c_str()))
...
bool Sequence::getAttribBool(TiXmlElement *XMLNode, const String &attrib,
                             bool defaultValue) {
  if (!XMLNode->Attribute(attrib.c_str()))
    return defaultValue;
  ...
  return orientation *
         Vector3(
             StringConverter::parseReal(XMLNode->Attribute("x")) +
                 getAttribReal(
                     XMLNode, "tx",
                     0.0f), // StringConverter::parseReal(XMLNode->Attribute("tx")),
             StringConverter::parseReal(XMLNode->Attribute("y")) +
                 getAttribReal(
                     XMLNode, "ty",
                     0.0f), // StringConverter::parseReal(XMLNode->Attribute("ty")),
             StringConverter::parseReal(XMLNode->Attribute("z")) +
                 getAttribReal(
                     XMLNode, "tz",
                     0.0f) // StringConverter::parseReal(XMLNode->Attribute("tz"))
             ) *
