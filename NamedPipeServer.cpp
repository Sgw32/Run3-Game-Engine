...
NamedPipeServer::~NamedPipeServer() { CloseHandle(hPipe); }

void NamedPipeServer::init() { createNamedPipe(); }

int NamedPipeServer::createNamedPipe() {
  hPipe = CreateNamedPipe(g_szPipeName,               // pipe name
  ...
  if (INVALID_HANDLE_VALUE == hPipe) {
    LogManager::getSingleton().logMessage(
        "Error occurred while creating the pipe.");
...
int NamedPipeServer::WriteToPipe(string data) {
  ...
  bResult = WriteFile(hPipe,        // handle to pipe
                      ...
                      strlen(data.c_str()) +
                          1,    // number of bytes to write, include the NULL
...
int NamedPipeServer::readMessage() {
  ...
  bResult = ReadFile(hPipe,            // handle to pipe
...
void NamedPipeServer::parseToMotorValues(string dta) {
  ...
  LogManager::getSingleton().logMessage(
      "motor1234 " + StringConverter::toString(motor1) + " " +
