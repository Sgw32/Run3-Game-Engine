...
void PhysObject::init(SceneManager *SceneMgr, OgreNewt::World *world) {
  if (!SceneMgr) {
    throw Ogre::Exception(
        -1, "SceneMgr :  assertion failed at PhysObject::init", "");
  }
  scene = SceneMgr;
  if (!world) {
...
void PhysObject::CreateStaticBoxTransparent(String name,
                                            Ogre::SceneNode *node) {
...
void PhysObject::addToBatch() {
  Run3Batcher::getSingleton().addObject(ent, nod->getPosition(),
                                        nod->getScale(), nod->getOrientation());
...
void PhysObject::CreateStaticCylTransparent(String name, Ogre::SceneNode *node,
                                            Real radius, Real height) {
  // Ogre::Vector3 size=node->getScale();
  OgreNewt::Collision *col =
      new OgreNewt::CollisionPrimitives::Cylinder(mWorld, radius, height);
  OgreNewt::Body *bod = new OgreNewt::Body(mWorld, col);
  bod->attachToNode(node);
  bod->setName(node->getName());
  bod->setPositionOrientation(node->getPosition(), node->getOrientation());
...
void PhysObject::CreateStaticCyl(String name, Ogre::SceneNode *node,
                                 Real radius, Real height) {
...
/*void PhysObject::CreateObject(String mesh,Ogre::SceneNode *node,Ogre::Real
mass,bool idleanim,String idle)
{
//Entity
...
isdynamic=true;
        ...
        Ogre::Vector3 Ogre::Vector3 size =
Vector3(fabs(max.x-min.x),fabs(max.y-min.y),fabs(max.z-min.z));
        //Rigid body
        OgreNewt::Collision* col = new
OgreNewt::CollisionPrimitives::Box(mWorld, Ogre::Vector3 size); bod = new
OgreNewt::Body( mWorld, col ); bod->attachToNode( node );
        // initial position
        bod->setPositionOrientation( node->getPosition()-(center*scale),
node->getOrientation() ); delete col;
        //Make it movable!
        //mass;
        inertia = OgreNewt::MomentOfInertia::CalcBoxSolid( mass, Ogre::Vector3
Ogre::Vector3 size); bod->setMassMatrix( mass, inertia ); String n =
node->getName(); bod->setName(n); bod->setMaterialGroupID( physicalMat );
        //bod->setStandardForceCallback();
        bod->setCustomForceAndTorqueCallback<PhysObject>(
&PhysObject::camera_force_callback ,this); nod=node; bod->setAutoFreeze(0);
        midleanim=idleanim;
                if (idleanim)
                {
...
void PhysObject::CreateStaticPhys_Box(String name, String mesh,
                                      Ogre::SceneNode *node) {
  if (!(scene->hasEntity(name)))
    ent = scene->createEntity(name, mesh);
  else {
...
void PhysObject::CreateStaticObject(String name, String mesh,
                                    Ogre::SceneNode *node) {
  if (!(scene->hasEntity(name)))
    ent = scene->createEntity(name, mesh);
  else {
...
void PhysObject::CreateObject(String name, String mesh, Ogre::SceneNode *node,
                              Ogre::Real mass, bool idleanim, String idle) {
  // Entity
  if (!(scene->hasEntity(name)))
    ent = scene->createEntity(name, mesh);
  else {
  ...
  // bod->setStandardForceCallback();
  bod->setCustomForceAndTorqueCallback<PhysObject>(
      &PhysObject::camera_force_callback, this);
  nod = node;
  bod->setAutoFreeze(0);
  midleanim = idleanim;
  if (idleanim) {
...
void PhysObject::transformToPhysObj() {
  if (!isdynamic) {
    ...
    // initial position
    bod->setPositionOrientation(nod->getPosition() - (center * scale),
                                nod->getOrientation());
    delete col;
    // Make it movable!
    // mass;
...
void PhysObject::addForce(Vector3 force) {
  ...
  applyforce = true;
  if (breakable) {
    LogManager::getSingleton().logMessage("breakable:health " +
                                          StringConverter::toString(CurHealth));
...
void PhysObject::brk() {
  if (breakable) {
    ...
    Run3SoundRuntime::getSingleton().emitSound(
        "run3/sounds/wood_crate_break4.wav", 2, false, p, 300, 300);
    GibManager::getSingleton().spawnGibs(mGibFile, p, mGibScale, 10.0f, gc, 40);
    if (isExplosive) {
      ExplosionManager::getSingleton().spawnExplosionNosound(p,
                                                             Vector3(2, 2, 2));
...
void PhysObject::camera_force_callback(OgreNewt::Body *me) {
  ...
  {
          LogManager::getSingleton().logMessage("breakable:health
  "+StringConverter::toString(CurHealth)); CurHealth -= mStrength;
  }*/
  ...
  if (dragging)
    me->setPositionOrientation(
        plPos + global::getSingleton().getPlayer()->get_direction() * 200 +
            Vector3(0, 50, 0),
        global::getSingleton().getPlayer()->get_orientation());
  ...
  if (attachedToBone) {
    me->setPositionOrientation(
        deltaToObject +
            attachedToBone->_getDerivedPosition() *
                attachedToParent->_getDerivedScale() +
            attachedToParent->_getDerivedPosition(),
        Quaternion::IDENTITY); // attachedToBone->_getDerivedOrientation());
  ...
  // me->setOmega(Vector3(0,camera_rotation_x,0));  // To rotate the camera in
  // the X axes
  if (breakable) {
