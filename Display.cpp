...
void Display::init(bool shows, lua_State *pL) {
  ...
  /*Ogre::String lMaterialName = "MyRttMaterial";
          {
                          Ogre::MaterialManager& lMaterialManager =
     Ogre::MaterialManager::getSingleton(); Ogre::MaterialPtr lMaterial =
     lMaterialManager.create(lMaterialName,"General"); Ogre::Technique *
     lTechnique = lMaterial->getTechnique(0); Ogre::Pass* lPass =
     lTechnique->getPass(0); Ogre::TextureUnitState* lTextureUnit =
     lPass->createTextureUnitState();
...
void Display::setMaterial(String name) { dispCont->setMaterialName(name); }
...
bool Display::frameEnded(const Ogre::FrameEvent &evt) { return true; }
...
void Display::show() {
  ...
  if ((Loader::getSingleton().rtt_display) && (allowVirtualDisplay)) {
    global::getSingleton().getCamera()->getViewport()->setOverlaysEnabled(
        false);
    Ogre::TextureManager &lTextureManager =
    ...
    lTextureWithRtt = lTextureManager.createManual(
        lTextureName, "General", Ogre::TEX_TYPE_2D, 512, 512, lNumMipmaps,
        Ogre::PF_R8G8B8, Ogre::TU_RENDERTARGET, 0, lGammaCorrection,
        lAntiAliasing);

    Ogre::RenderTexture *lRenderTarget = NULL;
    {
      Ogre::HardwarePixelBufferSharedPtr lRttBuffer =
          lTextureWithRtt->getBuffer();
      ...
      lRttViewport1 =
          lRenderTarget->addViewport(lRttCamera, 50, 0.00f, 0.00f, 1.0f, 1.0f);
    ...
    Ogre::MaterialPtr lMaterial =
        lMaterialManager.getByName(((Computer *)curcomputer)->mDispMatName);
...
void Display::runScript(String luas) { RunLuaScript(pLuaState, luas.c_str()); }
