...
void Event::setSpawnPhys(String name, String fileName, Vector3 dest,
                         Real secs) {
...
void Event::interpretate_callback() {
  ...
  if (entc_callback) {
    ...
    for (i = 0; i != entc_name.size(); i++) {
      ...
      SceneNode *newNode = global::getSingleton()
                               .getSceneManager()
                               ->getRootSceneNode()
                               ->createChildSceneNode(entc_spawn[i]);
      try {
        phys->init(global::getSingleton().getSceneManager(),
                   global::getSingleton().getWorld());
        phys->CreateObject(entc_name[i], entc_meshFile[i], newNode, 100.0,
                           false, "");
