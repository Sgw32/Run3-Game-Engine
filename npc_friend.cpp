...
void npc_friend::init(NPCSpawnProps props) {
  // LogManager::getSingleton().logMessage("Npc manager: npc friend
  // initializing");
  mProps = props;
  ...
  // LogManager::getSingleton().logMessage("Npc manager: npc friend ended
  // initializing"); mDebugPathNode =
  // mSceneMgr->getRootSceneNode()->createChildSceneNode("npc_friend_path_"+props.mName);
  // mDebugLines = new Ogre::ManualObject("npc_friend_path_"+props.mName);
  // mRoot->addFrameListener(this);
  health = props.health;
}

void npc_friend::setPlayerPosition(Vector3 pos) { pPosit = pos; }

void npc_friend::start() { going = true; }

void npc_friend::suspend() { going = false; }
...
void npc_friend::npc_force_callback(OgreNewt::Body *me) {
  ...
  // Ogre::Vector3
  // force2(steps.x*mVel+stdAdd.x*5*mVel,0,steps.z*1000+stdAdd.z*5*mVel);
...
bool npc_friend::isOnEarth() {
  ...
  OgreNewt::BasicRaycast floorRay(
      mWorld, myPos, myPos + Ogre::Vector3::NEGATIVE_UNIT_Y * (size.y + 50));
  int hits;
  ...
  if (hits > 0) {
    ...
    if (!hit.mBody)
      return false;
    distToFloor =
        hit.mDistance * (size.y + 50); // calculale the distance to the floor
...
void npc_friend::processEvent(int flag, String param1, String param2) {
  if (flag == SPAWN_NPC) {
    ...
    npcBody->setCustomForceAndTorqueCallback<npc_friend>(
        &npc_friend::npc_force_callback, this);
  ...
  if (flag == FEAR_NPC) {
    LogManager::getSingleton().logMessage(
        "NPC is trying to escape from this horrible situation");
    moveActivity = 0.3f;
  }
  if (flag == CRAZY_NPC) {
    moveActivity = 0.3f;
  ...
  if (going) {
    ...
    if (fstTimer > 0.5f) {
      ...
      if (fstindex>footsteps.size()-1)
              fstindex=0;
      fstTimer=0.0f;*/
      String fname = "run3/sounds/footsteps/foot" +
                     StringConverter::toString(fstindex - 1) + ".wav";
      Run3SoundRuntime::getSingleton().emitSound(fname, 1.0f, false, myPos, 200,
                                                 50, 800);
    ...
    if (farFindB && path_find && !stra) {
      ...
      if (!dyeNow) {
        ...
        if (rotation_needed) {
          ...
          ///"+StringConverter::toString(dir)+"
          ///"+StringConverter::toString(angle));
          ////LogManager::getSingleton().logMessage("Rotating in Player not
          ///found");
    ...
    if (!path_find || (!goal_achieved && stra)) {
      if (!dyeNow) {
        ...
        if (sounds) {
          if ((rand() % 50) == 2)
            Run3SoundRuntime::getSingleton().emitSound(
                "run3/sounds/breathing3.wav", 3, false, myPos, 200, 50, 800);
          if ((rand() % 50) == 3)
            Run3SoundRuntime::getSingleton().emitSound(
                "run3/sounds/cough" +
                    StringConverter::toString(rand() % 3 + 1) + ".wav",
                3, false, myPos, 200, 50, 800);
        }
        ...
        if (previous_player) {
          ...
          Run3SoundRuntime::getSingleton().emitSound(
              "run3/sounds/attack_growl3.wav", 6, false, myPos, 200, 50, 800);
        ...
        if (farFindB) {
          ...
          if (dir.length() < attackAnimDist) {
            ...
            if ((rand() % 20) == 4)
              Run3SoundRuntime::getSingleton().emitSound(
                  "run3/sounds/attack_growl3.wav", 3, false, myPos, 200, 50,
                  800);

            if ((rand() % 5) == 4)
              Run3SoundRuntime::getSingleton().emitSound(
                  "run3/sounds/iceaxe_swing1.wav", 3, false, myPos, 200, 50,
                  800);
...
bool npc_friend::find_path() {
  ...
  // LogManager::getSingleton().logMessage("Nodes
  // size:"+StringConverter::toString(nodes.size()));
  NPCNode *pos = nodes[0];
  NPCNode *end = nodes[0];
  s = nodes.size();

  // LogManager::getSingleton().logMessage("finding closest
  // nodes..."+StringConverter::toString(s));
  for (unsigned int i = 0; i != s; i++) {
  ...
  // LogManager::getSingleton().logMessage("to
  // friend"+StringConverter::toString(pos->getPosition()));
  // LogManager::getSingleton().logMessage("to
  // player"+StringConverter::toString(end->getPosition()));
  // LogManager::getSingleton().logMessage("Starting pathfinding...");
  mPath = AIManager::getSingleton().getPathFinder()->search(DEPTH_FIRST_SEARCH,
                                                            pos, end);
#ifdef _DEBUG
