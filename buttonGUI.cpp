...
buttonPosition::buttonPosition(const relativePosition &relPosition,
                               short offsetLeft, short offsetTop) {
  usingRelative = true;
...
textScheme::textScheme(std::string font, short unsigned size, float r, float g,
                       float b, float a)
...
button::button(std::string &buttonName, std::string &material,
    ...
    : name(buttonName), buttonMgr(mgr), overlay(o), parent(parentButton),
      ...
      centerOnGrab(true), isMouseOver(0), textStyle(style),
      isActive(isActiveButton),
      trigger_onClick(
          true), // if you would like events to be sent out or suppressed by
                 // default you can set that behavior here
      trigger_onRelease(true), trigger_mouseOver(true), trigger_mouseOff(true),
      trigger_onSubmit(true), useLimits(false), x1Limit(0), x2Limit(0),
      y1Limit(0), y2Limit(0) {
  suppressed = false;
  mLua = "";
  if (parent) {
...
button *button::createChildButton(std::string name, std::string material,
                                  buttonPosition &position, short width,
                                  short height, bool isActiveButton) {
  return createChildButton(name, material, position, width, height, textStyle,
                           isActiveButton);
}
button *button::createChildButton(std::string name, std::string material,
                                  buttonPosition &position, short width,
                                  short height, textScheme &scheme,
                                  bool isActiveButton) {
  button *b = new button(name, material, position, width, height, buttonMgr,
                         NULL, this, scheme, isActiveButton);
...
textInputArea *button::createTextInputArea(std::string textInputName,
                                           TextAreaOverlayElement::Alignment a,
                                           buttonPosition &position,
                                           short width, short height,
                                           short textPosX, short textPosY,
                                           bool isActiveButton) {
  return createTextInputArea(textInputName, a,
                             buttonMgr->defaultTextFieldMaterial, position,
                             width, height, textPosX, textPosY, textStyle,
                             isActiveButton); // use the last textStyle assigned
                                              // to this button and pass it on.
}

textInputArea *button::createTextInputArea(
    ...
    short textPosX, short textPosY, bool isActiveButton) {
  return createTextInputArea(textInputName, a, material, position, width,
                             height, textPosX, textPosY, textStyle,
                             isActiveButton); // use the last textStyle assigned
                                              // to this button and pass it on.
}
...
textInputArea *button::createTextInputArea(
    std::string textInputName, TextAreaOverlayElement::Alignment a,
    std::string material, buttonPosition &position, short width, short height,
    short textPosX, short textPosY, textScheme &style, bool isActiveButton) {
  textInputArea *t = new textInputArea(buttonMgr, textInputName, a, material,
                                       position, this, width, height, textPosX,
                                       textPosY, style, isActiveButton);
  t->setMovable(false);
...
buttonMesh *button::addButtonMesh(std::string name, std::string meshFile,
                                  short positionX, short positionY, short width,
                                  short height) {
  buttonMesh *bm =
      new buttonMesh(name, meshFile, buttonMgr, this, buttonMgr->zOrderCounter,
...
buttonMesh *button::getButtonMesh(std::string name) {
  ...
  for (meshItr = buttonMeshes.begin(); meshItr != buttonMeshes.end();
       ++meshItr) {
...
button *button::getChildButton(std::string name) {
  ...
  for (buttonItr = childButtons.begin(); buttonItr != childButtons.end();
       ++buttonItr) {
...
button *button::hide(bool fade, unsigned short fadeDurationMS) {
  ...
  if (fadingIn)
    fadingInStart = fadingInEnd = fadingOutStart = fadingOutEnd = fadingIn =
        fadingOut = 0;
...
button *button::show(bool fade, unsigned short fadeDurationMS) {
  ...
  if (fade) {
    ...
    fadingIn = true;
    setOpacity(0); // clear any half-transparency action from past events so it
                   // can fade from pure nothing.
  } else
...
void button::update(void) {
  ...
  if (fadingIn) {
    if (fadingInEnd < timer.getMilliseconds()) {
      ...
      setSuppressed(
          false); // allow messages to be sent again now that its fully showing.

      setOpacity(1);
    } else {
      fadeMod = (float)(timer.getMilliseconds() - fadingInStart) /
                (float)(fadingInEnd - fadingInStart);
      setOpacity(fadeMod);
    }
  } else if (fadingOut) {
    if (fadingOutEnd < timer.getMilliseconds()) {
      ...
      fadeMod = 1 - (float)(timer.getMilliseconds() - fadingOutStart) /
                        (float)(fadingOutEnd - fadingOutStart);
      setOpacity(fadeMod);
    }
  }
}
...
button *button::setOpacity(float opacity) {
  // do not check for panel visibility here so that you can still set the
  // opacity of something to prepare it for showing at half opacity, for
  // example.

  if (!panel->isTransparent()) // if the panel is transparent it means it has no
                               // material at all.
  {
...
void button::setMaterialOpacity(float opacity, Ogre::MaterialPtr &mat) {
  ...
  Technique *t = mat->getTechnique(
      0); // we are only bothering the fade with the first technique.
  ...
  unsigned short s;
  for (p = 0; p < passNum; p++) {
    t->getPass(p)->setSceneBlending(
        SBT_TRANSPARENT_ALPHA); // comment this line if you do not want to
                                // enforce material setup needed for fading.
    unitStateNum = t->getPass(p)->getNumTextureUnitStates();
    for (s = 0; s < unitStateNum; s++) {
      t->getPass(p)->getTextureUnitState(s)->setAlphaOperation(
          LBX_MODULATE, LBS_MANUAL, LBS_TEXTURE, opacity);
    }
  }
}

button *button::setMaterial(std::string materialName) {
  // we need to make an instance of every material for every button so that when
  // one fades,  it doesnt fade any other buttons that may share the material.
...
button *button::setPosition(buttonPosition &p) {
  ...
  if (position.usingRelative) {
    ...
    if (parent)
      center = int((parent->panel->getWidth() / 2.0f) - (width / 2.0f)) +
               position.data.rel
                   .x; // if this is a child, this should be relative to parent.
    int right = winWidth - (int)width - position.data.rel.x;
    if (parent)
      right = int(parent->panel->getWidth() - width) -
              position.data.rel
                  .x; // if this is a child, this should be relative to parent.

    int top = 0 + position.data.rel.y;
    int middle =
        int((winHeight / 2.0f) - (height / 2.0f)) + position.data.rel.y;
    if (parent)
      middle = int((parent->panel->getHeight() / 2.0f) - (height / 2.0f)) +
               position.data.rel
                   .y; // if this is a child, this should be relative to parent.
    int bottom = winHeight - (int)height - position.data.rel.y;
    if (parent)
      bottom = int(parent->panel->getHeight() - height) -
               position.data.rel
                   .y; // if this is a child, this should be relative to parent.
    ...
    case CENTER:
      ...
      break;
    default:
      panel->setPosition((Ogre::Real)position.data.rel.x,
                         (Ogre::Real)position.data.rel.y);
      break;
    }
  } else // using absolute position
  {
    if (overlay == NULL) // are we a child?
    { // we are a child so we need to get the offset we are inheriting from
      // parents so we can set to our actual world position by using the local
      ...
      int targetLocalLeft =
          position.data.abs.left + currentLocalLeft - absPos.data.abs.left;
      int targetLocalTop =
          position.data.abs.top + currentLocalTop - absPos.data.abs.top;
      panel->setPosition((Ogre::Real)targetLocalLeft,
                         (Ogre::Real)targetLocalTop);
    } else
...
void button::resetPosition(void) { setPosition(position); }

const buttonPosition *button::getPosition(void) { return &position; }
...
buttonPosition button::getAbsolutePosition(void) {
  ...
  // we need to find out if he has parents,  and if so,  offset  by their
  // top/left values
  if (overlay == NULL) // this means this guy is a child because only the top
                       // level buttons have overlays.
  {
...
button *button::setLimits(short int x1, short int y1, short int x2,
                          short int y2, bool useLimits) {
  this->useLimits = useLimits;
...
button *button::setSize(short unsigned int width, short unsigned int height) {
  ...
  // reposition all its children which may be positioned relatively inside the
  // button.
  for (buttonItr = childButtons.begin(); buttonItr != childButtons.end();
       ++buttonItr) {
...
button *button::setUV(Ogre::Real u1, Ogre::Real v1, Ogre::Real u2,
                      Ogre::Real v2) {
...
void button::setParent(button *newParentButton) {
  if (name == "CURSOR") // do not allow reparenting of the cursor button,  this
                        // is unsupported. (ie there is no interface to parent
                        // it to its own overlay again... .)
    return;
  ...
  if (newParentButton) {
    ...
    } else if (parent) // this is a child of another button
    {
      ...
      // remove ourselves from the list that our parent keeps of its child
      // buttons

      for (buttonItr = parent->childButtons.begin();
           buttonItr != parent->childButtons.end(); ++buttonItr) {
        if ((*buttonItr) == this) {
      ...
      Ogre::LogManager::getSingleton()
          .logMessage("[buttonGUI]  ERROR!!! unknown button!  can't determine "
                      "parent!"); // we have no idea what this is!
      return;
    }
...
bool button::hitTest(int x, int y) {
  ...
  if ((pos.data.abs.left <= x) && (x <= right) && (pos.data.abs.top <= y) &&
      (y <= bottom)) {
    return 1;
  }
  return 0;
...
textInputArea::textInputArea(buttonManager *mgr, std::string &textInputName,
                             TextAreaOverlayElement::Alignment a,
                             std::string &material, buttonPosition &position,
                             button *parentButton, short width, short height,
                             short textPosX, short textPosY, textScheme &style,
                             bool isActiveButton)
    : button(textInputName, material, position, width, height, mgr, NULL,
             parentButton, style, isActiveButton) {
...
textInputArea *buttonGUI::textInputArea::setTextScheme(textScheme &scheme) {
  ...
  return this; // the purpose of this method is the same as the one for button
               // except to return as textInputArea *
}
...
///**   BUTTONMESH CLASS    **
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
buttonMesh::buttonMesh(const Ogre::String name, const Ogre::String meshFile,
                       buttonManager *buttonMgr, button *parentButton,
                       short unsigned int zOrder, short positionX,
                       short positionY, short width, short height)
    : name(name), buttonMgr(buttonMgr), parent(parentButton), relX(positionX),
      relY(positionY), zoom(1), objectAngle(0.9273f), rotatableX(true),
      rotatableY(true), mouseOn(false), momentumX(0), momentumY(0),
      friction(.035f), turnSpeedModifier(.025f) // less makes it turn slower
{
  overlay = OverlayManager::getSingleton().create(name + "_buttonMeshOverlay");
  overlay->setZOrder(zOrder);
  overlay->show();

  baseNode = new Ogre::SceneNode(
      buttonMgr->sceneMgr,
      name + "_buttonGUIBaseNode"); // because this node is going into an
                                    // overlay, should not use sceneManager to
                                    // make this instance.  See comments in
                                    // ogreoverlay.h for more info.
  e = buttonMgr->sceneMgr->createEntity(name + "_buttonGUIEntity", meshFile);
  baseNode->attachObject(e);
  overlay->add3D(baseNode);

  // replace all the materials on the mesh with instances,  so that we can fade
  // buttonMeshes independently of each other
  Ogre::MaterialManager &mgr = Ogre::MaterialManager::getSingleton();
  ...
  // size the node to fit inside a 1 unit box so we can then use that as a
  // modifier to get our target height.
...
void buttonMesh::resetDimensions(void) { setDimensions(width, height); }
buttonMesh *buttonMesh::setDimensions(short w, short h) {
  width = w;
  height = h;
  Ogre::Radian fov =
      buttonMgr->sceneMgr->getCamera(buttonMgr->camera)->getFOVy();
  const Ogre::Real percentageModifier =
      fov.valueRadians() /
      objectAngle; // this is our basepoint modifier for the object scale.
  viewPlaneLengthAtOne = tan(fov.valueRadians() / 2) *
                         2; // this is our basepoint modifier for the object
                            // position.. how long is the viewplane in y at a
                            // distance of 1, used later for object placement.

  aspectRatio = float(buttonMgr->screenResX) / float(buttonMgr->screenResY);
  screenSpaceModifier = (float(1) / float(buttonMgr->screenResY));

  Ogre::Real scale =
      screenSpaceModifier * width * percentageModifier * zoom * scaleModifier;
...
buttonMesh *buttonMesh::setZoom(Ogre::Real z) {
  zoom = abs(z); // negative values will put the object behind the camera... ie
                 // not visible!
  resetDimensions();
...
buttonMesh *buttonMesh::setPosition(short x, short y) {
  x = x + width / 2; // models use their pivot as a reference point instead of
                     // upper left corner like buttons...so we adjust here.
  y = y + height / 2;

  // include offset  (distance from center of the screen to top at z = 1  and
  // distance from center of screen to left at z = 1)
  Ogre::Real xPos = (x * screenSpaceModifier * viewPlaneLengthAtOne) -
                    (viewPlaneLengthAtOne / 2 *
                     aspectRatio); // the offset at the end puts the pivot at
                                   // the exact upper left of the screen so we
                                   // can make proper screen calculations.
  Ogre::Real yPos =
      y * screenSpaceModifier * viewPlaneLengthAtOne - viewPlaneLengthAtOne / 2;
...
void buttonMesh::update(void) {
  ...
  setPosition(p.data.abs.left + relX,
              p.data.abs.top +
                  relY); // we can't truly parent buttonMeshes to buttons.  so I
                         // update their positions each frame here

  if ((rotatableX) || (rotatableY)) {
    if (rotatableX) {
      if (momentumX) {
        baseNode->yaw(Ogre::Radian(-momentumX * turnSpeedModifier),
                      Ogre::Node::TS_WORLD);
    ...
    if (rotatableY) {
      if (momentumY) {
        baseNode->pitch(Ogre::Radian(-momentumY * turnSpeedModifier),
                        Ogre::Node::TS_WORLD);
...
void buttonMesh::injectMouseUp(short &x, short &y) { mouseOn = false; }
...
buttonManager::buttonManager(std::string defaultTextFieldM,
  ...
  resetScreenResolution(); // set our resolution so our stuff will be sized
                           // right

  Ogre::Overlay *mouseOverlay =
      OverlayManager::getSingleton().create("buttonGUI_CURSOR");
  mouseOverlay->setZOrder(650); // closest possible to camera
  mouseOverlay->show();
  // create a button that represents  the cursor.
  cursorButton = new button(std::string("CURSOR"), std::string("BLANK"),
                            buttonPosition(0, 0), 1, 1, this, mouseOverlay,
                            NULL, defaultTextScheme, false);
...
textInputArea *buttonManager::cycleTextInputArea(void) {
  ...
  for (buttonItr = tias.begin(); buttonItr != tias.end(); ++buttonItr) {
    if ((*buttonItr) == activeTextInputArea) {
      ++buttonItr;
      if (buttonItr == tias.end()) // cycle to the first one if we are at the
                                   // end of the array
...
button *buttonManager::setCursor(std::string material, unsigned short width,
  ...
  cursorButton->setPosition(
      buttonPosition(mouseX - mouseOffsetX, mouseY - mouseOffsetY));
...
bool buttonManager::injectKeyPressed(const OIS::KeyEvent &arg) {
  ...
  if (activeTextInputArea) { // inject the key into the active textArea
    if (arg.key == KC_RETURN) {
      ...
      eventStringContainer = keyCodeToString(
          arg.key, shift,
          activeTextInputArea
              ->getAlphanumericOnly()); // in theory,  one could support unicode
                                        // here...
      bool accepted;
...
button *buttonManager::getTopButton(void) {
  std::vector<button *>
      hits; // determine who is on top and run the command only on him.
  for (buttonItr = buttons.begin(); buttonItr != buttons.end(); ++buttonItr) {
  ...
  for (buttonItr = hits.begin(); buttonItr != hits.end(); ++buttonItr) {
    if ((*buttonItr)->panel->getZOrder() >= topValue) {
      if ((*buttonItr) !=
          grabbedButton) // skip the dragged button,  we already know what it is
                         // and we want to click and drop things on buttons
                         // other than him.
      {
...
bool buttonManager::injectMouseDown(OIS::MouseButtonID &id) {
  ...
  if (topButton) {
    ...
    if (topButton->setFocused(true)) // only textInputAreas can be focused, so
                                     // only they return true with setFocused()
    {
      activeTextInputArea = static_cast<textInputArea *>(
          topButton); // store this textInputArea as the focus so we can channel
                      // the inputs there
    }
    ...
    if (id == grabbingMouseButton) // should we check for movable buttons?
    {
      if (topButton->getMovable()) {
        ...
        injectMouseMove(mouseX,
                        mouseY); // force a move to the same location so that
                                 // the button will snap to the mouse
      }
    }
  }
...
bool buttonManager::injectMouseUp(OIS::MouseButtonID &id) {
  ...
  if (topButton) {
    if (topButton->onRelease()) {
      std::string message;
      // if a button is currently grabbed,  pass its name as the 'additional
      // message' because that means that that button has been drag/dropped onto
      // this one.
      if (grabbedButton)
        eventLog.push_back(buttonEvent(ONRELEASE, id, topButton,
                                       grabbedButton)); // log the event
      else
        eventLog.push_back(
            buttonEvent(ONRELEASE, id, topButton)); // log the event
    }
  }
  ...
  if (id == grabbingMouseButton) {
    if (grabbedButton) {
      ...
      if (!topButton) // send an event that the dragged button was released on
                      // nothing.
...
bool buttonManager::injectMouseMove(int xPos, int yPos) {
  ...
  if (grabbedButton) {
    if (grabbedButton
            ->centerOnGrab) // this centers the grabbed button to the mouse
    {
    ...
    } else // do a relative drag.
    {
      ...
      grabbedButton->offsetPositionToScreenCoord(
          p.data.abs.left + xPos - mouseX,
          p.data.abs.top + yPos -
              mouseY); // current pos + old mousePos - new mousePos
    }
  }

  mouseX = xPos;
  mouseY = yPos;
  cursorButton->setPosition(buttonPosition(
      xPos - mouseOffsetX,
      yPos -
          mouseOffsetY)); // update our cursor, line it up with trigger point.

  //  trigger all the mouseOvers and mouseOffs
  for (buttonItr = buttons.begin(); buttonItr != buttons.end(); ++buttonItr) {
    if ((*buttonItr)->hitTest(mouseX, mouseY)) // are we over x button?
    {
      if (!(*buttonItr)->isMouseOver) // if the mouse was previously off,
  ...
  // send the message to all buttonMeshes,  this is to allow rotation even when
  // the mouse has moved off the actual button
  for (meshItr = buttonMeshes.begin(); meshItr != buttonMeshes.end();
       ++meshItr) {
...
void buttonGUI::buttonManager::forceClickButton(
  ...
  if (b) {
    ...
    injectMouseMove(pos.data.abs.left + 1,
                    pos.data.abs.top + 1); // position mouse
...
button *buttonManager::createButton(std::string buttonName,
                                    std::string material,
                                    buttonPosition &position,
                                    unsigned short width, unsigned short height,
                                    unsigned short zOrder, bool visible,
                                    bool isActive) {
  return createButton(buttonName, material, position, width, height,
                      defaultTextStyle, zOrder, visible, isActive);
}
...
button *buttonManager::createButton(std::string buttonName,
                                    ...
                                    unsigned short width, unsigned short height,
                                    unsigned short zOrder, bool visible,
                                    bool isActive, String lua) {

  button *b = createButton(buttonName, material, position, width, height,
                           defaultTextStyle, zOrder, visible, isActive);
  b->mLua = lua;
...
button *buttonManager::createButton(std::string buttonName,
                                    ...
                                    buttonPosition &position,
                                    unsigned short width, unsigned short height,
                                    textScheme &scheme, unsigned short zOrder,
                                    bool visible, bool isActive) {
  if (!zOrder)
    zOrder = zOrderCounter++;

  Ogre::Overlay *o =
      OverlayManager::getSingleton().create(buttonName + "buttonOverlay");
  o->setZOrder(zOrder);
  o->show();

  button *b = new button(buttonName, material, position, width, height, this, o,
                         NULL, scheme, isActive);

  o->add2D(b->panel); // add the panel that the button created to the overlay.
                      // Overlays are only created for top level buttons,  the
                      // panels of child buttons are parented to panel of the
...
void buttonManager::setDefaults(std::string textFieldMaterial,
                                textScheme defaultTextScheme) {
...
buttonEvent *buttonManager::getEvent(void) {
  // basically,  this is a hopper system that you can get the events from one by
  // one
  if (eventLog.empty())
    return NULL;
  else {
...
void buttonManager::deleteButton(button *b) {
  // this function is a little dangerous... lets make sure none of the buttons
  // being deleted are being pointed to by these...
  if (activeTextInputArea) {
    activeTextInputArea->setFocused(false);
    activeTextInputArea = NULL;
  }
  ...
  if (b) // this is a recursive algorithm,  so i just want to be safe and make
         // sure b hasnt been deleted already if someone decides to call this
         // inside some kind of loop
  {
    // delete children first.
    std::vector<button *> children =
        b->childButtons; // we need to make a copy of the vector so that when we
                         // delete children so they dont muck with the list we
                         // are iterating on while we are deleting them.
    std::vector<button *>::iterator
        itr; // don't use the buttonManager button vector iterator here because
    ...
    std::vector<buttonMesh *> bmChildren =
        b->buttonMeshes; // we need to make a copy of the vector so that when we
                         // delete children so they dont muck with the list we
                         // are iterating on while we are deleting them.
    std::vector<buttonMesh *>::iterator
        childBMitr; // don't use the buttonManager buttonMesh vector iterator
                    // here because this function is recursive.
    for (childBMitr = bmChildren.begin(); childBMitr != bmChildren.end();
         ++childBMitr) {
      ...
      // then delete the actual pointer... no need to bother removing it from
      // the buttonMesh's parent's list,  because the parent button is about to
      // get deleted.
      delete (*childBMitr);
      (*childBMitr) = NULL;
    }
    ...
    if (overlay) // this is a top level button
    {
      ...
      OverlayManager::getSingleton().destroyOverlayElement(
          b->panel); // SUGGESTED BY DarkScythe
      OverlayManager::getSingleton().destroy(overlay->getName());
      overlay = NULL;
    } else if (parent) // this is a child of another button
    {
      ...
      // remove the button from the list that its parent keeps of its child
      // buttons
      for (itr = parent->childButtons.begin();
           itr != parent->childButtons.end(); ++itr) {
        if ((*itr) == b) {
...
void buttonManager::deleteAllButtons(void) {
  ...
  // cycle through the top buttons only, that way the rest will be deleted in a
  // non-conflictive order.
  for (buttonItr = topButtons.begin(); buttonItr != topButtons.end();
       ++buttonItr) {
...
void buttonManager::update(void) {
  ...
  // this handles the action of holding down backspace while a textArea is
  // active.
  if (activeTextInputArea) {
    if (backSpace) {
      if (backSpaceHeld) {
      ...
      } else if (backSpaceHeldWait <
                 (currentTime -
                  backSpaceHeldTime)) // calculate if we need to hold it.
      {
