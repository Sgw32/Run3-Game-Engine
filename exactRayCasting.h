...
int getFirstName(Ogre::Real _x, Ogre::Real _y) {
  ...
  Ogre::Ray _ray =
      mCamera->getCameraToViewportRay(x, y); ///    
    ...
    if (query_result[qr_idx].movable != NULL) {
      ...
      if (ent) {
        ...
        if (!ent->getMesh().isNull()) {
          if (ent->getMesh()->getNumSubMeshes() > 10000)
            continue;
          GetMeshInformation(ent->getMesh(), vertex_count, vertices,
                             index_count, indices,
          ...
          for (int i = 0; i < static_cast<int>(index_count); i += 3) {
            std::pair<bool, Ogre::Real> hit = Ogre::Math::intersects(
                _ray, vertices[indices[i]], vertices[indices[i + 1]],
                vertices[indices[i + 2]],
                Ogre::Math::calculateBasicFaceNormal(vertices[indices[i]],
