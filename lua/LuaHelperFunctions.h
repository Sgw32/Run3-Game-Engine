  ...
  try {
    if (int error = lua_dofile(L, script_name) != 0) {
      LogManager::getSingleton().logMessage(
          "Lua subsystem error! Fatal error. Stopping script " +
          String(script_name));
      LogManager::getSingleton().logMessage("Error " + ttos(error));
      // throw std::runtime_error("ERROR(" + ttos(error) + "): Problem with lua
      // script file " + script_name);
    }
  } catch (...) {
    LogManager::getSingleton().logMessage(
        "Unknown lua script execution error.");
  }
}
...
//-----------------------------------------------------------------------------
template <class T> inline T PopLuaNumber(lua_State *pL, const char *name) {
  lua_settop(pL, 0);
