...
#ifndef __NPC_AERIAL_RUN3_H__
#define __NPC_AERIAL_RUN3_H__

#define __NPC_AERIAL_VELOCITY__ 5
/*namespace Ogre
{
class Node
{
...
public:
  ...
  // find path
  bool find_path(); // return true if it isn't a straight path between player
                    // and npc, and false if it is.
  // find path
  bool
  find_path(Vector3 destPos); // return true if it isn't a straight path between
                              // player and npc, and false if it is.
  // is NPC is killed
  void kill();
  // HEADSHOT!!!
  void headshot();
  // classic NPC behavior
  void behavior0(const Ogre::FrameEvent &evt);
  // AEROXO data
  void behavior1(const Ogre::FrameEvent &evt);
  // set newt material
  ...
  void setDirection(Node *t, const Vector3 &vec,
  ...
  void lookAt(Node *t, const Vector3 &targetPoint,
              Node::TransformSpace relativeTo,
              const Vector3 &localDirectionVector) {
