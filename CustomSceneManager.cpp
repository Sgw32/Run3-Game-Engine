...
const Ogre::String CustomSceneManagerFactory::FACTORY_TYPE_NAME =
    "CustomSceneManager";

Ogre::SceneManager *
CustomSceneManagerFactory::createInstance(const Ogre::String &instanceName) {
...
void CustomSceneManager::renderSingleObject(Renderable *rend, const Pass *pass,
  ...
  if (g_InspectRenderable == rend) {
    hardwareOcclusionQuery = Ogre::Root::getSingleton()
                                 .getRenderSystem()
  ...
  if (numMatrices > 0) {
    ...
    if (mCameraRelativeRendering && !rend->getUseIdentityView()) {
      for (unsigned short i = 0; i < numMatrices; ++i) {
        mTempXform[i].setTrans(mTempXform[i].getTrans() -
  ...
  if (!mSuppressRenderStateChanges) {
    ...
    if (pass->isProgrammable()) {
      ...
      // Tell auto params object about the world matrices, eliminated query from
      // renderable again
      ...
      if (pass->hasVertexProgram()) {
        passSurfaceAndLightParams =
            pass->getVertexProgram()->getPassSurfaceAndLightStates();
      }
    }

    // Reissue any texture gen settings which are dependent on view matrix
    Pass::ConstTextureUnitStateIterator texIter =
    ...
    if (mFlipCullingOnNegativeScale) {
      ...
      // this also copes with returning from negative scale in previous render
      // op for same pass
    ...
    if (pass->getPolygonModeOverrideable() &&
        rend->getPolygonModeOverrideable()) {
    ...
    if (doLightIteration) {
      ...
      if (iteratePerLight) {
        lightsLeft =
            static_cast<int>(rendLightList.size()) - pass->getStartLight();
        // Don't allow total light count for all iterations to exceed max per
        // pass
      ...
      while (lightsLeft > 0) {
        ...
        if (iteratePerLight) {
            ...
            *destit++ = currLight;
            ...
            if (isShadowTechniqueTextureBased()) {
              size_t textureCountPerLight =
                  mShadowTextureCountPerType[currLight->getType()];
              for (size_t j = 0; j < textureCountPerLight &&
                                 shadowTexIndex < mShadowTextures.size();
                   ++j) {
          ...
          // Did we run out of lights before slots? e.g. 5 lights, 2 per
          // iteration
        ...
        } else // !iterate per light
        {
          ...
          if (pass->getStartLight() || pass->getMaxSimultaneousLights() !=
            ...
            // out of lights?
            // skip manual 2nd lighting passes onwards if we run out of lights,
            ...
            if (pass->getStartLight() > 0 &&
              ...
              localLightList.insert(localLightList.begin(), copyStart, copyEnd);
              pLightListToUse = &localLightList;
            }
          } else {
        ...
        if (pass->isProgrammable()) {
          ...
          if (pass->hasVertexProgram()) {
            mDestRenderSystem->bindGpuProgramParameters(
                GPT_VERTEX_PROGRAM, pass->getVertexProgramParameters());
          }
          if (pass->hasGeometryProgram()) {
            mDestRenderSystem->bindGpuProgramParameters(
                GPT_GEOMETRY_PROGRAM, pass->getGeometryProgramParameters());
          }
          if (pass->hasFragmentProgram()) {
            mDestRenderSystem->bindGpuProgramParameters(
                GPT_FRAGMENT_PROGRAM, pass->getFragmentProgramParameters());
          }
        }
        // Do we need to update light states?
        // Only do this if fixed-function vertex lighting applies
        if (pass->getLightingEnabled() && passSurfaceAndLightParams) {
          mDestRenderSystem->_useLights(*pLightListToUse,
        ...
        mDestRenderSystem->setCurrentPassIterationCount(
        ...
        if (pass->getIterationDepthBias() != 0.0f) {
          ...
          mDestRenderSystem->setDeriveDepthBias(true, depthBiasBase,
                                                pass->getIterationDepthBias(),
                                                pass->getDepthBiasSlopeScale());
        } else {
    ...
    } else // no automatic light processing
    {
      ...
      if (!skipBecauseOfLightType) {
        ...
        // Use manual lights if present, and not using vertex programs that
        // don't use fixed pipeline
        if (manualLightList && pass->getLightingEnabled() &&
            passSurfaceAndLightParams) {
          mDestRenderSystem->_useLights(*manualLightList,
                                        pass->getMaxSimultaneousLights());
        }
  ...
  if (g_InspectRenderable == rend) {
    ...
    ///
    Ogre::Root::getSingleton().getRenderSystem()->destroyHardwareOcclusionQuery(
...
RenderableOcclusionPair::RenderableOcclusionPair(Ogre::Renderable *_renderable)
    : m_Renderable(_renderable), m_PixelCount(0),
      m_PixelState(RenderableOcclusionPair::READY_FOR_RENDER) {
  ///
  /// CREATE HardwareOcclusionQuery
  ///
  m_HardwareOcclusionQuery = Ogre::Root::getSingleton()
                                 .getRenderSystem()
                                 ->createHardwareOcclusionQuery();
