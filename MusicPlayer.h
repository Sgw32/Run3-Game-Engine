...
public:
  ...
  void setFadeOutLevel(Real fadel) { fade_level = fadel; }
  void toggleMusic(String filename, bool loop);
  void setVolume(Real volume) { stream->setVolume(volume); }
  ...
  void activatePReg() { preg = true; }
  void deactivatePReg() { preg = false; }
  void modulatePRegToValue(Real volume) { modVol = volume; }
  void cleanup();
