...
public:
  ...
  void activateElisa() {
    ...
    LogManager::getSingleton().logMessage(
        "Chatterbot Eliza v2.0 Copyright(c) 2005 - 2006 Gonzales Cenelia");
    elisa_chat = true;
  ...
  void setAllowVirtualDisplay(bool allow) { allowVirtualDisplay = allow; }
  void clrScr() { lines.clear(); }
  void setColor(ColourValue c) { textbox->setColour(c); };
  ...
  static int setDimensions(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
        lua_isstring(pL, 4)) {
      DisplayLuaCallback::getSingleton().setDimensions(
          StringConverter::parseReal(lua_tostring(pL, 1)),
  ...
  static int writeLn(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1))
      DisplayLuaCallback::getSingleton().setWriteLnText(
          lua_tostring(pL, 1)); // toWrite=
  ...
  static int setTextColor(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
        lua_isstring(pL, 4)) {
      ColourValue c(StringConverter::parseReal(lua_tostring(pL, 1)),
                    StringConverter::parseReal(lua_tostring(pL, 2)),
                    StringConverter::parseReal(lua_tostring(pL, 3)),
                    StringConverter::parseReal(lua_tostring(pL, 4)));
  ...
  void activateEasyExit(bool ac) { mEasyExit = ac; }
