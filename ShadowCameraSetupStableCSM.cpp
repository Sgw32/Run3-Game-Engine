...
void StableCSMShadowCameraSetup::calculateSplitPoints(size_t cascadeCount,
  ...
  if (cascadeCount < 2)
    OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS,
                "Cannot specify less than 2 cascades",
  ...
  for (size_t i = 1; i <= mCascadeCount; i++) {
    ...
    Real logDist =
        firstSplitDist * Math::Pow(farDist / firstSplitDist, fraction);
...
void StableCSMShadowCameraSetup::getShadowCamera(const Ogre::SceneManager *sm,
  ...
  // Add a padding factor to internal distances so that the connecting split
  // point will not have bad artifacts.
  if (iteration > 0) {
  ...
  getShadowCameraForCascade(sm, cam, vp, light, texCam, iteration, nearDist,
                            farDist);
}

void StableCSMShadowCameraSetup::getShadowCameraForCascade(
    const SceneManager *sm, const Camera *cam, const Viewport *vp,
    const Light *light, Camera *texCam, size_t iteration, Ogre::Real nearSplit,
  ...
  if (light->getType() == Light::LT_DIRECTIONAL) {
    ...
    float farClip =
        light->getShadowFarDistance(); // light->getShadowFarClipDistance();
    if (farClip <= 0.0f)
    ...
    // We do this in local space so that we don't have slight precision
    ...
    float dist =
        (topLeftFar.squaredLength() - bottomRightNear.squaredLength()) /
    ...
    Vector3 localPos(
        0, 0, -dist); // we have now found the point along frustum center which
    ...
    diameter *= (Real)1.01; // allow some boundary pixels for filtering etc.
                            // TODO - make this a user constant
    pos += dir * 0.5 *
           (farClip - nearClip); // pull light back so we can see the scene
    ...
    Real worldTexelSize = (texCam->getOrthoWindowWidth()) /
                          texCam->getViewport()->getActualWidth();
