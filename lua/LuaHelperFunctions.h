...
inline void RunLuaScript(lua_State* L, const char* script_name)
{
	try
	{
		...
		{
			LogManager::getSingleton().logMessage("Lua subsystem error! Fatal error. Stopping script "+String(script_name));
	...
	{
		LogManager::getSingleton().logMessage("Unknown lua script execution error.");
	}
}
