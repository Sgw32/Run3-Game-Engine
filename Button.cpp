...
void Run3::Button::initMovable(SceneNode *node, int type, Real mass) {
  ...
  OgreNewt::Collision *col = new OgreNewt::CollisionPrimitives::Box(
      mWorld, size, Quaternion::IDENTITY, center);
  bod = new OgreNewt::Body(mWorld, col);
  delete col;
  // Make it movable!
  // mass;
  ...
  bod->setName(node->getName());
  bod->setPositionOrientation(node->getPosition(), node->getOrientation());
  bod->setCustomForceAndTorqueCallback<Run3::Button>(
      &Run3::Button::camera_force_callback, this);
  bod->setAutoFreeze(0);
  // delete col;
}
...
bool Run3::Button::frameStarted(const Ogre::FrameEvent &evt) {
  if (showFrame) {
    if (p)
      bod->setPositionOrientation(mparent->getPosition() + d_b,
...
void Run3::Button::setCallback(String luaScript) { mLuaScript = luaScript; }

void Run3::Button::use() { RunLuaScript(pLuaState, mLuaScript.c_str()); }
