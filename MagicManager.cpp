...
void MagicManager::init(Camera *mCamera, RenderWindow *win, String path) {
  ...
  lua_register(pLua, "buttonGUI_create3DButtonQuat",
               buttonGUI_create3DButtonQuat);
  ...
  lua_register(pLua, "buttonGUI_activateCenter640",
               buttonGUI_activateCenter640);
  lua_register(pLua, "buttonGUI_deactivateCenter640",
               buttonGUI_deactivateCenter640);
  lua_register(pLua, "buttonGUI_activateTopLeft640",
               buttonGUI_activateTopLeft640);
  lua_register(pLua, "buttonGUI_activateTopLeftComp640",
  ...
  buttonGUI::textScheme vip("vipond", 20, 0, 1, 0,
                            1); // here, i declare a simple textscheme to start
                                // with.  (just make it green all the time)

  buttonGUI::textScheme t(
      "neuropol", 22, 1.0f, 1.0f, 1.0f,
      1.0f); // for this textscheme i will be a little more specific and specify
             // different colors for some of the events and gradients for each.
  ...
  buttonMgr = new buttonGUI::buttonManager(
      "darkInput", t, mSceneMgr, global::getSingleton().getCamera()->getName());
  ...
  buttonGUI::button* btn = buttonMgr->createButton("star1", "BLANK",
  buttonGUI::buttonPosition(buttonGUI::CENTER, -128, -128),
  150,150,2,true,true,""); btn->setMovable(true);
          btn->addButtonMesh("star1mesh", "ninjaStar.mesh", -25,-25,
  200,200)->setZoom(3);*/
           ...
           if(RenderName == "Direct3D9 Rendering Subsystem")
            {
               ...
               if(DX9Device != 0)
                {
                     Ogre::LogManager::getSingleton().logMessage("MagicDXManager:
     ok , getting D3DDEVICE\n");
                }
...
if (!act)
{
        LogManager::getSingleton().logMessage("MagicManager: removing Render
Queue");
...
if (act && !mQueue)
{
        LogManager::getSingleton().logMessage("MagicManager: creating Render
Queue"); mQueue = new MagicRenderQueue(mWindow,mPath);
mSceneMgr->addRenderQueueListener(mQueue);
}*/
  /*if (act)
  {
          LogManager::getSingleton().logMessage("buttonGUIManager: invalid
  request"); return;
  }*/
  if (!act) {
    LogManager::getSingleton().logMessage(
        "buttonGUIManager: removing Render Queue");
    ...
    // CompositorManager::getSingleton().setCompositorEnabled(global::getSingleton().getCamera()->getViewport(),
    // "Glow", glowWasEnabled);
  }
  if (act) {
    LogManager::getSingleton().logMessage(
        "buttonGUIManager: creating Render Queue1");
    buttonMgr->deleteAllButtons();
    buttonMgr->showCursor();
    // CompositorManager::getSingleton().setCompositorEnabled(global::getSingleton().getCamera()->getViewport(),
    // "Glow", false); glowWasEnabled =
    // CompositorManager::getSingleton().getCompositorEnabled(mCamera->getViewport(),
    // "Glow"); create red button in the middle
    // buttonMgr->createButton("centerButton", "middleButtonMat",
    // buttonGUI::buttonPosition(450,384), 128,128);		//notice that
    // all the material variants are automatically detected and used
  }
}
...
void MagicManager::activateMagic(bool act, String lua) {
  ...
  if (act) {
    LogManager::getSingleton().logMessage(
        "buttonGUIManager: creating Render Queue2");
    buttonMgr->showCursor();
    RunLuaScript(pLua, lua.c_str());
    // CompositorManager::getSingleton().setCompositorEnabled(global::getSingleton().getCamera()->getViewport(),
    // "Glow", false); create red button in the middle
    // buttonMgr->createButton("centerButton", "middleButtonMat",
    // buttonGUI::buttonPosition(450,384), 128,128);		//notice that
    // all the material variants are automatically detected and used
  }
...
void MagicManager::upd(const FrameEvent &evt) {
  ...
  buttonGUI::buttonEvent *e =
      buttonMgr->getEvent(); // THE FOLLOWING LOOP IS HOW TO GET EVENTS FROM
                             // buttonGUI
  while (e) {
    handleButtonEvent(e);
