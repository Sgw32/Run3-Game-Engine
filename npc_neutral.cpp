...
void npc_neutral::init(NPCSpawnProps props) {
  // LogManager::getSingleton().logMessage("Npc manager: npc neutral
  // initializing");
  mProps = props;
  ...
  // LogManager::getSingleton().logMessage("Npc manager: npc neutral ended
  // initializing"); mDebugPathNode =
  // mSceneMgr->getRootSceneNode()->createChildSceneNode("npc_neutral_path_"+props.mName);
  // mDebugLines = new Ogre::ManualObject("npc_neutral_path_"+props.mName);
  // mRoot->addFrameListener(this);
  health = props.health;
}

void npc_neutral::setPlayerPosition(Vector3 pos) { pPosit = pos; }

void npc_neutral::headshot() {
  Run3SoundRuntime::getSingleton().emitSound("run3/sounds/bodysplat_1.wav", 2,
                                             false, npcBody->damagePosition,
                                             200, 50, 800);
...
inline void npc_neutral::processHealthLoss(OgreNewt::Body *me) {
  ...
  if (((me->damagePosition - getpos(me)).y > mProps.headshotDist) &&
      (mProps.headshot)) {
    BloodEmitter::getSingleton().emitBlood(me->damagePosition,
                                           Vector3(0.7, 0.7, 0.7));
    GibManager::getSingleton().spawnGib(mProps.headMesh, me->damagePosition,
                                        mHeliNode->getScale(), 4.0f);
    headshot();
  }
  BloodEmitter::getSingleton().emitBlood(me->damagePosition,
                                         Vector3(0.3, 0.3, 0.3));
  if (mProps.exploding) {
    ExplosionManager::getSingleton().spawnExplosionNosound(me->damagePosition,
                                                           Vector3(2, 2, 2));
    Run3SoundRuntime::getSingleton().emitSound("run3/sounds/explode_3.wav", 4,
                                               false, me->damagePosition, 200,
                                               50, 800);
    kill();
...
void npc_neutral::npc_force_callback(OgreNewt::Body *me) {
  ...
  if (parentRelation) {
    me->setPositionOrientation(
        mPar->_getDerivedPosition() +
            (mPar->_getDerivedOrientation() * pRelQuat2) * pRelPosition,
        mPar->_getDerivedOrientation() * pRelQuat); // getorient(npcBody));
  } else {
    // Main behaviour
    Ogre::Real mass;
    ...
    Ogre::Vector3 force2(steps.x * mVel + stdAdd.x * 5 * mVel, 0,
                         steps.z * mVel + stdAdd.z * 5 * mVel);
    Ogre::Vector3 force;
...
void npc_neutral::spawnNPC() {
  ...
  // LogManager::getSingleton().logMessage(StringConverter::toString(npc_scale)+"
  // "+StringConverter::toString(size));
  goal_achieved = true;
  // Vector3 size=physSize;
  OgreNewt::Collision *col =
      new OgreNewt::CollisionPrimitives::Box(mWorld, size * physSize);
  ...
  npcBody->setPositionOrientation(mHeliNode->getPosition(),
                                  mHeliNode->getOrientation());
  ...
  npcBody->setCustomForceAndTorqueCallback<npc_neutral>(
      &npc_neutral::npc_force_callback, this);
  npcBody->setAutoFreeze(0);
  OgreNewt::BasicJoints::UpVector *uv1 =
      new OgreNewt::BasicJoints::UpVector(mWorld, npcBody, Vector3::UNIT_Y);
  OgreNewt::BasicJoints::UpVector *uv2 =
      new OgreNewt::BasicJoints::UpVector(mWorld, npcBody, Vector3::UNIT_Z);
  OgreNewt::BasicJoints::UpVector *uv3 =
      new OgreNewt::BasicJoints::UpVector(mWorld, npcBody, Vector3::UNIT_X);
...
void npc_neutral::processEvent(int flag, String param1, String param2) {
  ...
  if (flag == TOGGLE_FLASHLIGHT) {
    ...
    if (flashLight &&
        (mSceneMgr->hasLight("flashLighter" + niNode->getName()))) {
      ...
      flashLight->setAttenuation(
          3000, 1, 1, 1); // meter range.  the others our shader ignores
  ...
  if (flag == KILL_NPC) {
    ...
    if (((npcBody->damagePosition - getpos(npcBody)).y > mProps.headshotDist) &&
        (mProps.headshot)) {
      BloodEmitter::getSingleton().emitBlood(npcBody->damagePosition,
                                             Vector3(0.7, 0.7, 0.7));
      GibManager::getSingleton().spawnGib(mProps.headMesh,
                                          npcBody->damagePosition,
                                          mHeliNode->getScale(), 4.0f);
      headshot();
    }
    BloodEmitter::getSingleton().emitBlood(npcBody->damagePosition,
                                           Vector3(0.3, 0.3, 0.3));
    if (mProps.exploding) {
      ExplosionManager::getSingleton().spawnExplosionNosound(
          npcBody->damagePosition, Vector3(2, 2, 2));
      Run3SoundRuntime::getSingleton().emitSound("run3/sounds/explode_3.wav", 4,
                                                 false, npcBody->damagePosition,
                                                 200, 50, 800);
  ...
  if (flag == ROTATE_OVERRIDE_NPC) {
    npcBody->setPositionOrientation(getpos(npcBody),
                                    StringConverter::parseQuaternion(param1));
  }
  if (flag == TELEPORT_PARENT_NPC) {
    // mPar=pPar;
    if (mPar) {
      // It just adds to mPar position, so no more components
      pRelPosition = mPar->getOrientation() *
                     StringConverter::parseVector3(param1) * mPar->getScale();
...
inline void npc_neutral::processLook(Real mulTime) {
  if (mProps.strange_look) {
    ...
    Vector3 ppsit =
        global::getSingleton().getPlayer()->get_location() * (1 - getCycle8()) +
        getCycle8() * (getpos(npcBody) - Vector3(dir1.x, 0, -dir1.z));
    Vector3 eyeDir =
        Vector3(getpos(npcBody).x - ppsit.x, 0, getpos(npcBody).z - ppsit.z);
    // LogManager::getSingleton().logMessage(StringConverter::toString(getCycle8()));
    Quaternion newRot =
        eyeDir.getRotationTo(Vector3(dir1.x, 0, -dir1.z), mProps.headAxis);
    newRot = newRot.Inverse();
    Real yawAngle;
    yawAngle = newRot.getYaw().valueDegrees();
    if ((fabs(newRot.getYaw().valueDegrees())) > 45.0f)
      yawAngle *= Ogre::Math::Exp(-Ogre::Math::Sqr(
          fabs(newRot.getYaw().valueDegrees()) / 45.0f - 1.0f));
    ...
    m->setOrientation(getorient(npcBody) *
                      Quaternion(Radian(Degree(goalHeadYaw)), mProps.headAxis));
  }
}
...
void npc_neutral::step(const Ogre::FrameEvent &evt) {
  ...
  Real mulTime =
      evt.timeSinceLastFrame * TIME_SHIFT * moveActivity * uniqueMult;
  ...
  if (animated) {
    // #ifdef DEBUG_FACIAL
    // LogManager::getSingleton().logMessage("Animation
    // add."+mAnimState->getAnimationName()+"fsd"+StringConverter::toString(mAnimState->getEnabled()));
    // #endif
  ...
  if (going) {
    ...
    if (fstTimer > 0.5f) {
      ...
      if (mProps.sounds) {
        ...
        Run3SoundRuntime::getSingleton().emitSound(fname, 1.0f, false, myPos,
                                                   200, 50, 800);
      }
    ...
    if (stopAtDist) {
      ...
      LogManager::getSingleton().logMessage(
          "Distance to destination: " + StringConverter::toString(pPosit) +
          " is " + StringConverter::toString(getpos(npcBody).distance(pPosit)));
#endif
...
#ifdef DEBUG_TARGET_BLIZKO
      LogManager::getSingleton().logMessage(
          "NPC is near target position. Exec script.");
#endif
      if (!(luaToExec.empty()))
        RunLuaScript(global::getSingleton().getLuaState(), luaToExec.c_str());
    }

    if (farFindB && goal_achieved) // We continue pathfinding because it
                                   // finished the previous path.
    {
      path_find = find_path();
      i = 0;
      if (path_find)
      ...
      LogManager::getSingleton().logMessage(
          StringConverter::toString(farFindB) + " " +
...
#ifdef DEBUG_PATHFINDING
    LogManager::getSingleton().logMessage("Check straight returned " +
                                          StringConverter::toString(stra));
#endif
    ...
    if ((farFindB && path_find && !stra) ||
        (cmdMode && farFindB && path_find && !stra)) {
#ifdef DEBUG_PATHFINDING
      ...
      if (!dyeNow) {
...
#ifdef DEBUG_PATHFINDING
        LogManager::getSingleton().logMessage("GOAL:" +
                                              StringConverter::toString(dest));
#endif
        bool in_rad_precision =
            (pow((dest.x - myPos.x), 2) + pow((dest.z - myPos.z), 2)) <= 50;
        ...
        if (in_rad_precision) {
          ...
          if ((i) == mPath.size()) {
            ...
            // LogManager::getSingleton().logMessage(StringConverter::toString(dir1)+"
            // "+StringConverter::toString(dir)+"
            // "+StringConverter::toString(angle));
        ...
        LogManager::getSingleton().logMessage(
            StringConverter::toString(rotation_needed));
#endif
        if (rotation_needed) {
          if (animated) {
            mAnimState->setEnabled(false);
          ...
          if (rot_dir) {
            ...
            niNode->rotate(
                Vector3::UNIT_Y,
                Degree(10.0f * evt.timeSinceLastFrame * TIME_SHIFT * 5));
          } else {
            // LogManager::getSingleton().logMessage(StringConverter::toString(angle)+"-");
            niNode->rotate(
                Vector3::UNIT_Y,
    ...
    if (!path_find ||
        (!goal_achieved &&
         stra)) // Either path is found, and NPC is going on it, or goal is not
                // achieved, and it is a straight path.
    {
#ifdef DEBUG_PATHFINDING
      ...
      if (!dyeNow) {
        if (!farFindB) {
          ...
          if (!notsetIdle) {
            if (animated) {
              //	mAnimState->setEnabled(false);
              //		mAnimState =
              //heliEnt->getAnimationState("Idle1");
              //	mAnimState->setEnabled(true);
            }
          }
        }
        ...
        if (mProps.sounds) {
          if ((rand() % 20) == 4)
            Run3SoundRuntime::getSingleton().emitSound(
                mProps.soundAttack, 3, false, myPos, 200, 50, 800);

          if ((rand() % 5) == 4)
            Run3SoundRuntime::getSingleton().emitSound(
                mProps.soundHit, 3, false, myPos, 200, 50, 800);
        }
        ...
        if (previous_player) {
          ...
          if (mProps.sounds)
            Run3SoundRuntime::getSingleton().emitSound(
                mProps.soundFind, 6, false, myPos, 200, 50, 800);
          previous_player = false;
        }
        ...
        if (farFindB) // NPC found player in near place
        {
          ...
          if (dir.length() < attackAnimDist) {
            ...
            rotation_needed =
                Degree(fabs(angle.valueDegrees())) >= Degree(5.0f);
        ...
        if ((rand() % 3)==2)
            ...
            if (!rotation_needed) {
              ...
              if (rot_dir) {
                ...
                niNode->rotate(
                    Vector3::UNIT_Y,
                    Degree(rotateSpeed * evt.timeSinceLastFrame * TIME_SHIFT));
              } else {
                ////LogManager::getSingleton().logMessage(StringConverter::toString(angle)+"-");
                niNode->rotate(
                    Vector3::UNIT_Y,
...
void npc_neutral::kill() {
  ...
  if (!mProps.luaOnDeath.empty())
    RunLuaScript(global::getSingleton().getLuaState(),
                 mProps.luaOnDeath.c_str());
...
/*#define MAKE_SOME_GORE(s)
anim->destroyNodeTrack(skel->getBone(s)->getHandle()); \
  ...
  heliEnt->getSkeleton()->getBone("BMan0002-R
  Forearm")->setScale(0.01,0.01,0.01);*/
