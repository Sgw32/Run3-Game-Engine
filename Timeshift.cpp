...
void Timeshift::toggleTStop() {
  if (mK != 0) {
    ...
    nod->setOrientation(
        global::getSingleton().getPlayer()->get_body_orientation(
            global::getSingleton().getPlayer()->bod));
...
void Timeshift::init() {
  nod = global::getSingleton()
            .getSceneManager()
            ->getRootSceneNode()
            ->createChildSceneNode();
  ent = global::getSingleton().getSceneManager()->createEntity(
      "playerent_RUN3_RES", "marazm01.mesh");
