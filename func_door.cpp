...
void func_door::setup(Ogre::SceneNode *door_node, bool box) {
  mNode = door_node;
  aab = AxisAlignedBox(mNode->getPosition() - (mNode->getScale()),
                       mNode->getPosition() + (mNode->getScale()));
  OgreNewt::Collision *col;
  if (!box) {
    col = new OgreNewt::CollisionPrimitives::TreeCollision(
        global::getSingleton().getWorld(), mNode, true);
  } else {
    AxisAlignedBox aab =
        ((Entity *)mNode->getAttachedObject(0))->getBoundingBox();
    ...
    col = new OgreNewt::CollisionPrimitives::Box(
        global::getSingleton().getWorld(), size, Quaternion::IDENTITY, center);
  }

  door_bod = new OgreNewt::Body(global::getSingleton().getWorld(), col);
  door_bod->attachToNode(mNode);
  door_bod->setPositionOrientation(mNode->getPosition(),
...
void func_door::setup(Ogre::SceneNode *door_node) {
  ...
  aab = AxisAlignedBox(mNode->getPosition() - (mNode->getScale()),
                       mNode->getPosition() + (mNode->getScale()));
  OgreNewt::Collision *col = new OgreNewt::CollisionPrimitives::TreeCollision(
      global::getSingleton().getWorld(), mNode, true);
  door_bod = new OgreNewt::Body(global::getSingleton().getWorld(), col);
  door_bod->attachToNode(mNode);
  door_bod->setPositionOrientation(mNode->getPosition(),
...
void func_door::setDistanceDirection(Ogre::Real distance, Ogre::Vector3 dir,
                                     Ogre::Real speed) {
  dist = distance;
  direction = dir;
  mspeed = speed;
...
void func_door::Fire(String event) {
  ...
  if ((!nowfire) && (!funcrot)) {
    ...
    if (event == "toggle") {
      ...
      LogManager::getSingleton().logMessage("Opened:" +
                                            StringConverter::toString(opened));
      LogManager::getSingleton().logMessage("Closed:" +
                                            StringConverter::toString(closed));
    ...
    if (cur_event == "close" && scl && opened) {
      ...
      if (mOnClose != "none") {
        ...
        sMgr->setSound(sound, fPosition, Vector3::ZERO, Vector3::ZERO, 1000,
                       false, false, 200, 50, 800);
...
void func_door::setname(String name) { mName = name; }

String func_door::getname() { return mName; }
...
OgreNewt::Body *func_door::getBodyOnTop() {
  ...
  Ogre::AxisAlignedBox plyaab = Ogre::AxisAlignedBox(
      pos - size - Vector3(100, 100, 100), pos + size + Vector3(100, 100, 100));
...
bool func_door::frameStarted(const Ogre::FrameEvent &evt) {
  ...
  if (nowfire) {
    ...
    if (funcrot) {
      if (rotDoor) {
        Real mpt1 = -rotSpeed * evt.timeSinceLastFrame * 5 *
                    Ogre::Math::Sign(mp1) * TIME_SHIFT;
        Real myt1 = -rotSpeed * evt.timeSinceLastFrame * 5 * TIME_SHIFT *
                    Ogre::Math::Sign(my1);
        Real mrt1 = -rotSpeed * evt.timeSinceLastFrame * 5 * TIME_SHIFT *
                    Ogre::Math::Sign(mr1);
        ...
        set_position(get_position() -
                     vDirection * evt.timeSinceLastFrame * TIME_SHIFT / 0.2f);
      }
    }
      ...
      if (!opened) {
        ...
        if (funcrot) {
          if (rotDoor) {
            ...
            set_position(get_position() + vDirection * evt.timeSinceLastFrame *
                                              TIME_SHIFT / 0.2f);
          }
        } else {
          if (rotDoor) {
            ...
            if (allowPReg) {
              // P-regulator code.
              // Pitch channel
              // LogManager::getSingleton().logMessage("Opening rotdoor using
              // P-reg, ");
              Real mpt1 = 0;
              if (mp)
                mpt1 = fabs((mp1) / mp) * rotSpeed * evt.timeSinceLastFrame *
                       TIME_SHIFT * 5 * Ogre::Math::Sign(mp); // Delta pitch
              Real myt1 = 0;
              if (my)
                myt1 = fabs((my1) / my) * rotSpeed * evt.timeSinceLastFrame *
                       TIME_SHIFT * 5 * Ogre::Math::Sign(my); // Delta yaw
              Real mrt1 = 0;
              if (mr)
                mrt1 = fabs((mr1) / mr) * rotSpeed * evt.timeSinceLastFrame *
                       TIME_SHIFT * 5 * Ogre::Math::Sign(mr); // Delta roll
              mp1 -= mpt1;                                    // mp1 - Real
              my1 -= myt1;                                    // mp1 - Real
              mr1 -= mrt1;                                    // mp1 - Real
              ...
              if ((fabs(mpt1) < DEGREE_DISCRAPENCY / 1000) &&
                  (fabs(myt1) < DEGREE_DISCRAPENCY / 1000) &&
                  (fabs(mrt1) < DEGREE_DISCRAPENCY / 1000)) {
                mp1 = 0;
                my1 = 0;
                mr1 = 0;
                nowfire = false;
          ...
          if (!rotDoor) {
            if (getBodyOnTop())
              global::getSingleton().getPlayer()->set_additional_force(
                  vDirection);
            if (allowPReg) {
              ...
              set_position(myPosXYZ + error * velocity / 100 *
              ...
              if (!x && !y && !z) {
                set_position(get_position() + vDirection *
        ...
        if (funcrot) {
          ...
          if (rotDoor) {
            ...
            if (allowPReg) {
              ...
              Real mpt1 = 0;
              if (mp)
                mpt1 = -fabs((mp - mp1) / mp) * rotSpeed *
                       evt.timeSinceLastFrame * TIME_SHIFT * 5 *
                       Ogre::Math::Sign(mp); // Delta pitch
              Real myt1 = 0;
              if (my)
                myt1 = -fabs((my - my1) / my) * rotSpeed *
                       evt.timeSinceLastFrame * TIME_SHIFT * 5 *
                       Ogre::Math::Sign(my); // Delta yaw
              Real mrt1 = 0;
              if (mr)
                mrt1 = -fabs((mr - mr1) / mr) * rotSpeed *
                       evt.timeSinceLastFrame * TIME_SHIFT * 5 *
                       Ogre::Math::Sign(mr); // Delta roll
              mp1 -= mpt1;                   // mp1 - Real
              my1 -= myt1;                   // mp1 - Real
              mr1 -= mrt1;                   // mp1 - Real
              ...
              if (!PRIMERNO_RAVNO(mp1, mp, DEGREE_DISCRAPENCY)) {
                Real mpt1 = -rotSpeed * evt.timeSinceLastFrame * TIME_SHIFT *
                            5 * Ogre::Math::Sign(mp);
              ...
              if (!PRIMERNO_RAVNO(my1, my, DEGREE_DISCRAPENCY)) {
                Real myt1 = -rotSpeed * evt.timeSinceLastFrame * TIME_SHIFT *
                            5 * Ogre::Math::Sign(my);
              ...
              if (!PRIMERNO_RAVNO(mr1, mr, DEGREE_DISCRAPENCY)) {
                Real mrt1 = -rotSpeed * evt.timeSinceLastFrame * TIME_SHIFT *
                            5 * Ogre::Math::Sign(mr);
                LogManager::getSingleton().logMessage(
                    StringConverter::toString(mr1));
                LogManager::getSingleton().logMessage(
                    StringConverter::toString(mr));
