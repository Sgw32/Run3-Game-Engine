///////////////////////////////////////////////////////////////////////
//		Dark zones darken the ambient light using a distrubution
//function// 		by Sgw32
...
#define EXPONENT_CONSTANT_DARKEN                                               \
  0.1 // for 2d,
      // f(x,y)=1-darken*exp(-(((x-x0)/h)^2+((y-y0)/w)^2)/EXPONENT_CONSTANT_DARKEN)

class DarkZone {
public:
  DarkZone(SceneManager *sceneMgr, Real x0, Real y0, Real z0, Real height,
           Real width, Real depth, Real darken, Real exp) {
    mSceneMgr = sceneMgr;
  ...
  DarkZone(SceneManager *sceneMgr, Vector3 pos, Vector3 scale, Real darken,
           Real exp) {
    mSceneMgr = sceneMgr;
    // ColourValue mAmbiLight = mSceneMgr->getAmbientLight();
    mX0 = pos.x;
  ...
  Real computeDarken(Ogre::Vector3 pos) {
    return 1 - mDarken *
                   exp(-(((pos.x - mX0) / mHeight) * ((pos.x - mX0) / mHeight) +
                         ((pos.y - mY0) / mWidth) * ((pos.y - mY0) / mWidth) +
                         ((pos.z - mZ0) / mDepth) * ((pos.z - mZ0) / mDepth)) /
                       mExp);
  }
  ...
  void addLight(Light *l) { lights[l] = l->getDiffuseColour(); }
