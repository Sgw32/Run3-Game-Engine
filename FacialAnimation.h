...
public:
  ...
  void upd(Real tm);
  void setSubtitle(String subt) { subtitle = subt; }
  void destroy() {}
    ...
    for (int curSubMesh = 1; curSubMesh <= numSubMeshes; curSubMesh++) {
      try {
        if (mPatched) {
          ...
          vertexPoseKeyFrameMap[curSubMesh]->updatePoseReference(
              curPoseIndex * numSubMeshes + curSubMesh - 1, weight);
          // Dirty animation state since we're fudging this manually
          // If we don't notify dirty, Ogre doesn't know to update this state
          // and nothing happens
          head->getAnimationState("manl" + head->getName())
              ->getParent()
              ->_notifyDirty();
        } else {
          // LogManager::getSingleton().logMessage("INDEX"+StringConverter::toString(index));
          vertexPoseKeyFrameMap[curSubMesh]->updatePoseReference(curPoseIndex,
                                                                 weight);
          // Dirty animation state since we're fudging this manually
          // If we don't notify dirty, Ogre doesn't know to update this state
          // and nothing happens
    ...
    for (int curSubMesh = 1; curSubMesh <= numSubMeshes; curSubMesh++) {
      try {
        ...
        if (patch) {
          ...
          vertexPoseKeyFrameMap[curSubMesh]->updatePoseReference(
              curPoseIndex + curSubMesh - 1, weight);
          // Dirty animation state since we're fudging this manually
          // If we don't notify dirty, Ogre doesn't know to update this state
          // and nothing happens
          head->getAnimationState("manl" + head->getName())
