...
the basic resources required for the progress bar and will be loaded
automatically.
*/
...
/** Defines an example loading progress bar which you can use during
        startup, level changes etc to display loading progress.
@remarks
        Basically you just need to create an instance of this class, call
start() before loading and finish() afterwards. You may also need to stop areas
of your scene rendering in between since this method will call
        RenderWindow::update() to update the display of the bar - we advise
using SceneManager's 'special case render queues' for this, see
...
@note
        ...
        resources required for the progress bar and will be loaded
automatically.
*/
class Run3LoadingBar : public ResourceGroupListener {
protected:
...
public:
  ...
  */
  virtual void start(RenderWindow *window, unsigned short numGroupsInit = 1,
    ...
    mLoadingDescriptionElement =
        omgr.getOverlayElement("Core/LoadPanel/Description");
  ...
  void resourceGroupScriptingStarted(const String &groupName,
                                     size_t scriptCount) {
    assert(mNumGroupsInit > 0 &&
           "You stated you were not going to init "
  ...
  void resourceGroupLoadStarted(const String &groupName, size_t resourceCount) {
    assert(mNumGroupsLoad > 0 &&
           "You stated you were not going to load "
           "any groups, but you did! Divide by zero would follow...");
    mProgressBarInc =
        mProgressBarMaxSize * (1 - mInitProportion) / (Real)resourceCount;
    mProgressBarInc /= mNumGroupsLoad;
    mLoadingDescriptionElement->setCaption("Finished!");
    mWindow->update();
  }
  ...
  void resourceGroupLoadEnded(const String &groupName) {
    mLoadingDescriptionElement->setCaption("Loading sounds...");
  }
  ...
  bool resourceCollision(ResourcePtr &resource,
