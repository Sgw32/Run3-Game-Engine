...
public:
  ...
  void CreateObject(String name, String mesh, Ogre::SceneNode *node,
                    Ogre::Real mass, bool idleanim, String idle);
  // void CreateObject(String mesh,Ogre::SceneNode *node,Ogre::Real mass,bool
  ...
  void CreateStaticCylTransparent(String name, Ogre::SceneNode *node,
                                  Real radius, Real height);
  void CreateStaticCyl(String name, Ogre::SceneNode *node, Real radius,
                       Real height);
  void DeleteObject();
  void set_breakable(Real health, Real strength) {
    mHealth = CurHealth = health;
    mStrength = strength;
    breakable = true;
    LogManager::getSingleton().logMessage("PhysObj is now breakable");
  }
  void explosive() { isExplosive = true; }
  void setMaxDist(Real maxDist) { dist = maxDist; }
  ...
  void setGibScale(Vector3 gscale) { mGibScale = gscale; }
  void setGibCount(unsigned int c) { gc = c; }
  void forceDelete();
  void brk();
  ...
  void addToBatch();
  String getName() { return bod->getName(); }
  Quaternion get_orient(void) {
    Vector3 pos;
    Quaternion orient;
  ...
  void attachToBone2(Ogre::Node *bone, Ogre::SceneNode *parent) {
    ...
    deltaToObject = get_pos() - (attachedToBone->_getDerivedPosition() *
  ...
  void attachToBone(Ogre::Node *bone, Ogre::SceneNode *parent) {
    ...
    gravity = 0;
    LogManager::getSingleton().logMessage("ATTACH TO BONE!" +
                                          StringConverter::toString(bone == 0));
    // plPos=bone->_getDerivedPosition();
  }
  ...
  void updateAttached() {
    if (attachedToParent && attachedToBone) {
      bod->setPositionOrientation(
          deltaToObject +
              attachedToParent->_getDerivedOrientation() *
                  attachedToBone->_getDerivedPosition() *
                  attachedToParent->_getDerivedScale() +
              attachedToParent->_getDerivedPosition(),
          deltaQuat * attachedToBone->_getDerivedOrientation());
    }
  }
