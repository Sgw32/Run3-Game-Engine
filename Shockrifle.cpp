...
void Shockrifle::init(Ogre::Root *_root, SceneManager *sceneMgr,
                      SceneNode *weaponNode, Entity *_ent, SoundManager *sound,
  ...
  mWeaponState = weapon->getAnimationState(
      cf.getSetting("idleAnim", StringUtil::BLANK, "idle"));
  // mWeaponState->setWeight(0.1);
  mWeaponState->setWeight(StringConverter::parseReal(
      cf.getSetting("animWeight", StringUtil::BLANK, "0.1")));
  ...
  minusg = 0;
  minusb = 0;
  /* lShock = mSceneMgr->createParticleSystem("RedLightning",
   "Weapons/ShockLaser_univ"); lShock_t =
   mSceneMgr->createParticleSystem("RedLightning_tree", "Weapons/Shock_tree");
   ...
   //lGreen = mSceneMgr->createParticleSystem("GreenLightning",
   "Weapons/ShockLaser_g");
   //lBlue = mSceneMgr->createParticleSystem("BlueLightning",
   "Weapons/ShockLaser_b"); mLightenNode =
   weaponNode->createChildSceneNode(Vector3(0,0,1000)); mTreeNode =
   weaponNode->createChildSceneNode(Vector3(0,0,1000));
  ...
  MeshManager::getSingleton().createPlane(
      ...
      ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, plane, 10, 200, 1, 1,
      true, 1, 1, 1, Vector3::UNIT_Z);
  MeshManager::getSingleton().createPlane(
      "Shockrifle_Light2_n" + _ent->getName(),
      ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, plane2, 10, 200, 1, 1,
      true, 1, 1, 1, Vector3::UNIT_Z);
  ...
  // mater =
  // (MaterialPtr)MaterialManager::getSingleton().getByName("lightning_gun");
  mater = (MaterialPtr)MaterialManager::getSingleton().getByName(
      cf.getSetting("shockMaterial", StringUtil::BLANK, "lightning_gun"));

  ent->setCastShadows(false);
  ent2->setCastShadows(false);
  mLightenNode->attachObject(ent);
  mLightenNode->attachObject(ent2);
  lShock_t = mSceneMgr->createParticleSystem(
      "RedLightning_tree" + _ent->getName(), "Weapons/Shock_tree");
...
void Shockrifle::MousePress(const OIS::MouseEvent &arg, OIS::MouseButtonID id) {
  ...
  if (id == OIS::MB_Left) {
    ...
    Run3SoundRuntime::getSingleton().emitSound("run3/sounds/beamstart10.wav", 2,
                                               false);
      ...
      for (i = 0; i != pobj.size(); i++) {
        if (pobj[i]->isThisPO(bod)) {
          ...
          if (!right) {
            ...
            // bullet->init(mPlayerNode->getPosition()+mPlayerNode->getOrientation()*Vector3(0,0,-100),dir,1000000,
            // mWorld,mSceneMgr); bullet->start(); USE BULLETS FROM HERE!!!
...
bool Shockrifle::frameEnded(const Ogre::FrameEvent &evt) { return true; }