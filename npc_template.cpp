...
bool npc_template::find_path() {
  ...
  // neutral"+StringConverter::toString(pos->getPosition()));
  // LogManager::getSingleton().logMessage("to
...
bool npc_template::find_path(Vector3 destPos) {
  ...
  Vector3 myPos = getpos(npcBody);
  bool straight =
      check_straight(myPos, destPos) && (!cmdMode); // if cmdmode - bypass this
...
void npc_template::start() { going = true; }

void npc_template::suspend() { going = false; }
...
void npc_template::setDirection(Node *t, const Vector3 &vec,
  ...
  if ((currentDir + targetDir).squaredLength() < 0.00005f) {
    ...
    targetOrientation = Quaternion(-currentOrient.y, -currentOrient.z,
  ...
  if (t->getParent())
    t->setOrientation(t->getParent()->_getDerivedOrientation().UnitInverse() *
                      targetOrientation);
  else
    t->setOrientation(targetOrientation);
}

void npc_template::lookAt(Node *t, const Vector3 &targetPoint,
                          Node::TransformSpace relativeTo,
                          const Vector3 &localDirectionVector) {
...
void npc_template::processBaseEvent(int flag, String param1, String param2) {
  ...
  if (flag == SETANIM_NPC) {
    ...
    if (animated) {
      LogManager::getSingleton().logMessage("Setting anim..." +
                                            heliEnt->getName());
      ...
      LogManager::getSingleton().logMessage(
          "Animation test." + mAnimState->getAnimationName() + "fsd" +
          StringConverter::toString(mAnimState->getEnabled()));
    }
  }
