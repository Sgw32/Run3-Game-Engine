...
void Loader::init(SceneManager *scene, Viewport *LeftViewport,
...
String Loader::getPrefix() { return mapPrefix; }
...
void Loader::parseGFX() {
  ...
  material_disable =
      StringConverter::parseBool(cf.getSetting("MaterialDisable", "", "false"));
  SoftwareOcclusionCulling::getSingleton().setEnabled(
      StringConverter::parseBool(cf.getSetting("OCEnabled", "", "false")));
  ...
  if (StringConverter::parseBool(cf.getSetting("enableGlow", "", "true"))) {
    ...
    CompositorManager::getSingleton().removeCompositor(mCamera->getViewport(),
                                                       "Glow");
  ...
  if (StringConverter::parseBool(cf.getSetting("COMEnable", "", "false"))) {
    ...
    LogManager::getSingleton().logMessage("Serial: init COM" +
                                          cf.getSetting("COMPort", "8"));
    if (!global::getSingleton().port->Open(port, baud))
      LogManager::getSingleton().logMessage("error");
  }

  rtt_display =
      StringConverter::parseBool(cf.getSetting("rttDisplay", "", "true"));
  if (result == "depthshadowmap") {
    PCF = StringConverter::parseBool(
        cf.getSetting("pcfEnable", StringUtil::BLANK, "false"));
    // Allow self shadowing (note: this only works in conjunction with the
    ...
    //  You can switch this on or off, I suggest you try both and see which
    //  works best for you
    ...
    mSceneMgr->setShadowTextureCasterMaterial(
        "Ogre/DepthShadowmap/Caster/Float");
  }
  ...
  if (result == "run3shadow") {
    ...
    // float 32 works a lot better with a low/none VSM epsilon (wait till the
    // shaders) but float 16 is good enough and supports bilinear filtering on a
    // lot of cards (we should use _GR, but OpenGL doesn't really like it for
    // some reason)
  ...
  if (result == "deferred") {
    // iSystem
    DeferredShadingSystem *mSystem = new DeferredShadingSystem(
        global::getSingleton().getWindow()->getViewport(0),
        global::getSingleton().getSceneManager(),
        global::getSingleton().getCamera());
  ...
  //
  mCurrentShadowCameraSetup = ShadowCameraSetupPtr(dscs);
  ...
  if (result == "r3pssm") {
    // mSceneMgr->addListener(new
    // PSSMShadowListener(mSceneMgr,light,mCurrentShadowCameraSetup,mCamera));
  ...
  mSceneMgr->setShadowFarDistance(StringConverter::parseReal(
      cf.getSetting("shadowDist", StringUtil::BLANK, "100000")));
  if (result == "r3pssm2") {
    mSceneMgr->setShadowTechnique(Ogre::SHADOWTYPE_TEXTURE_ADDITIVE_INTEGRATED);
    mSceneMgr->setShadowFarDistance(50000);
    mSceneMgr->setShadowTextureCountPerLightType(Ogre::Light::LT_DIRECTIONAL,
                                                 3);
    mSceneMgr->setShadowTextureCount(3);
    ...
    pssmSetup->calculateSplitPoints(
        3, global::getSingleton().getCamera()->getNearClipDistance(),
        mSceneMgr->getShadowFarDistance());
    pssmSetup->setSplitPadding(
        global::getSingleton().getCamera()->getNearClipDistance());
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      LogManager::getSingleton().logMessage(
          StringConverter::toString(splitPoints[i]) + " number " +
  ...
  bool hdron = StringConverter::parseBool(
      cf.getSetting("HDREnable", StringUtil::BLANK, "true"));
  if (hdron) {
    /*CompositorManager::getSingleton().addCompositor(mainView, "NewHDR");
    CompositorManager::getSingleton().setCompositorEnabled(mainView, "NewHDR",
    true);*/
    SuperFX::getSingleton().setHDREnabled(true);
  }
  SuperFX::getSingleton().allowMotionBlur(StringConverter::parseBool(
      cf.getSetting("allowMotionBlur", StringUtil::BLANK, "true")));

  texLOD = StringConverter::parseInt(
      cf.getSetting("textureLOD", StringUtil::BLANK, "-1"));
  meshLOD = StringConverter::parseInt(
      cf.getSetting("meshLOD", StringUtil::BLANK, "-1"));
  global::getSingleton().setMeshLod(meshLOD);
  global::getSingleton().setTexLod(texLOD);
  stereo = StringConverter::parseBool(
      cf.getSetting("enableStereo", StringUtil::BLANK, "false"));
  if (stereo) {
    try {
      // try to load the config file
      mStereoManager.init(gLeftViewport, gRightViewport,
                          "run3/core/stereo.cfg");
...
void Loader::enableSSAO() {
  ssao = Ogre::CompositorManager::getSingleton().addCompositor(
      global::getSingleton().getCamera()->getViewport(), "ssao");
  ssao->setEnabled(true);
  ssao->addListener(&ssaoParamUpdater);
}

void Loader::enaSSAO() { ssao->setEnabled(true); }

void Loader::disableSSAO() { ssao->setEnabled(false); }
