...
public:
  ...
  static int startSeqScript(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      CutScene *cscene;
      for (unsigned int i = 0; i != Sequence::getSingleton().cutscenes.size();
           i++) {
  ...
  static int enableTimer(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      Run3Timer *cscene = 0;
      for (unsigned int i = 0; i != Sequence::getSingleton().timers.size();
           i++) {
  ...
  static int rotateFuzzyObject(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3)) {
      ...
      Quaternion tex =
          Quaternion(
              Radian(Degree(StringConverter::parseReal(lua_tostring(pL, 2)))),
              Vector3(1, 0, 0)) *
          Quaternion(
              Radian(Degree(StringConverter::parseReal(lua_tostring(pL, 3)))),
              Vector3(0, 0, 1));
      ...
      for (unsigned int i = 0; i != Sequence::getSingleton().fobj.size(); i++) {
        LogManager::getSingleton().logMessage(
            "Name: " + Sequence::getSingleton().fobj[i]->getname());
  ...
  static int getFov(lua_State *pL) {
    lua_pushnumber(
        pL, global::getSingleton().getCamera()->getFOVy().valueDegrees());
    return 1;
  }

  static int exitAllComputers(lua_State *pL) {
    vector<Computer *>::iterator itc =
        Sequence::getSingleton().computers.begin();
    // for (itc =
    // Sequence::getSingleton().computers.begin();itc!=Sequence::getSingleton().computers.end();itc++)
    (*itc)->logoff();
    return 1;
  }

  static int processDotScene(lua_State *pL) { return 1; }
  ...
  static int createParticleSystem(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
      ...
      ParticleSystem *p =
          global::getSingleton().getSceneManager()->createParticleSystem(
              lua_tostring(pL, 2), tex);
      SceneNode *n = global::getSingleton()
                         .getSceneManager()
                         ->getRootSceneNode()
  ...
  static int setSkyBox(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      if (tex == "none") {
        global::getSingleton().getSceneManager()->setSkyBox(
            false, tex, 6000, true, Quaternion::IDENTITY, "General");
      } else {
        global::getSingleton().getSceneManager()->setSkyBox(
            true, tex, 6000, true, Quaternion::IDENTITY, "General");
      }
    }
    return 1;
  }
  ...
  static int deleteParticleSystem(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      ...
      if (global::getSingleton().getSceneManager()->hasParticleSystem(tex)) {
        ParticleSystem *p =
            global::getSingleton().getSceneManager()->getParticleSystem(tex);
  ...
  static int random_number(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      lua_pushnumber(
          pL, Ogre::Math::RangeRandom(0, StringConverter::parseReal(tex)));
    }
    return 1;
  }
  ...
  static int gameText(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3)) {
      String tex = lua_tostring(pL, 1);
      HUD::getSingleton().flashText(
          tex, StringConverter::parseReal(lua_tostring(pL, 2)),
  ...
  static int emitSound2(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3) &&
        lua_isstring(pL, 4) && lua_isstring(pL, 5)) {
      String tex = lua_tostring(pL, 1);
      Real dur = StringConverter::parseReal(lua_tostring(pL, 2));
      Run3SoundRuntime::getSingleton().emitSound(
          tex, dur, false,
          Vector3(StringConverter::parseReal(lua_tostring(pL, 3)),
                  StringConverter::parseReal(lua_tostring(pL, 4)),
  ...
  static int getPixelCount1(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      ...
      CustomSceneManager *mgr =
          (CustomSceneManager *)global::getSingleton().getSceneManager();
      ...
      if (mgr->hasEntity(tex)) {
        ...
        //Ogre::Vector3 hcsPosition = _camera->getProjectionMatrix() *
        _camera->getViewMatrix() * _position;
        //return Ogre::Vector2(0.5 - hcsPosition.x * 0.5f, hcsPosition.y * 0.5f
        + 0.5);
  ...
  static int getPixelCount2(lua_State *pL) {
    CustomSceneManager *mgr =
        (CustomSceneManager *)global::getSingleton().getSceneManager();
    LogManager::getSingleton().logMessage(
        StringConverter::toString(mgr->getPixelCount()));
    return 1;
  }
  ...
  static int commandLeaderGroup(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3)) {
      ...
      String tex1 = lua_tostring(pL, 2);
      String tex2 = lua_tostring(pL, 3);
      AIManager::getSingleton().commandGroup(
          tex, tex1, tex2); // Do all AI calculations - in real world there is
                            // no second step
      AIManager::getSingleton().processGroupCommands(
          tex); // Output them (as we are in the 3d reality)
    }
    return 1;
  }
  ...
  static int toggleParticleSystem(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      ...
      if (mgr->hasParticleSystem(tex)) {
        mgr->getParticleSystem(tex)->setVisible(
            !mgr->getParticleSystem(tex)->getVisible());
      } else {
        return 0;
      }
    }
  ...
  static int toggleEmitter(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      ...
      SceneManager *mgr = global::getSingleton().getSceneManager();
      if (mgr->hasParticleSystem(tex)) {
        mgr->getParticleSystem(tex)->getEmitter(0)->setEnabled(
            !mgr->getParticleSystem(tex)->getEmitter(0)->getEnabled());
      } else {
        return 0;
      }
    }
  ...
  static int enableTrigger(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      Trigger *cscene;
      for (unsigned int i = 0; i != Sequence::getSingleton().triggers.size();
           i++) {
  ...
  static int fireFire(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      Fire *firee;
      for (unsigned int i = 0; i != Sequence::getSingleton().fires.size();
           i++) {
  ...
  static int materialEntity(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2)) {
      ...
      SceneManager *mgr = global::getSingleton().getSceneManager();
      if (mgr->hasEntity(tex)) {
        LogManager::getSingleton().logMessage("Setting material to entity:" +
                                              tex);
        LogManager::getSingleton().logMessage(tex2);
        mgr->getEntity(tex)->getSubEntity(0)->setMaterial(
            MaterialManager::getSingleton().getByName(tex2));
      } else {
        return 0;
      }
    }
  ...
  static int setCompositorEnab(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2)) {
      ...
      if (enabled)
        CompositorManager::getSingleton().addCompositor(
            global::getSingleton().getCamera()->getViewport(), tex);

      CompositorManager::getSingleton().setCompositorEnabled(
          global::getSingleton().getCamera()->getViewport(), tex, enabled);
      if (!enabled)
  ...
  static int fragmentGPUProgramParams(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3)) {
      LogManager::getSingleton().logMessage(
          "Setting fragment program param...");
      String tex = lua_tostring(pL, 1);
      /*Ogre::HighLevelGpuProgramPtr mFragmentProgram =
      GpuProgramManager::getSingleton().getByName(tex);
      LogManager::getSingleton().logMessage(mFragmentProgram->getName());
      GpuProgramParametersSharedPtr gprpar =
      mFragmentProgram->getDefaultParameters();
      //LogManager::getSingleton().logMessage(gprpar->getNamedConstant
      gprpar->setNamedConstant("sater", 0.1f);*/
      MaterialPtr mActiveMaterial =
          Ogre::MaterialManager::getSingleton().getByName(tex);
      GpuProgramPtr mActiveFragmentProgram =
          mActiveMaterial->getSupportedTechnique(0)
              ->getPass(0)
              ->getFragmentProgram();
      ...
      mActiveFragmentParameters->setNamedConstant(
          lua_tostring(pL, 2), StringConverter::parseReal(lua_tostring(pL, 3)));
      // LogManager::getSingleton().logMessage(StringConverter::toString(mFragmentProgram->setParameter(lua_tostring(pL,
      // 2),lua_tostring(pL, 3))));
      // mFragmentProgram->setParameter(lua_tostring(pL, 2),lua_tostring(pL,
      // 3));
    }
    return 1;
  }
  ...
  static int fragmentGPUProgramTime(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      LogManager::getSingleton().logMessage(
          "Getting fragment program param...");
      String tex = lua_tostring(pL, 1);
      /*Ogre::HighLevelGpuProgramPtr mFragmentProgram =
      GpuProgramManager::getSingleton().getByName(tex);
      LogManager::getSingleton().logMessage(mFragmentProgram->getName());
    ...
    if (lua_isstring(pL, 1)) //&&lua_isstring(pL, 2))
    {
      ...
      for (int i = 0; i != pobj.size(); i++) {
        if (pobj[i]->getName() == tex) {
          ...
          lua_pushnumber(pL,pobj[i]->get_pos().z);*/
          lua_pushstring(pL,
                         StringConverter::toString(pobj[i]->get_pos()).c_str());
          return 1;
        }
      }
    }
  ...
  static int nod__getpos(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2)) {
      ...
      if (global::getSingleton().getSceneManager()->hasSceneNode(tex)) {
        if (tex2 == "x")
          lua_pushnumber(pL, (int)global::getSingleton()
                                 .getSceneManager()
                                 ->getSceneNode(tex)
                                 ->getPosition()
                                 .x);
  ...
  static int nod__getpos2(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) //&&lua_isstring(pL, 2))
    {
      ...
      if (global::getSingleton().getSceneManager()->hasSceneNode(tex)) {
        ...
        if (tex2=="z")
        lua_pushnumber(pL,pobj[i]->get_pos().z);*/
        lua_pushstring(pL, StringConverter::toString(global::getSingleton()
                                                         .getSceneManager()
                                                         ->getSceneNode(tex)
                                                         ->getPosition())
                               .c_str());
  ...
  static int bod__setpos(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3)) {
      ...
      vector<PhysObject *> pobj = global::getSingleton().pobjects;
      LogManager::getSingleton().logMessage("(Lua): Set Object Pos " +
                                            StringConverter::toString(tex3));
      for (int i = 0; i != pobj.size(); i++) {
        if (pobj[i]->getName() == tex) {
          ...
          if (tex2 == "x")
            pobj[i]->bod->setPositionOrientation(Vector3(tex3, pso.y, pso.z),
                                                 ori);
          if (tex2 == "y")
            pobj[i]->bod->setPositionOrientation(Vector3(pso.x, tex3, pso.z),
                                                 ori);
          if (tex2 == "z")
            pobj[i]->bod->setPositionOrientation(Vector3(pso.x, pso.y, tex3),
                                                 ori);
          return 1;
        }
      }
  ...
  static int bod__setpos2(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2)) //&&lua_isstring(pL, 3))
    {
      ...
      String tex2 = lua_tostring(pL, 2);
      vector<PhysObject *> pobj = global::getSingleton().pobjects;
      //			LogManager::getSingleton().logMessage("(Lua):
      //Set Object Pos "+StringConverter::toString(tex3));
      for (int i = 0; i != pobj.size(); i++) {
        if (pobj[i]->getName() == tex) {

          // Vector3 pso = pobj[i]->get_pos();
          Quaternion ori = pobj[i]->get_orient();
          pobj[i]->bod->setPositionOrientation(
              StringConverter::parseVector3(tex2), ori);
          return 1;
        }
      }
    }
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2)) //&&lua_isstring(pL, 3))
    {
      ...
      if (global::getSingleton().getSceneManager()->hasSceneNode(tex)) {
        global::getSingleton()
            .getSceneManager()
            ->getSceneNode(tex)
            ->setPosition(StringConverter::parseVector3(tex2));
        return 1;
      }
    }
    LogManager::getSingleton().logMessage("(Lua): Set Object not found");
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2) && lua_isstring(pL, 3)) {
      ...
      for (int i = 0; i != pobj.size(); i++) {
        if (pobj[i]->getName() == tex) {
          ...
          LogManager::getSingleton().logMessage(
              "(Lua): DEBUG" +
              StringConverter::toString(ori1.getPitch().valueDegrees()));
          LogManager::getSingleton().logMessage(
              StringConverter::toString(ori1.getYaw().valueDegrees()));
          LogManager::getSingleton().logMessage(
              StringConverter::toString(ori1.getRoll().valueDegrees()));
          if (tex2 == "pitch")
          ...
          LogManager::getSingleton().logMessage(
              StringConverter::toString(ori1.getRoll().valueDegrees()));
          pobj[i]->bod->setPositionOrientation(pso,
                                               pobj[i]->nod->getOrientation());
          return 1;
        }
      }
    }
  ...
  static int tostringl(lua_State *pL) {
    ...
    lua_pushstring(pL,
                   StringConverter::toString(int(lua_tonumber(pL, 1))).c_str());
    return 1;
  }
  ...
  static int tostringl2(lua_State *pL) {
    ...
    lua_pushstring(pL,
                   StringConverter::toString(Vector3(int(lua_tostring(pL, 1)),
                                                     int(lua_tostring(pL, 2)),
                                                     int(lua_tostring(pL, 3))))
                       .c_str());
    return 1;
  }
  ...
  static int turnoff__Module(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      ...
      global::getSingleton().restData[mod] =
          !global::getSingleton().restData[mod];
    }
    return 1;
  }
  ...
  static int startEvent(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      ...
      for (i = Sequence::getSingleton().s_events.begin();
           i != Sequence::getSingleton().s_events.end(); i++) {
        if ((*i)->getName() == tex) {
          (*i)->trigger();
        }
      }
  ...
  static int setSpeedTrain(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1), lua_isstring(pL, 2)) {
      String tex = lua_tostring(pL, 1);
      String tex2 = lua_tostring(pL, 2);
      Sequence::getSingleton().s_setSpeedTrain(
          tex, StringConverter::parseReal(tex2));
    }
  ...
  static int setAccTrain(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1), lua_isstring(pL, 2)) {
      String tex = lua_tostring(pL, 1);
      String tex2 = lua_tostring(pL, 2);
      Sequence::getSingleton().s_setAccTrain(tex,
                                             StringConverter::parseReal(tex2));
    }
  ...
  static int setFullCameraParent(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      String tex = lua_tostring(pL, 1);
      global::getSingleton().getPlayer()->setFullParentRelation(
          StringConverter::parseBool(tex));
    }
  ...
  static int teleport_rel(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      Vector3 tex = StringConverter::parseVector3(lua_tostring(pL, 1));
      global::getSingleton().getPlayer()->teleport(
          global::getSingleton().getPlayer()->get_location() + tex);
    }
