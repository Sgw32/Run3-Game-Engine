...
Pickup::Pickup() {
...
#endif
  mMatPair = new OgreNewt::MaterialPair(global::getSingleton().getWorld(),
                                        mMatDefault, physicalMat);
#ifdef DEBUG_AT_RELEASE
  ...
  LogManager::getSingleton().logMessage(
      "PICKUP DEBUG: successfully added phys-materials");
#endif
}

Pickup::~Pickup() {}

void Pickup::init(String event, String object, Vector3 pos, Quaternion rot,
                  Vector3 scale, String meshFile, unsigned int value) {
  mEvent = event;
  mObject = object;
  ...
  bod->setCustomForceAndTorqueCallback<Pickup>(&Pickup::camera_force_callback,
                                               this);
  bod->setAutoFreeze(0);
  mValue = value;
#ifdef DEBUG_AT_RELEASE
  LogManager::getSingleton().logMessage(
      "PICKUP DEBUG: Initialization succesful");
#endif
}
...
#ifdef DEBUG_AT_RELEASE
    LogManager::getSingleton().logMessage(
        "PICKUP DEBUG: Used or contact, firing and disposing");
#endif
  }
