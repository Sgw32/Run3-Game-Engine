...
String FacialAnimation::getAttrib(TiXmlElement *XMLNode, const String &attrib,
...
Real FacialAnimation::getAttribReal(TiXmlElement *XMLNode, const String &attrib,
                                    Real defaultValue) {
  if (XMLNode->Attribute(attrib.c_str()))
...
bool FacialAnimation::getAttribBool(TiXmlElement *XMLNode, const String &attrib,
                                    bool defaultValue) {
  if (!XMLNode->Attribute(attrib.c_str()))
    return defaultValue;
...
Entity *FacialAnimation::preInit(String meshFile, String name) {
  /*LogManager::getSingleton().logMessage( "[FacialAnimation] 1" );
          mesh = MeshManager::getSingleton().load(meshFile,
     ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
          ...
          head = global::getSingleton().getSceneManager()->createEntity(name,
     meshFile); LogManager::getSingleton().logMessage( "[FacialAnimation] 7" );
  ...
  try {
    ...
    mesh = MeshManager::getSingleton().load(
        meshFile, ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
    ...
    if (mesh->hasAnimation("manl" + name)) {
      head = global::getSingleton().getSceneManager()->createEntity(name,
                                                                    meshFile);
      // LogManager::getSingleton().logMessage( "[FacialAnimation] Getting
      // animation: manl"+name );
      ...
      // LogManager::getSingleton().logMessage( "[FacialAnimation] New
      // animation: manl"+name );
      Animation *anim = mesh->createAnimation("manl" + name, 0);
      // skip submesh 0 since it is the shared geometry, and we have no poses on
      // that
      for (int curSubMesh = 1; curSubMesh <= numSubMeshes; curSubMesh++) {
        // create the VertexTrack on this animation
        Ogre::VertexAnimationTrack *vt =
            anim->createVertexTrack(curSubMesh, VAT_POSE);
        ...
        while (poseList[curPoseIndex]->getTarget() == curSubMesh - 1) {
          ...
          vertexPoseKeyFrameMap[curSubMesh]->addPoseReference(curPoseIndex,
                                                              0.0f);
          curPoseIndex++;
        }
      }
...
void FacialAnimation::init(String xmlData) {
  LogManager::getSingleton().logMessage(
      "[FacialAnimation] Loading facial animation.");
  letters.clear();
  try {
    // Strip the path
    ...
    DataStreamPtr pStream =
        ResourceGroupManager::getSingleton().openResource(basename, "General");
    ...
    if (SequenceDoc->Error()) {
      ...
      LogManager::getSingleton().logMessage(
          "[FacialAnimation] TiXml reporated an error.");
      return;
    }
  } catch (...) {
    // rest=true;
    // We'll just log, and continue on gracefully
    LogManager::getSingleton().logMessage(
        "[FacialAnimation] SaveFile does not present,or TiXml reporated an "
        "error.");
    return;
  }
  ...
  if (String(SequenceRoot->Value()) != "facialanimation") {
    LogManager::getSingleton().logMessage(
        "[FacialAnimation] Error: Invalid .xml File. Missing "
        "<facialanimation>");
    return;
  }
  ...
  LogManager::getSingleton().logMessage(
      "[FacialAnimation] processing nodes...");
  ...
  while (pElement) {
    ...
    LogManager::getSingleton().logMessage(
        "[FacialAnimation] processing node...");
  }

  soundFile = getAttrib(SequenceRoot->FirstChildElement("file"), "name",
                        "run3/sounds/pogran/document01.wav");
  // duration =
  // getAttribReal(SequenceRoot->FirstChildElement("file"),"duration",3.0f);
...
void FacialAnimation::animate(Vector3 pos) {
  ...
  curstate = 0;
  LogManager::getSingleton().logMessage(
      "[FacialAnimation] Starting animation.");
...
void FacialAnimation::upd(Real tm) {
  try {
    ...
    if (((mAnimate)) && (curstate == 0)) {
      ...
      LogManager::getSingleton().logMessage(
          "[FacialAnimation] Pre-ready sequence.");
      if (letters[curstate].getIndex() != -1) {
        // manualKeyFrame->updatePoseReference(letters[curstate].getIndex(),
        // timeAnim/PROCESS_ANIM); manualAnimState->getParent()->_notifyDirty();
      ...
      if (timeAnim > PROCESS_ANIM) {
        ...
        curstate++;
        Run3SoundRuntime::getSingleton().emitSound(soundFile, duration, false,
                                                   mPos, 200, 50, 800);
        LogManager::getSingleton().logMessage(
            "[FacialAnimation] Pre-ready ended.");
      }
      LogManager::getSingleton().logMessage("[FacialAnimation] Ok pre.");
    }

    // if
    // (((mAnimate)&&(timeAnim>PROCESS_ANIM)&&(timeAnim<endAnim+PROCESS_ANIM))&&(curstate<letters.size()))
    if (((mAnimate) && (timeAnim > PROCESS_ANIM)) &&
        (curstate < letters.size())) {
      ...
      Real down = (letters[curstate].getSecond() - (timeAnim - PROCESS_ANIM)) /
                  (letters[curstate].getSecond());
      Real up = 1 - down;
      ...
      // manualKeyFrame->updatePoseReference(letters[curstate-1].getIndex(),
      // down);
    ...
    if ((mAnimate) && (timeAnim > endAnim + PROCESS_ANIM)) {
      // manualKeyFrame->updatePoseReference(letters[curstate].getIndex(),
      // (endAnim+timeAnim)/(endAnim+PROCESS_ANIM));
      // manualAnimState->getParent()->_notifyDirty();
      if (letters[curstate - 1].getIndex() != -1) {
        updatePose(letters[curstate - 1].getIndex(),
                   1.0f -
                       (timeAnim - (endAnim + PROCESS_ANIM)) / (PROCESS_ANIM));
      }

      /*if (letters[curstate].getIndex()!=-1)
      {
      updatePose(letters[curstate].getIndex(),
      (timeAnim-(endAnim+PROCESS_ANIM))/(PROCESS_ANIM));
      }*/
      timeAnim += tm;
