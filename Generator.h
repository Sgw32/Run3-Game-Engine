...
public:
  ...
  static int setPhysObject(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1) && lua_isstring(pL, 2)) {
      SharedLuaCallback::getSingleton().shared_genSetPO(
          lua_tostring(pL, 1),
          StringConverter::parseVector3(lua_tostring(pL, 2)));
    }
    return 1;
  }
  ...
  static int setCenter(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      SharedLuaCallback::getSingleton().shared_genSetCenter(
          StringConverter::parseVector3(lua_tostring(pL, 1)));
    }
    return 1;
  }
  static int setSpace(lua_State *pL) {
    ...
    if (lua_isstring(pL, 1)) {
      SharedLuaCallback::getSingleton().shared_genSetSp(
