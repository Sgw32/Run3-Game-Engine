...
void HUD::setHUDAllowed(bool allowed) { HUDAllowed = allowed; }

void HUD::SSBlood() {
  wTime3 = 0;
  bloodonscreen = true;
  CompositorManager::getSingleton().addCompositor(
      global::getSingleton().getCamera()->getViewport(), "ColorCorrection");
  CompositorManager::getSingleton().setCompositorEnabled(
      global::getSingleton().getCamera()->getViewport(), "ColorCorrection",
      true);
  MaterialPtr mActiveMaterial = Ogre::MaterialManager::getSingleton().getByName(
      "Ogre/Compositor/RunColorCorrection");
  GpuProgramPtr mActiveFragmentProgram =
      mActiveMaterial->getSupportedTechnique(0)
          ->getPass(0)
          ->getFragmentProgram();
  mActiveFragmentParameters = mActiveMaterial->getSupportedTechnique(0)
                                  ->getPass(0)
...
void HUD::setTextSize(Real size) {
  ...
  LogManager::getSingleton().logMessage(
      ...
      StringConverter::toString(
          ((TextAreaOverlayElement *)gMsgTextArea)->getCharHeight()));
  // unused -> replaced by flashPanel
}
...
void HUD::Hide_crosshair() { crosshair->hide(); }

void HUD::Show_crosshair() { crosshair->show(); }

bool HUD::IsVisibleCross() { return crosshair->isVisible(); }
...
void HUD::processFlashText2String(Real wTime) {
  ...
  else
  {
          ...
          {
          sym_count = flText2.length()*(fadeTime+stayTime-wTime)/(stayTime/2);
  // int, of course.
          }
          else
          {
  ...
  if ((wTime > fadeTime) && (wTime < (fadeTime + stayTime))) {
    sym_count =
        flText2.length() * (wTime - fadeTime) / (stayTime); // int, of course.
  } else {
...
bool HUD::frameRenderingQueued(const Ogre::FrameEvent &evt) {
  ...
  if (wTime2 <
      (fadeTime * 2 + stayTime)) // If so, it means that fade is in processing.
  {
    ...
    if (wTime2 < fadeTime) // Starting fade.
    {
      ...
      // "+StringConverter::toString(texcolor.a));
      gMsgTextArea->setColour(texcolor);
    }
    if (wTime2 >= fadeTime &&
        wTime2 <= (fadeTime + stayTime)) // Static or dynamic display.
    {
      ColourValue texcolor = gMsgTextArea->getColour();
      texcolor.a = 1;
  ...
  if (shootCrosshair) {
    crosshair->setScale(crosshair->getScaleX() * 1.2,
                        crosshair->getScaleY() * 1.2);
...
bool HUD::frameStarted(const Ogre::FrameEvent &evt) { return true; }
