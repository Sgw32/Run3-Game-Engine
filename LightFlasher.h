...
public:
  LightFlasher(Light *light, ColourValue maxColour) {
    ...
    LogManager::getSingleton().logMessage(
        "LightFlasher: light pass max color " +
        StringConverter::toString(maxColour) + " cur color " +
        StringConverter::toString(mColourF));
  }

  virtual Real getValue(void) const { return intensity; }
...
public:
  LightFlasherControllerFunction(WaveformType wavetype, Real frequency,
                                 Real phase)
