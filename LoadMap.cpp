...
LoadMap::LoadMap() { scriptOnExit = ""; }
LoadMap::~LoadMap() {}
void LoadMap::init(SceneManager *SceneMgr, OgreNewt::World *world,
                   Camera *camera, SoundManager *soundMgr, Player *ply) {
  if (!SceneMgr) {
  ...
  mnode = scene->getRootSceneNode()->createChildSceneNode("rootn",
                                                          Vector3(0, 0, 0));
  mWorld->setWorldSize(Vector3(-10000, -10000, -10000),
                       Vector3(10000, 10000, 10000));
  dsl->texture_copy_counter = 0;
}

void LoadMap::LoadM(const String &map, const String &title, const String &sdir,
  ...
  String skybox;
  ConfigFile cf;
  // String &map,String &title,String &sdir,String &mapdir,const String&
  // groupName, SceneManager *mSceneMgr
  fpath = mapdir + "/" + Loader::getSingleton().getPrefix() + map;
  spath = mapdir + "/" + sdir;
  LogManager::getSingleton().logMessage("loading map..");
  try {
    cf.load(mapdir + "/" + Loader::getSingleton().getPrefix() + map +
            "/scene.cfg");
    ...
    mWorld->setWorldSize(
        Vector3(atof(x1.c_str()), atof(y1.c_str()), atof(z1.c_str())),
        Vector3(atof(x2.c_str()), atof(y2.c_str()), atof(z2.c_str())));
    LogManager::getSingleton().logMessage("2");
    Run3Batcher::getSingleton().prepareBatch();
    LogManager::getSingleton().logMessage("3");
    dsl->parseDotScene(
        mapdir + "/" + Loader::getSingleton().getPrefix() + map + scenefile,
        groupName, mSceneMgr, mnode, "", mWorld, mCamera, sound, player);
    ...
    LogManager::getSingleton().logMessage(
        "[LoadMap] Error loading map. (Maybe doesn't exist?)");
...
void LoadMap::MergeM(String map) {
  ...
  dsl->mergeDotScene(map, "General", mSceneMgr, mnode, "", mWorld, mCamera,
                     sound, player);
}

void LoadMap::LoadPrev(const String &title, const String &sdir,
                       const String &mapdir, const String &groupName,
                       SceneManager *mSceneMgr) {
  ...
  try {
    cf.load(mapdir + "/" + Loader::getSingleton().getPrefix() + prevMap +
            "/scene.cfg");
    ...
    Run3Batcher::getSingleton().prepareBatch();
    dsl->parseDotScene(
        mapdir + "/" + Loader::getSingleton().getPrefix() + prevMap + scenefile,
        groupName, mSceneMgr, mnode, "", mWorld, mCamera, sound, player);
    Run3Batcher::getSingleton().buildBatch();
    SceneLoadOverlay::getSingleton().Hide_all();
    scriptOnExit = "";
