...
// FUNCTION(Modulator::modulate_scenenode_pos)(SceneNode* node,Vector3 dest,
// Real sec)
void Modulator::modulate_scenenode_pos(SceneNode *node, Vector3 dest,
                                       Real sec) {
  logic_erase(node);
...
void Modulator::modulator_complete_fov() {
  if (start_modulate_fov) {
    start_modulate_fov = false;
    fs1 = 0;
    LogManager::getSingleton().logMessage(StringConverter::toString(fovs) +
                                          "FS");
    LogManager::getSingleton().logMessage(StringConverter::toString(fovend) +
                                          "FE");
...
bool Modulator::frameStarted(const Ogre::FrameEvent &evt) {
  ...
  if (scenenodes_modulate.size() > 0) {
    ...
    for (unsigned int i = 0; i != scenenodes_modulate.size(); i++) {
      ...
      if (((scenenodes_dest[i] - scenenodes_modulate[i]->getPosition())
               .length() <= 0) ||
          (scenenodes_sec[i] == 0)) {
      ...
      // Vector3 trans =
      // (scenenodes_dest[i]-scenenodes_modulate[i]->getPosition())*evt.timeSinceLastFrame*20/scenenodes_sec[i];
      Vector3 trans =
          (scenenodes_dest[i] - scenenodes_spos[i]) * scenenodes_sec[i];
    ...
    while (stop) {
      stop = false;
      s = 0;
      for (vector<SceneNode *>::iterator i = scenenodes_modulate.begin();
           i != scenenodes_modulate.end(); i++) {
        if (!stop) {
  ...
  if (start_modulate_fov) {
    global::getSingleton().getPlayer()->changeFOV(
        fovs +
        (fovend - fs1) / fovvel * evt.timeSinceLastFrame); //*TIME_SHIFT);
    fovs = global::getSingleton().getPlayer()->getFOV();
    LogManager::getSingleton().logMessage(
        StringConverter::toString(fovs + (fovend - fs1) / fovvel *
                                             evt.timeSinceLastFrame *
                                             TIME_SHIFT) +
        "FOV");
    cvel -= evt.timeSinceLastFrame; //*TIME_SHIFT;
    LogManager::getSingleton().logMessage(StringConverter::toString(cvel) +
                                          "SEC");
    if (cvel < 0) {
