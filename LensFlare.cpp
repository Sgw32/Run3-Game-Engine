...
/* ------------------------------------------------------------------------- */
LensFlare::LensFlare(Vector3 LightPosition, Camera *camera,
                     SceneManager *SceneMgr, Real scale) {
  mSceneMgr = SceneMgr;
  mCamera = camera;
  mHidden = true;
  this->createLensFlare(scale);
...
/* ------------------------------------------------------------------------- */
void LensFlare::createLensFlare(Real scale) {
  Real LF_scale = scale;
  ...
  // mNode->rotate(Vector3::UNIT_Z,Degree(90));
}
...
void LensFlare::update() {
  ...
  LFvect.x = LFvect.z;
  LFvect.z = zx;
  // LFvect += Vector3(-64,-64,0);  // sprite dimension (to be adjusted, but not
  // necessary)
  ...
  // We redraw the lensflare (in case it was previouly out of the camera field,
  // and hidden)
