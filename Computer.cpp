...
void Computer::init(SceneNode *cComp, int type, String fileName) {
  ...
  OgreNewt::Collision *col =
      new OgreNewt::CollisionPrimitives::TreeCollision(mWorld, cComp, true);
...
void Computer::logoff() {
  if (Display::getSingleton().isVisible()) {
    ...
    LogManager::getSingleton().logMessage(
        "Computer: last FOV is. Reverting changes.");
    LogManager::getSingleton().logMessage(StringConverter::toString(mFov));
...
bool Computer::frameStarted(const Ogre::FrameEvent &evt) {
  ...
  if (notice) {
    Vector3 dist = global::getSingleton().getPlayer()->get_location() -
                   mComp->getPosition();
...
bool Computer::frameEnded(const Ogre::FrameEvent &evt) { return true; }
