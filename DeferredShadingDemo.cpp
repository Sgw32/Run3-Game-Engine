/**
Demo of Deferred Shading in OGRE using Multiple Render Targets and HLSL/GLSL
high level language shaders.
...
class _OgreSampleClassExport Sample_DeferredShading
    : public SdkSample,
...
public:
  Sample_DeferredShading() {
    ...
    mInfo["Description"] = "A sample implementation of a deferred renderer "
                           "using the compositor framework.";
    mInfo["Thumbnail"] = "thumb_deferred.png";
    mInfo["Category"] = "Lighting";
    mInfo["Help"] = "See http://www.ogre3d.org/wiki/index.php/Deferred_Shading "
                    "for more info";
  }
...
protected:
  ...
  void setupControls() {
    ...
    mTrayMgr
        ->createCheckBox(TL_TOPLEFT, "DeferredShading", "Deferred Shading", 220)
        ->setChecked(true, false);
    mTrayMgr->createCheckBox(TL_TOPLEFT, "SSAO", "Ambient Occlusion", 220)
        ->setChecked(false, false);
    mTrayMgr->createCheckBox(TL_TOPLEFT, "GlobalLight", "Global Light", 220)
        ->setChecked(true, false);
    mTrayMgr->createCheckBox(TL_TOPLEFT, "Shadows", "Shadows", 220)
        ->setChecked(true, false);

    // create a menu to choose the model displayed
    mDisplayModeMenu = mTrayMgr->createThickSelectMenu(
        TL_TOPLEFT, "DisplayMode", "Display Mode", 220, 4);
  ...
  void checkBoxToggled(CheckBox *box) {
    if (box->getName() == "SSAO") {
      ...
      mSceneMgr->setShadowTechnique(
          box->isChecked() ? SHADOWTYPE_TEXTURE_ADDITIVE : SHADOWTYPE_NONE);
  ...
  void createAtheneScene(SceneNode *rootNode) {
    ...
    MeshPtr pAthene = MeshManager::getSingleton().load(
        "athene.mesh", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
  ...
  void createKnotScene(SceneNode *rootNode) {
    ...
    MeshPtr pKnot = MeshManager::getSingleton().load(
        "knot.mesh", ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
    unsigned short src, dest;
  ...
  void setupContent(void) {
    ...
    if (caps->getNumMultiRenderTargets() < 2) {
      OGRE_EXCEPT(Exception::ERR_NOT_IMPLEMENTED,
                  "Your card does not support at least two simultaneous render "
                  "targets, so cannot "
                  "run this demo. Sorry!",
                  "DeferredShading::createScene");
    }
    ...
    mSystem =
        new DeferredShadingSystem(mWindow->getViewport(0), mSceneMgr, mCamera);
  ...
  void createSampleLights() {
    ...
    SceneNode *parentNode =
        mSceneMgr->getRootSceneNode()->createChildSceneNode("LightsParent");
    // Create light nodes
    ...
    for (vector<Light *>::type::iterator i = lights.begin(); i != lights.end();
         ++i) {
      Light *light = *i;
      ...
      ent->setRenderQueueGroup(
          DeferredShadingSystem::POST_GBUFFER_RENDER_QUEUE);
    ...
    for (size_t x = 0; x < s_to_top; ++x) {
      ...
      station_pos[x] = base + Vector3(Math::Sin(theta) * r, ascend * x,
                                      Math::Cos(theta) * r);
    }
    for (size_t x = s_to_top; x < stations; ++x) {
      float theta = ((float)x / stations_per_revolution) * 2.0f * Math::PI;
      station_pos[x] =
          base + Vector3(Math::Sin(theta) * r, h - ascend * (x - s_to_top),
                         Math::Cos(theta) * r);
    }
    // Create a track for the light swarm
    Animation *anim = mSceneMgr->createAnimation(
        "LightSwarmTrack", stations * seconds_per_station);
    ...
    for (unsigned int x = 0; x < nodes.size(); ++x) {
      ...
      for (size_t y = 0; y <= stations; ++y) {
        // Setup keyframes
        TransformKeyFrame *key = track->createNodeKeyFrame(
            y * seconds_per_station); // A start position
    ...
    SharedData::getSingleton().mMLAnimState =
        mSceneMgr->createAnimationState("LightSwarmTrack");
