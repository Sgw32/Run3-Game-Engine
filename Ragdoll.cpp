...
RagDoll::RagBone::RagBone(RagDoll *creator, OgreNewt::World *world,
                          RagDoll::RagBone *parent, Ogre::Bone *ogreBone,
                          Ogre::MeshPtr mesh, Ogre::Vector3 dir,
                          RagDoll::RagBone::BoneShape shape, Ogre::Vector3 size,
                          Ogre::Real mass) {
  mDoll = creator;
  mParent = parent;
  mOgreBone = ogreBone;
  ...
  // in the case of the cylindrical primitives, they need to be rotated to align
  // the main axis with the direction vector.
  ...
  case RagDoll::RagBone::BS_CAPSULE:
    col = new OgreNewt::CollisionPrimitives::Capsule(world, size.y, size.x,
                                                     orient, pos);
    break;

  case RagDoll::RagBone::BS_CONE:
    col = new OgreNewt::CollisionPrimitives::Cone(world, size.y, size.x, orient,
                                                  pos);
    break;

  case RagDoll::RagBone::BS_CYLINDER:
    col = new OgreNewt::CollisionPrimitives::Cylinder(world, size.y, size.x,
                                                      orient, pos);
    break;
  ...
  mBody->setCustomForceAndTorqueCallback<RagBone>(
      &RagDoll::RagBone::bone_force_callback, this);
...
RagDoll::RagBone::_makeConvexHull(OgreNewt::World *world, Ogre::MeshPtr mesh,
                                  Ogre::Real minWeight) {
  std::vector<Ogre::Vector3> vertexVector;

  // for this bone, gather all of the vertices linked to it, and make an
  ...
  invMatrix.makeInverseTransform(
      -mOgreBone->_getBindingPoseInversePosition(),
  ...
  for (unsigned int i = 0; i < num_sub; i++) {
    ...
    Ogre::SubMesh::BoneAssignmentIterator bai =
        submesh->getBoneAssignmentIterator();
    ...
    // pointer
    Ogre::HardwareVertexBufferSharedPtr v_sptr =
        v_data->vertexBufferBinding->getBuffer(p_elem->getSource());
    unsigned char *v_ptr = static_cast<unsigned char *>(
      ...
      if (vba.boneIndex == boneIndex) {
        ...
        if (vba.weight >= minWeight) {
          ...
          Ogre::LogManager::getSingletonPtr()->logMessage(
              "  vertex found! id:" +
              Ogre::StringConverter::toString(vba.vertexIndex));
        }
      }
    }
  ...
  //////////////////////////////////////////////////////////////////////////////////
  OgreNewt::ConvexCollision *col =
      new OgreNewt::CollisionPrimitives::ConvexHull(world, verts, numVerts);
...
RagDoll::RagDoll(Ogre::String filename, OgreNewt::World *world,
                 Ogre::SceneNode *node, Real lifetime) {
  init(filename, world, node);
  mLifetime = lifetime;
  lifet = true;
}

void RagDoll::init(Ogre::String filename, OgreNewt::World *world,
                   Ogre::SceneNode *node) {
  mLifetime = 10;
  lifet = false;
  mNode = node;
  ...
  if (Ogre::ResourceGroupManager::getSingleton().resourceExists("General",
                                                                basename)) {
    pStream = Ogre::ResourceGroupManager::getSingleton().openResource(
        basename, "General");
  ...
  if (doc->Error()) {
    ...
    Ogre::LogManager::getSingleton().logMessage(
        "[RagDoll module] The TiXmlDocument reported an error");
    delete doc;
    return;
  }
  ...
  if (!root) {
    // error!
    Ogre::LogManager::getSingleton().logMessage(
        "[RagDoll module] cannot find 'root' in xml file: " + filename);
    return;
  }
...
void RagDoll::_addAllBones(RagDoll::RagBone *parent, TiXmlElement *bone) {
  ...
  Ogre::Vector3 dir =
      Ogre::StringConverter::parseVector3(bone->Attribute("dir"));
  Ogre::Real length =
      Ogre::StringConverter::parseReal(bone->Attribute("length"));

  Ogre::Vector3 size =
      Ogre::StringConverter::parseVector3(bone->Attribute("size")) *
      mNode->getScale();
  ...
  // set offsets
  if (!parent) {
    Ogre::Quaternion offsetorient =
        (boneorient.Inverse()) * mNode->getOrientation();
    Ogre::Vector3 offsetpos =
        boneorient.Inverse() * (mNode->getPosition() - bonepos);
  ...
  if (parent) {
    ...
    if (!joint) {
      // error!
      Ogre::LogManager::getSingleton().logMessage(
          " ERROR! cannot find 'Joint' in xml file!");
      return;
    }

    Ogre::Vector3 jointpin =
        Ogre::StringConverter::parseVector3(joint->Attribute("pin"));
    ...
    Ogre::Real limit1 =
        Ogre::StringConverter::parseReal(joint->Attribute("limit1"));
    Ogre::Real limit2 =
        Ogre::StringConverter::parseReal(joint->Attribute("limit2"));

    Ogre::Vector3 jpos =
        mNode->_getFullTransform() * ogrebone->_getDerivedPosition();
    Ogre::Vector3 jpin = (mNode->getOrientation() *
                          parent->getOgreBone()->_getDerivedOrientation()) *
                         jointpin;

    _joinBones(jointtype, parent, me, jpos, jpin, limit1, limit2);
  }

  Ogre::LogManager::getSingleton().logMessage(" added bone from " +
                                              ogrebone->getName() + ".");
...
void RagDoll::_autoAddAllBones(RagDoll::RagBone *parent, Ogre::Bone *bone) {
  ...
  Ogre::LogManager::getSingleton().logMessage("Adding bone " +
                                              ogrebone->getName());
  ...
  if (parent) {
    ...
    Ogre::Vector3 jointpin =
        dir; // Ogre::StringConverter::parseVector3( joint->Attribute("pin") );
...
RagDoll::RagBone *RagDoll::_addBone(OgreNewt::World *world,
                                    RagDoll::RagBone *parent, Ogre::Vector3 dir,
                                    RagBone::BoneShape shape,
                                    Ogre::Vector3 size, Ogre::Real mass,
                                    Ogre::Bone *ogrebone) {
  RagDoll::RagBone *bone = new RagDoll::RagBone(this, world, parent, ogrebone,
                                                mMesh, dir, shape, size, mass);
...
void RagDoll::_joinBones(RagDoll::JointType type, RagBone *parent,
                         RagBone *child, Ogre::Vector3 pos, Ogre::Vector3 pin,
                         Ogre::Real limit1, Ogre::Real limit2) {
  pin.normalise();
  OgreNewt::Joint *joint = NULL;

  switch (type) {
  case RagDoll::JT_BALLSOCKET:
    joint = new OgreNewt::BasicJoints::BallAndSocket(
        child->getBody()->getWorld(), child->getBody(), parent->getBody(), pos);
    ((OgreNewt::BasicJoints::BallAndSocket *)joint)
        ->setLimits(pin, Ogre::Degree(limit1), Ogre::Degree(limit2));
    break;

  case RagDoll::JT_HINGE:
    joint = new OgreNewt::BasicJoints::Hinge(child->getBody()->getWorld(),
                                             child->getBody(),
                                             parent->getBody(), pos, pin);
    ((OgreNewt::BasicJoints::Hinge *)joint)
        ->setCallback(RagBone::_hingeCallback);
...
void RagDoll::_placementCallback(OgreNewt::Body *me,
  ...
  if (!bone->getParent()) {
    ...
    // standard bone, calculate the local orientation between it and it's
    // parent.
    Ogre::Quaternion parentorient;
    Ogre::Vector3 parentpos;

    bone->getParent()->getBody()->getPositionOrientation(parentpos,
                                                         parentorient);
