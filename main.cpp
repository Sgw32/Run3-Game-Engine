...
#include "MainUtils.h"   //
class MainListener : public Run3FrameListener,
  ...
  MainListener(RenderWindow *win, Camera *cam, SceneManager *sceneMgr)
    ...
    if (backMap != "no_map_run3") {
      if ((backMap != "multiple") ||
          ((backMap == "multiple") && (backMaps.size() > 0))) {
        LogManager::getSingleton().logMessage(
            "Run3: Now loading background map...");
        loadBackgroundMap();
      }
    // soundMgr->playAudio( audioId, true );
  }
  static void Quit(vector<String> &param) { g_ShutdownRequested = true; }
  ...
  static void Map(vector<String> &param) {
    ...
    LoadMap::getSingleton().LoadM(param[1], "Ogre Testmap", "/scripts",
                                  "run3/maps", "General", MapSm);
    first = false;
    ingame = true;
    player->noclip = false;
  }
  static void Phys(vector<String> &param) {
    ...
    try {
      ...
      phys->CreateObject(param[1], param[2], MapSm->getSceneNode("rootn"),
                         100.0, false, "");
    } catch (Ogre::Exception & /*e*/) {
      LogManager::getSingleton().logMessage(
          "[Console] Error loading a physics object!");
  }
  static void hOverlay(vector<String> &param) { g_MenuOverlayUnsafe->hide(); }
  void hideOverlay(void) { mMenuOverlay->hide(); }
  void requestShutdown(void) { mShutdownRequested = true; }

  bool frameStarted(const FrameEvent &evt) {
    if (global::getSingleton().restData[17])
      return mContinue;
    // LogManager::getSingleton().logMessage("g1");
    global::getSingleton().getRoot()->getRenderSystem()->clearFrameBuffer(
    ...
    if (change) {
      ...
      player->noclip = false;
      HUD::getSingleton().Show();
      LoadMap::getSingleton().LoadM(global::getSingleton().mMap, "Ogre Testmap",
                                    "/scripts", "run3/maps", "General", MapSm);
      return true;
    }
  ...
  void loadBackgroundMap(void) {
    ...
    LoadMap::getSingleton().LoadM(mapToLoad, "Ogre Testmap", "/scripts",
                                  "run3/maps", "General", mSceneMgr);
  ...
  bool keyPressed(const OIS::KeyEvent &arg) {
    ...
    Run3Input::getSingleton().processPress(
        arg, global::getSingleton().GUIorGame, ingame);
  ...
  bool keyReleased(const OIS::KeyEvent &arg) {
    // ingame noclip
    Run3Input::getSingleton().processRelease(
        arg, global::getSingleton().GUIorGame, ingame);
    // ingame
    if (!global::getSingleton().GUIorGame &&
        !OgreConsole::getSingleton().isVisible()) {
...
private:
  ...
  SceneNode *mCamNode;     // SceneNode,     
  Overlay *mMenuOverlay;
  bool mShutdownRequested;
};
class Run3App : public Run3Application {
public:
  Run3App() : mGUISystem(0), mGUIRenderer(0), mEditorGuiSheet(0) {
    new OgreConsole();
    new LoadMap;
...
protected:
  ...
  void loadResources(void) {
    ...
    ResourceGroupManager::getSingleton().loadResourceGroup(
        ResourceGroupManager::getSingleton().getWorldResourceGroupName(), false,
        true);
  ...
  void loadNewGame(void) {
    ...
    LoadMap::getSingleton().LoadM(
        mChCtrl.getMapToLoad(), "Ogre Testmap", "/scripts", "run3/maps",
        "General",
        mSceneMgr); // chapter01,"Ogre
                    // Testmap","/scripts","run3/maps","General",mSceneMgr);
    first = false;
    ingame = true;
    player->noclip = false;
  }
  ...
  void loadSaveGame(void) {
    ...
    if (!first) {
      LoadMap::getSingleton().UnloadM(MapSm);
    HUD::getSingleton().Show();
    LoadMap::getSingleton().LoadM(SaveGame::getSingleton().openLast(),
                                  "Ogre Testmap", "/scripts", "run3/maps",
                                  "General", mSceneMgr);
    first = false;
    ingame = true;
    player->noclip = false;
  ...
  void createScene(void) {
    ...
    physUpdate =
        StringConverter::parseReal(cfg.getSetting("PhysUPD", "", "60"));
    backMap = cfg.getSetting("backgroundMap", "", "no_map_run3");

    if (backMap == "multiple") {
      LogManager::getSingleton().logMessage("Run3 multiple background!");
      backMaps_number = StringConverter::parseUnsignedInt(
          cfg.getSetting("backMapsN", "", "0"));
      backMaps = cfg.getMultiSetting("backMap", "");
    // ai config
    cfg.load("run3/core/ai.cfg");
    AIType = StringConverter::parseInt(cfg.getSetting("aiType"));
    Vector3 genNodes(StringConverter::parseReal(cfg.getSetting("XGenNode")),
                     StringConverter::parseReal(cfg.getSetting("YGenNode")),
                     StringConverter::parseReal(cfg.getSetting("ZGenNode")));
    bool autoGen =
        StringConverter::parseBool(cfg.getSetting("autoCreateNPCNodes"));
    // MaterialManager::
    // GUI Def
    mGUIRenderer = new CEGUI::OgreCEGUIRenderer(
        mWindow, Ogre::RENDER_QUEUE_OVERLAY, true, 3000, mSceneMgr);
...
if(RenderName == "Direct3D9 Rendering Subsystem")
{
 ...
 if(DX9Device != 0)
  {
       Ogre::LogManager::getSingleton().logMessage("MagicDXManager: ok , getting
D3DDEVICE\n");
  }
 InitialMagicLibrary(DX9Device,screenwidth,screenheight);
      }*/
    // cfg.load("run3/core/magic.cfg");
    MagicManager::getSingleton().init(
        mCamera, mWindow, ""); // cfg.getSetting("magicPath","","run3/"));

    // camera defs
    /*nodeC = mSceneMgr->getRootSceneNode()->createChildSceneNode("CamNode1",
    Vector3(-400, 200, 400)); nodeC->yaw(Degree(-45));
    ...
    soundMgr->setListenerPosition(Vector3(0, 0, 0), Vector3::ZERO,
                                  Quaternion::IDENTITY);
  ...
  void createFrameListener(void) {
    ...
    mOgreNewtListener = new OgreNewt::BasicFrameListener(
        mWindow, mCamera, mSceneMgr, mWorld, physUpdate);
  ...
  /*
      static void Quit(vector<String>& param)
      {
         CEGUI::WindowManager& qwmgr = CEGUI::WindowManager::getSingleton();
             qwmgr.getWindow((CEGUI::utf8*)"Run3Gui/TabCtrl/Page1/QuitButton")->fireEvent(CEGUI::PushButton::EventClicked,
     CEGUI::EventArgs());
      }*/
  void setupEventHandlers(void) {
    ...
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/QuitButton")
        ->setText(quit);
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/QuitButton")
        ->subscribeEvent(CEGUI::PushButton::EventClicked,
                         CEGUI::Event::Subscriber(&Run3App::handleQuit, this));
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/QuitButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventMouseEnters,
            CEGUI::Event::Subscriber(&Run3App::handleMoveExit, this));
    // new button
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/NewButton")
        ->setText(newgame);
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/NewButton")
        ->subscribeEvent(CEGUI::PushButton::EventClicked,
                         CEGUI::Event::Subscriber(&Run3App::handleNew, this));
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/NewButton")
        ->subscribeEvent(CEGUI::PushButton::EventMouseEnters,
                         CEGUI::Event::Subscriber(&Run3App::handleMove, this));
    // new game test button

    // Options

    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/NewButton1")
        ->setText(options_t);
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/NewButton1")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handleOptions, this));
    wmgr.getWindow((CEGUI::utf8 *)"Run3Gui/TabCtrl/Page1/NewButton1")
        ->subscribeEvent(CEGUI::PushButton::EventMouseEnters,
                         CEGUI::Event::Subscriber(&Run3App::handleMove, this));
  ...
  void setupNewHandler(void) {
    ...
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton")
        ->setText(newgame2);
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handleNewGame, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventMouseEnters,
            CEGUI::Event::Subscriber(&Run3App::handleMoveNewG, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton1")
        ->setText(newgame3);
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton1")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handleLoadGame, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton1")
        ->subscribeEvent(
            CEGUI::PushButton::EventMouseEnters,
            CEGUI::Event::Subscriber(&Run3App::handleMoveNewG, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/PrvButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventMouseEnters,
            CEGUI::Event::Subscriber(&Run3App::handleMoveNewG, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/NextButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventMouseEnters,
            CEGUI::Event::Subscriber(&Run3App::handleMoveNewG, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/PrvButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handlePrevChapter, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/NextButton")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handleNextChapter, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/PrvButton")->hide();

    mChCtrl.setPrevNextWindows(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/PrvButton"),
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/NextButton"));
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/"), "tlwintro");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/2"), "tlwhome01");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/3"), "tlwhome02");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/4"),
        "tlwstations01");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/5"),
        "tlwstations02");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/6"),
        "tlwstations03");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/7"), "tlwdolg");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/8"), "tlwcao");
    mChCtrl.pushBackChapter(
        wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/9"), "tlwoutro");
    mChCtrl.finalize();
    // wmgr2.getWindow((CEGUI::utf8*)"Run3Win2/MainWindow/")->setLookNFeel("TaharezLook/StaticShared3T");
    // wmgr2.getWindow((CEGUI::utf8*)"Run3Win2/MainWindow/")->setProperty("Image",
    // "Chapter01/ClientBrush"); String dta =
    // Ogre::String(wmgr2.getWindow((CEGUI::utf8*)"Run3Win2/MainWindow/")->getProperty("Image").c_str());
    // LogManager::getSingleton().logMessage(dta);
    // CEGUI::Window *mChapter =
    // wmgr2.getWindow((CEGUI::utf8*)"Run3Win2/MainWindow/NewGameButton1");
    // mChapter->getProperty
    // Options
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton2")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handleOptionsAccept, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton2")
        ->subscribeEvent(CEGUI::PushButton::EventMouseEnters,
                         CEGUI::Event::Subscriber(&Run3App::handleMove, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton3")
        ->subscribeEvent(
            CEGUI::PushButton::EventClicked,
            CEGUI::Event::Subscriber(&Run3App::handleOptionsCancel, this));
    wmgr2.getWindow((CEGUI::utf8 *)"Run3Win2/MainWindow/NewGameButton3")
        ->subscribeEvent(CEGUI::PushButton::EventMouseEnters,
                         CEGUI::Event::Subscriber(&Run3App::handleMove, this));
  }
  ...
  bool handleOptionsAccept(const CEGUI::EventArgs &e) {
    // CEGUI::Slider* sldr =
    // (CEGUI::Slider*)CEGUI::WindowManager::getSingleton().getWindow((CEGUI::utf8*)"Run3Menu//11");
    CEGUI::Scrollbar *sldr =
        (CEGUI::Scrollbar *)CEGUI::WindowManager::getSingleton().getWindow(
            (CEGUI::utf8 *)"Run3Menu//11");
    // Mouse sens
    Real sens = sldr->getScrollPosition();
    player->setMouseSensivity(sens);
    // sldr =
    // (CEGUI::Slider*)CEGUI::WindowManager::getSingleton().getWindow((CEGUI::utf8*)"Run3Menu//1");

    CEGUI::Checkbox *alls =
        (CEGUI::Checkbox *)CEGUI::WindowManager::getSingleton().getWindow(
            (CEGUI::utf8 *)"Run3Menu//Subt");

    bool allowsubtitles = alls->isSelected();
    player->setAllowSubtitles(allowsubtitles);
    CEGUI::Window *editorWindow =
        CEGUI::WindowManager::getSingleton().getWindow((CEGUI::utf8 *)"root");
  ...
  bool handleNextChapter(const CEGUI::EventArgs &e) {
    if (!mChCtrl.getBusy()) {
      ...
      Run3SoundRuntime::getSingleton().emitSound(
          "run3/sounds/tank_readyfire1.wav", 2, false);
    return true;
  }
  ...
  bool handleNew(const CEGUI::EventArgs &e) {
    ...
    if (!menuShown) {
      ...
      mEditorGuiSheet = CEGUI::WindowManager::getSingleton().loadWindowLayout(
          (CEGUI::utf8 *)"run3menu.layout");

      CEGUI::Window *editorWindow =
          CEGUI::WindowManager::getSingleton().getWindow((CEGUI::utf8 *)"root");
      editorWindow->addChildWindow(
          mEditorGuiSheet->getChild((CEGUI::utf8 *)"Run3Win2/MainWindow"));
      editorWindow->addChildWindow(
          mEditorGuiSheet->getChild((CEGUI::utf8 *)"Run3Menu/"));
      editorWindow->getChild((CEGUI::utf8 *)"Run3Menu/")->hide();
